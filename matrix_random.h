...
auto random_matrix(mt_64i& mt, distribution&& d, std::size_t nrows, std::size_t ncols) {
    ...
    for (std::size_t i = 0; i < out.size(); ++i) out[i] = std::forward<distribution>(d)(mt);
    return out;
}

inline auto random_matrix_normal(mt_64i& mt, std::size_t nrows, std::size_t ncols, double mean = 0,
                                 double stddev = 1) {
    return random_matrix(mt, std::normal_distribution<double>{mean, stddev}, nrows, ncols);
}

inline auto random_matrix_exponential(mt_64i& mt, std::size_t nrows, std::size_t ncols,
                                      double lambda) {
