...
.DAT <- list(NR = d_NR, NMR = d_NMR, R = d_R, MR = d_MR, VR = d_VR, IR = d_IR)
.FB  <- list(NR = fB_NR, NMR = fB_NMR, R = fB_R, MR = fB_MR, VR = fB_VR, IR = fB_IR)
.FC  <- list(NR = fC_NR, NMR = fC_NMR, R = fC_R, MR = fC_MR, VR = fC_VR, IR = fC_IR)
.FD  <- list(R = fD_R, MR = fD_MR, VR = fD_VR, IR = fD_IR)     # the non-recursive pair has no update
.NAIVE <- c("NR", "NMR")
...
.DISP <- c(NR = "NR", NMR = "MNR", R = "R", MR = "MR", VR = "VR", IR = "IR")
