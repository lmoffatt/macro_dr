...
struct thermo_levenberg_mcmc {
    by_beta<levenberg_mcmc<Parameters>> walkers;
...
step_levenberg_thermo_mcmc_i(FunctionTable &&f, mt_64i &mt,
                             ...
                             levenberg_mcmc<Parameters> &current,
                             Prior const &prior, Likelihood const &lik,
                             const DataType &y, const Variables &x,
...
void step_levenberg_thermo_mcmc(FunctionTable &&f, std::size_t &iter,
    ...
    auto n_par = current.walkers[0].m_x.size();
...
//#pragma omp parallel for //collapse(2)
        for (std::size_t i_thread = 0; i_thread < num_threads; ++i_thread) {
            ...
            for (std::size_t iib = 0; (iib < n_beta_f)&&(i_thread * n_beta_f + iib<n_beta-1); iib+=2) {
                ...
                double logA =
                    calc_logA(beta[ib], beta[ib + 1], current.walkers[ib].m_logL,
                              current.walkers[ib + 1].m_logL);
                auto pJump = std::min(1.0, std::exp(logA));
                if (pJump > r) {
                    std::swap(current.walkers[ib], current.walkers[ib + 1]);
 ...
 void report(FunctionTable &&, std::size_t iter, const Duration &dur,
    ...
    if (iter % s.save_every == 0) {
            ...
            for (std::size_t i_beta = t_beta.size(); i_beta > 0; --i_beta) {
                auto logL0 = t_logL;
                double beta0 = beta;
                t_logL = current.walkers[i_beta - 1].get_logLs();
                beta = t_beta[i_beta - 1];
                auto plog_Evidence = (beta - beta0) * (logL0 + t_logL) / 2.0;
                log_Evidence =log_Evidence+ plog_Evidence;
                s.f  << iter << s.sep << dur << s.sep
                    << beta << s.sep << s.sep
                    << current.walkers[i_beta - 1].i_walker << s.sep
                    << get<logL>(current.walkers[i_beta - 1].m_logP)
                    << t_logL.sep(s.sep)
        ...
        if (iter % s.save_every == 0)
            for (std::size_t i_beta = 0; i_beta < t_beta.size(); ++i_beta)
                    for (std::size_t i_par = 0; i_par < num_Parameters(current); ++i_par)
                        ...
                        s.f << iter << s.sep << dur << s.sep
                            << t_beta[i_beta] << s.sep 
                            << current.walkers[i_beta].i_walker << s.sep << i_par << s.sep
                            << current.walkers[i_beta].m_x[i_par]<< s.sep
                            << get<Grad>(current.walkers[i_beta].m_logL)()[i_par]
                            << "\n";
    }
...
auto init_levenberg_thermo_mcmc(FunctionTable &&f, 
    ...
    by_beta<levenberg_mcmc<Parameters>> walker(beta.size());
...
auto thermo_levenberg_evidence(FunctionTable &&f,
    ...
    auto current =
        init_levenberg_thermo_mcmc(f, beta_run, mts, prior, lik, y, x);
