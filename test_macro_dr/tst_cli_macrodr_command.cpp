...
TEST_CASE("calculation of fractions of a simulation with N_states",
          "[save_fractioned_simulation/load_fractioned_simulation]") {
    /**
...
TEST_CASE("loading and saving experiments work together", "[save_experiment/load_experiment]") {
    ...
    experiment_type fexperiment = get_Experiment(exp_filename, 50e3, 0);
    std::string sep = ",";
    save_experiment(save_name, sep, fexperiment);
    ...
    save_experiment(i_name2, sep, exp_ideal);
