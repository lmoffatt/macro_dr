...
TEST_CASE("Another test with Catch2", "[fancy]") {
  ...
  std::transform(yv.begin(), yv.end(),
                 std::back_inserter(get<Recording>(sim())()),
  ...
  std::transform(
      ev.begin(), ev.end(), std::back_inserter(get<Recording_conditions>(x)()),
      [](std::tuple<double, double, double> x0) {
          return Experiment_step(Time(get<0>(x0)),
                                 ATP_evolution(std::vector<ATP_step>{
                                                                     ATP_step(number_of_samples(get<1>(x0)),
                                              ATP_concentration(get<2>(x0)))}));
      });
  ...
  deprecated::by_fraction<
      Simulated_Recording<macrodr::includes_N_state_evolution(false)>>
      y_out(n_frac);
  ...
  for (std::size_t i = n_frac - 1; i > 0; --i) {
      std::tie(get<Recording_conditions>(x_out[i - 1]), y_out[i - 1]) =
          ...
          get<Recording_conditions>(x_out[i]), y_out[i], indexes[i],
          indexes[i - 1], false);
  }
  
  CHECK(x_out == deprecated::by_fraction<Experiment>{});
  CHECK(y_out == std::vector<macrodr::Simulated_Recording<
            macrodr::includes_N_state_evolution(false)>>{});
}
