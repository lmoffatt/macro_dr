...
auto prod_variant_of_tuples_and_variant(std::variant<Tu...>const & x,const std::variant<T1...>& y, const Vs&...vs)
{
     auto next_x=std::visit([&y](auto& tu)
             {
              auto xy=prod_tuple_variant(tu,y);
              using variant_all=std::decay_t<decltype(sum_variant_type(prod_tuple_variant(Tu{},y)...))>;
...
{
    return std::visit([](auto& e)->std::variant<std::tuple<T1>...>
                      {return std::tuple(e);},y);
}
...
{
    using return_type=std::variant<std::decay_t<
        decltype(Apply_tuples(std::forward<F>(f),Tus{}))>...>;
    
    
    
    return std::visit([&f](auto& tu){
        return return_type(Apply_tuples(f,tu));}
                      ,x);
  }
...
{
    auto variant_of_tuples=std::apply([](auto&...e)
       {return prod_variant(e...);},x);
