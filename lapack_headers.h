...
inline auto sort_by_eigenvalue(std::tuple<Matrix<double>,DiagonalMatrix<double>,Matrix<double>> const&e) {
    auto &[VL, L, VR] = e;
...
Lapack_EigenSystem(const Matrix<double> &x, bool does_permutations,
    ...
    return_error<
        std::tuple< Matrix<double>,DiagonalMatrix<double>,Matrix<double> >, Lapack_EigenSystem > Error;
...
**/
    ...
    char BALANC=does_diagonal_scaling?(does_permutations?'B':'S'):(does_permutations?'P':'N');
/**
Parameters
    [in]	BALANC	
...
**/
    ...
    char SENSE=computes_eigenvalues_condition_numbers?(computes_eigenvectors_condition_numbers?'B':'E'):(computes_eigenvectors_condition_numbers?'V':'N');
...
**/
  ...
  */
    DiagonalMatrix<double> /* precision; dimension( * ) */ RCONDE(
        x.nrows(),x.nrows());
  ...
  [out]	RCONDV
            ...
            right eigenvector.
  */
    DiagonalMatrix<double> /* precision; dimension( * ) */ RCONDV(
        x.nrows(),x.nrows());
  ...
  */
  ...
  [out]	INFO
    ...
    int INFO;
    dgeevx_(&BALANC, &JOBVL, &JOBVR, &SENSE, &N, &A(0, 0), &LDA, &WR[0],
            &WI[0], &VL_lapack[0], &LDVL, &VR_lapack[0], &LDVR, &ILO, &IHI,
    ...
    if (INFO != 0) {
     ...
     //   assert((are_Equal<true, Matrix<double>>().test_prod(VR_cpp * WR * VL_cpp,
     //                                                         x, std::cerr)));
