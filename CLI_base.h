...
template <typename T> class Value : public typed_expression {
  T m_x;

public:
  Value(const T &x) : m_x{x} {}
  virtual ~Value() override{};
  virtual Identifier type() const { return T_s<T>::name(); }

  auto &operator()() const { return m_x; }
  auto &operator()() { return m_x; }
};

template <typename T> class typed_Identifier : public typed_expression {
  Identifier m_id;
...
public:
  ...
  Maybe_error<T>
  extract(std::map<Identifier, std::unique_ptr<base_value>> const &args) {
    auto it = args.find(name());
    if (it == args.end())
      return error_message(name() + " argument was not found in " +
                           Identifiers_to_string(args));
    else if (T_s<T>::name() != it->second->type())
      return error_message(name() + " argument with unexpercted type " +
                           it->second->name() + " instead of " +
                           T_s<T>::name());
    else
...
template <typename T> class Arg_v : public Arg<T> {
  T m_default;

public:
  Arg_v(Identifier t_name, T t_default)
      : Arg<T>{t_name}, m_default{std::move(t_default)} {}

  auto &name() const { return Arg<T>::name(); }
  auto &value() const { return m_default; }
  Maybe_error<T>
  extract(std::map<Identifier, std::unique_ptr<base_value>> const &args) {
    auto it = args.find(name());
    if (it == args.end())
...
class function<F, Cs<As...>, Cs<A_ds...>> : public Base_function {
  ...
  Maybe_error<R> evaluate(Maybe_error<As>... t_arg,
                          Maybe_error<A_ds>... t_arg_d) {
    if (((t_arg) && ... && true) && ((t_arg_d) && ... && true))
      return std::invoke(m_f, *(t_arg)..., *(t_arg_d)...);
    else
      return error_message((t_arg.error()() + ...) +
                           ((t_arg_d.error()() + ...)));
  }

public:
  function(Identifier fun_name, F t_f, Arg<As>... t_args,
           Arg_v<A_ds>... t_args_v)
      : m_name{fun_name}, m_f{t_f}, m_args{t_args...}, m_args_v{t_args_v...} {}

  auto evaluate(
      std::map<Identifier, std::unique_ptr<base_value>> const &args) const {
    return std::apply(
        [this, args](Arg<As> const &...t_arg) {
          return std::apply(
              [this, &args, &t_arg...](Arg_v<A_ds> &...t_arv_v) {
                return evaluate(t_arg.extract(args)...,
                                t_arv_v.extract(args)...);
              },
              m_args_v);
        },
        m_args);
