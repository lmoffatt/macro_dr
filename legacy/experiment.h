...
inline void save_fractioned_Recording(std::string const& fname, std::string const& separator,
        ...
        while (extract_double(ss >> i_frac >> septr(separator) >> i_step >> septr(separator), val,
            ...
            if (i_frac_prev != i_frac) {
                if ((i_frac > 0) && (i_frac != v.size() + 1))
                    return error_message("i_frac missmatch expected" + std::to_string(v.size()) +
...
inline Maybe_error<bool> load_Recording_Data(std::string const& fname, std::string const& separator,
                                             Recording& e) {
    std::ifstream f(fname);
    if (!f)
        return error_message("cannot open file " + fname);
...
inline void save_experiment(const std::string filename, std::string sep, Experiment const& e) {
    save_experiment(filename, sep, get<Recording_conditions>(e));
}

inline void save_fractioned_experiment(const std::string filename, std::string sep,
...
inline Maybe_error<bool> load_fractioned_experiment(const std::string filename,
    ...
    while ((ss >> i_frac >> septr(separator) >> i_step >> septr(separator) >> t >>
            septr(separator) >> i_sub_step >> septr(separator) >> v_number_of_samples >>
            septr(separator) >> val)) {
        if (i_step_prev != i_step) {
        ...
        if (i_frac_prev != i_frac) {
            if ((i_frac > 0) && (i_frac != e.size() + 1))
                return error_message("i_frac missmatch expected " + std::to_string(e.size() + 1) +
                                     " found:" + std::to_string(i_frac));
...
inline Maybe_error<bool> load_experiment(const std::string filename, std::string separator,
    ...
    while ((ss >> i_step >> septr(separator) >> t >> septr(separator) >> i_sub_step >>
            septr(separator) >> v_number_of_samples >> septr(separator) >> val)) {
        if (i_step_prev != i_step) {
            if ((i_step > 0) && (i_step != rec().size() + 1))
                return error_message("i_step missmatch expected " +
                                     std::to_string(rec().size() + 1) +
        ...
        if (i_frac_prev != i_frac) {
            if ((i_frac > 0) && (i_frac != 1))
                return error_message("i_frac missmatch expected " + std::to_string(1) +
                                     " found:" + std::to_string(i_frac));
 ...
 * */
    ...
    auto operator()(const Recording_conditions& x) {
        ...
        auto out = x;
        for (auto& e : pulses) out = idealize_Pulse(out, e);
        return out;
    }
