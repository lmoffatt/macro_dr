...
class Parameters_Normal_Distribution
    : public multivariate_normal_distribution<double, DiagPosDetMatrix<double>> {
   public:
   ...
   public:
    Parameters_Normal_Distribution(Parameters_Transformations&& t_tr, base_type&& dist)
        : base_type{std::move(dist)}, m_tr{std::move(t_tr)} {
    }

    Parameters_Normal_Distribution(Parameters_Transformations const& t_tr, base_type&& dist)
        : base_type{std::move(dist)}, m_tr{t_tr} {
    }

    Parameters_Normal_Distribution(Parameters_Transformations const& t_tr, base_type const& dist)
        : base_type{dist}, m_tr{t_tr} {
    }

    static Maybe_error<Parameters_Normal_Distribution> create(
        const Parameters_Transformations& t_par, base_type&& dist) {
        return Parameters_Normal_Distribution(t_par, std::move(dist));
    }

    static Maybe_error<Parameters_Normal_Distribution> create(
        const Parameters_Transformations& t_par, base_type const& dist) {
        return Parameters_Normal_Distribution(t_par, dist);
    }

    static Maybe_error<Parameters_Normal_Distribution> create(
        const std::string& ModelName, const std::vector<std::string>& ParamNames,
        transformations_vector&& trs, std::vector<double> values, std::vector<double> variances) {
        if (ParamNames.size() != trs.size())
            return error_message(
                "number of paremeters is different from number of transformations");
        ...
        auto Maybe_dist =
            make_multivariate_normal_distribution(tr_values, DiagPosDetMatrix<double>(tr_variance));
        if (!Maybe_dist)
            return Maybe_dist.error();

        return Parameters_Normal_Distribution(std::move(param), std::move(Maybe_dist.value()));
    }
...
inline void write_Prior(const std::string filename, const std::string separator,
                        Parameters_Normal_Distribution const& d) {
    ...
    f << "model_name" << separator << "i_par" << separator << "parameter_name" << separator
      << "parameter_transformation" << separator << "parameter_value" << separator
      << "transformed_mean" << separator << "transformed_variance"
      << "\n";

    std::size_t ii_par = 0;

    for (auto i_par = 0ul; i_par < n; ++i_par) {
        f << d.IdName() << separator << i_par << separator << d.names()[i_par] << separator

          << d.parameters().transf()[i_par]->to_string() << separator << m[i_par];
...
inline Maybe_error<Parameters_Normal_Distribution> load_Prior(
    ...
    if (!f)
        ...
        ss >> ::septr("model_name") >> ::septr(separator) >> ::septr("i_par") >>
            ...
            ::septr(separator) >> ::septr("transformed_variance");
        ...
        if (!ss)
            ...
            auto tr_values = param.tr(param.standard_values());
            auto Maybe_dist =
                make_multivariate_normal_distribution(tr_values, DiagPosDetMatrix<double>(vars));
            if (!Maybe_dist)
                return Maybe_dist.error();
...
inline auto prior_around(const Parameters_Transformations& tr, double error) {
    assert(error > 0);
