...
void report_all(FunctionTable& f, std::size_t iter, const Duration& dur,
                save_mcmc<Parameters, saving...>& s, thermo_fraction_mcmc<Parameters>& data,
                T const&... ts) {
...
void report_title(save_likelihood<Parameterstype>& s, thermo_fraction_mcmc<Parameters> const&,
                  ...) {
    s.f << "iter" << s.sep << "iter_time" << s.sep << "num_global_beta" << s.sep << "i_global_beta"
        << s.sep << "i_frac" << s.sep << "fraction_samples" << s.sep << "beta" << s.sep
        << "i_walker" << s.sep << "id_walker" << s.sep << "logP" << s.sep << "logL" << s.sep
        << "elogL" << s.sep << "vlogL" << s.sep << "logL1" << s.sep << "elogL1" << s.sep << "vlogL1"
        << s.sep << "plog_Evidence" << s.sep << "pelog_Evidence" << s.sep << "pvlog_Evidence"
...
void report_title(save_Parameter<Parameters>& s, thermo_fraction_mcmc<Parameters> const&, ...) {
    /*                s.f << iter<< s.sep
                ...
                s.f<< "\n";*/
    s.f << "iter" << s.sep << "iter_time" << s.sep << "num_global_beta" << s.sep << "i_global_beta"
        << s.sep << "i_frac" << s.sep << "fraction_samples" << s.sep << "i_walker" << s.sep
...
*/
    s.f << "iter" << s.sep << "iter_time" << s.sep << "num_global_beta" << s.sep << "i_global_beta"
        << s.sep << "i_frac" << s.sep << "fraction_samples" << s.sep << "beta" << s.sep << "S"
        << s.sep << "sample_size" << s.sep << "meanPrior" << s.sep << "logL" << s.sep << "elogL"
        << s.sep << "vlogL" << s.sep << "logL1" << s.sep << "elogL1" << s.sep << "vlogL1" << s.sep
        << "plog_Evidence" << s.sep << "eplog_Evidence" << s.sep << "pvarlog_Evidence" << s.sep
        << "log_Evidence" << s.sep << "elog_Evidence" << s.sep << "varlog_Evidence" << s.sep
...
void report_title(save_RateParameter<Parameters>& s, thermo_fraction_mcmc<Parameters> const&, ...) {
    s.f << "iter" << s.sep << "iter_time" << s.sep << "num_global_beta" << s.sep << "i_global_beta"
        << s.sep << "i_frac" << s.sep << "fraction_samples" << s.sep << "beta" << s.sep
        << "i_walker" << s.sep << "id_walker" << s.sep << "agonist" << s.sep << "i_state_from"
        << s.sep << "i_state_to" << s.sep << "rate_value"
        << "\n";
}
...
inline void report_title(save_Predictions<var::Parameters_transformed>& s,
    ...
    s.f << "iter" << s.sep << "iter_time" << s.sep << "num_global_beta" << s.sep << "i_global_beta"
        << s.sep << "i_frac" << s.sep << "fraction_samples" << s.sep << "beta" << s.sep
        << "i_walker" << s.sep << "id_walker" << s.sep << "i_step" << s.sep << "time" << s.sep
        << "num_samples" << s.sep << "Y_obs" << s.sep << "Y_pred" << s.sep << "Y_var" << s.sep
        << "plogL" << s.sep << "pelogL"
        << "\n";

    s.g << "iter" << s.sep << "iter_time" << s.sep << "num_global_beta" << s.sep << "i_global_beta"
        << s.sep << "i_frac" << s.sep << "fraction_samples" << s.sep << "beta" << s.sep
        << "i_walker" << s.sep << "id_walker" << s.sep << "i_step" << s.sep << "i_state" << s.sep
        << "j_state" << s.sep << "moment" << s.sep << "value"
        << "\n";
}

template <class FunctionTable, class Parameters, class... T>
void report(FunctionTable&&, std::size_t, no_save&, thermo_fraction_mcmc<Parameters> const&,
            T const&...) {
}
    ...
    if ((iter > 0) && (data.num_samples() > 0) && (iter % sampling_interval == 0)) {
        for (std::size_t i_walker = 0; i_walker < num_walkers(data); ++i_walker) {
            ...
            for (std::size_t i_beta = 0; i_beta < data.beta.size(); ++i_beta) {
                ...
                auto t_logL0 =
                    i_frac1 > 0 ? data.walkers[i_beta][i_walker].logL(i_frac0) : logLs_0();
                auto t__logL = t_logL;
                ...
                s.f << iter << s.sep << dur.count() << s.sep << data.beta.size() << s.sep << i_beta
                    << s.sep << i_frac1 << s.sep << data.samples_size[i_frac1] << s.sep << beta
                    << s.sep << i_walker << s.sep << data.i_walkers[i_beta][i_walker] << s.sep
                    << data.walkers[i_beta][i_walker].logP << t_logL.sep(s.sep)
                    << t_logL1.sep(s.sep) << plog_Evidence.sep(s.sep) << log_Evidence.sep(s.sep)
                    << "\n";

                // now consider the possibility of changing the i_frac
                if ((beta == 1) && i_beta + 1 < data.beta.size()) {
                    t_logL = data.walkers[i_beta][i_walker].logL(i_frac1 + 1) -
                             data.walkers[i_beta][i_walker].logL(i_frac1);
                    beta = 0;
                }
            }
        }
...
template <class FunctionTable, class Duration, class Parameters>
void report(FunctionTable& f, std::size_t iter, const Duration& dur, save_Evidence& s,
            thermo_fraction_mcmc<Parameters>& data, ...) {
    ...
    if ((iter > 0) && (data.num_samples() > 0) && (iter % sampling_interval == 0)) {
        ...
        for (std::size_t i_beta = 0; i_beta < data.beta.size(); ++i_beta) {
            ...
            s.f << iter << s.sep << dur.count() << s.sep << data.beta.size() << s.sep << i_beta
                << s.sep << i_frac << s.sep << data.samples_size[i_frac] << s.sep << beta << s.sep
                << data.global_beta[i_beta] << s.sep
                << calculate_sample_size(data.walkers_sta, i_beta, i_frac) << s.sep
                << meanPrior[i_beta] << r_logL.sep(s.sep) << r_logL1.sep(s.sep)
                << r_plogE.sep(s.sep) << logE.sep(s.sep) << s.sep << emcee_count << s.sep
                << emcee_rate << s.sep << thermo_count << s.sep << thermo_rate << s.sep
                << dBdL[std::max(1ul, i_beta) - 1] << "\n";
            if ((beta == 1) && i_beta + 1 < data.beta.size()) {
                r_logL =
                    meanLik_per_fraction[i_beta][i_frac + 1] - meanLik_per_fraction[i_beta][i_frac];
                beta = 0;
            }
        }
    }
                ...
                if ((beta[i_b] == 1) || (iter % (beta.size() * sampling_interval) == 0)) {
                    ...
                    if (is_valid(prediction)) {
                        ...
                        for (std::size_t i_step = 0; i_step < size(y); ++i_step) {
                            ...
                            s.f << iter << s.sep << dur.count() << s.sep << data.beta.size()
                                << s.sep << i_b << s.sep << i_frac << s.sep
                                << data.samples_size[i_frac] << s.sep << beta << s.sep << i_walker
                                << s.sep << walker_id << s.sep << i_step << s.sep << time << s.sep
                                << num_samples << s.sep << y[i_frac]()[i_step]() << s.sep
                                << get<macrodr::y_mean>(predictions()[i_step]) << s.sep
                                << get<macrodr::y_var>(predictions()[i_step]) << s.sep
                                << get<macrodr::plogL>(predictions()[i_step]) << s.sep
                            ...
                            if (((iter % state_sampling_interval == 0) && (beta[i_b] == 1)) ||
                                ...
                                for (std::size_t i_state = 0; i_state < n; ++i_state) {
                                    s.g
                                        ...
                                        << iter << s.sep << dur.count() << s.sep << data.beta.size()
                                        << s.sep << i_b << s.sep << i_frac << s.sep
                                        << data.samples_size[i_frac] << s.sep << beta << s.sep
                                    ...
                                    if ((iter % (state_sampling_interval * num_states) == 0) &&
                                        ...
                                        for (std::size_t j_state = 0; j_state <= i_state;
                                             ++j_state) {
                                            s.g << iter << s.sep << dur.count() << s.sep
                                                << data.beta.size() << s.sep << i_b << s.sep
                                                << i_frac << s.sep << data.samples_size[i_frac]
...
template <class FunctionTable, class Duration, class Parameters>
void report(FunctionTable& f, std::size_t iter, const Duration& dur, save_Parameter<Parameters>& s,
            thermo_fraction_mcmc<Parameters> const& data, ...) {
    std::size_t num_values = 4;
    auto num_beta = size(data.beta);
    std::size_t point_size =
    ...
    if ((iter > 0) && (data.num_samples() > 0) && (iter % sampling_interval == 0)) {
        for (std::size_t i_beta = 0; i_beta < num_betas(data); ++i_beta)
            for (std::size_t i_walker = 0; i_walker < num_walkers(data); ++i_walker) {
                for (std::size_t i_par = 0; i_par < num_Parameters(data); ++i_par)
                    s.f << iter << s.sep << dur.count() << s.sep << data.beta.size() << s.sep
                        << i_beta << s.sep << data.i_fraction[i_beta] << s.sep
                        << data.samples_size[data.i_fraction[i_beta]] << s.sep << data.beta[i_beta]
                        << s.sep << i_walker << s.sep << data.i_walkers[i_beta][i_walker] << s.sep
                        << i_par << s.sep << data.walkers[i_beta][i_walker].parameter[i_par]
                        << "\n";
            }
        s.f.flush();
    }
                    ...
                    for (std::size_t i_from = 0; i_from < v_Q0().nrows(); ++i_from)
                        for (std::size_t i_to = 0; i_to < v_Q0().ncols(); ++i_to) {
                            if (v_Qa()(i_from, i_to) > 0)
                                s.f << iter << s.sep << dur.count() << s.sep << data.beta.size()
                                    << s.sep << i_b << s.sep << data.i_fraction[i_b] << s.sep
                                    << data.samples_size[data.i_fraction[i_b]] << s.sep
                                    << data.beta[i_b] << s.sep << i_walker << s.sep << walker_id
