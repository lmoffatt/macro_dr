...
class N_channel_state_count : public var::Var<N_channel_state_count, Matrix<std::size_t>> {};

class N_channel_transition_mean : public var::Var<N_channel_transition_mean, Matrix<double>> {
    friend bool is_valid(N_channel_transition_mean const& x) {
        return (var::max(x()) > 0) && (var::min(x()) >= 0.0);
    }
};

static auto sample_Multinomial_state(mt_64i& mt, P_mean const& t_P_mean, std::size_t N) {
    ...
    for (std::size_t i = 0; i + 1 < k; ++i) {
        auto n =
            std::binomial_distribution<std::size_t>(N_remaining, t_P_mean()[i] / p_remaining)(mt);
        N_remaining -= n;
...
class Micror_state
    ...
    auto N_total_ij__mean_to_count(
        ...
        Matrix<std::size_t> out =
            applyMap([max_eps](double N) { return std::size_t(std::floor(N + max_eps)); }, v_Nr());
   ...
   public:
    ...
    using base_type = var::Var<Micror_state<averaging>, N_channel_transition_count>;
    using base_type::Var;
    auto size() const {
        return (*this)()().size();
    }
    ...
    friend auto stretch_move(mt_64i& mt, std::uniform_real_distribution<double>& rdist,
...
class Micror_parameters_distribution {
   ...
   public:
    ...
    Micror_parameters_distribution(N_Ch_mean_value N,
                                   multivariate_normal_distribution_of_probabilities t_Pmean,
                                   P const& t_P)
...
template <uses_averaging_aproximation averaging, uses_variance_aproximation variance>
class Micror_parameters_likelihood {
    ...
    template <class FuncTable, class Qdt>
    friend Maybe_error<double> logLikelihood(FuncTable& f, Micror_parameters_likelihood,
                                             Micror_state<averaging> const& p,
                                             const Patch_current& y, const Qdt& x) {
        auto out = logLikelihoodd(f, Micror_parameters_likelihood{}, p, y, x);
        if (std::isfinite(out))
            return out;
        else
    ...
    template <class Qdt>
    friend Patch_current simulate(mt_64i& mt, Micror_parameters_likelihood,
...
N_channel_state_count calc_Ntotal_j_sum(
    ...
    const std::vector<thermo_mcmc<Micror_state<averaging>>>& data_chain, std::size_t k,
    std::size_t i_beta_1) {
    return foldMap(
...
N_channel_state_XTX_count calc_Ntotal_j_XTX_iter(const thermo_mcmc<Micror_state<averaging>>& data,
    ...
    return foldMap(
        data.walkers,
        [i_beta_1, &u](std::vector<mcmc<Micror_state<averaging>>> const& walker) {
            return N_channel_state_XTX_count(XTX(u * walker[i_beta_1].parameter()()));
        },
        [](N_channel_state_XTX_count&& one, N_channel_state_XTX_count const& two) {
...
auto Micror_stochastic(FunctionTable& ftbl, C_Patch_State t_prior, C_Qdt const& t_Qdt,
                       C_Patch_Model const& m, C_double const& Nch, const Patch_current& p_y,
                       double fs, std::size_t myseed, std::size_t number_Of_samples,
                       std::vector<double> calculation_intervals_nodes, std::size_t save_every_iter,
                       std::size_t n_points_per_decade, double stops_at) {
    auto& p_P_cov = get<P_Cov>(t_prior);
    auto& p_P_mean = get<P_mean>(t_prior);
    auto current_noise = get<Current_Noise>(m).value() * fs / get<number_of_samples>(t_Qdt).value();

    std::size_t num_states = p_P_cov().nrows();
    auto num_par = num_states * (num_states);
    std::string path = "";
    std::size_t num_scouts_per_ensemble = std::pow(2, std::ceil(std::log2(num_par))) / 4;
    ...
    auto tmi = thermo_store_every_and_report<Micror_state<averaging>>(
        ...
        thermo_jumps_every, save_every_iter, max_iter_equilibrium, n_points_per_decade, stops_at,
    ...
    auto Maybe_Pmean_dist = make_multivariate_normal_distribution_of_probabilities(
        get<P_mean>(t_prior)(),
        SymPosDefMatrix<double>::I_sware_it_is_possitive(get<P_Cov>(t_prior)()));
    ...
    auto res = evidence(
        ftbl, std::move(tmi), r_prior, Micror_parameters_likelihood<averaging, variance>{}, p_y,
        Vector_Space(get<gmean_ij>(t_Qdt), get<gvar_ij>(t_Qdt), Current_Noise(current_noise)));
    std::vector<thermo_mcmc<Micror_state<averaging>>> chains = std::move(res.first.chains());

    return Map(calculation_intervals, [&chains, k](std::pair<double, double> inter) {
        auto n = chains.size();
