...
template <class X, class Y, class... Ks>
class Memoiza_closed_values {
    std::map<X, std::size_t> m_index;
    std::vector<Y> m_values;
    Y m_last_value;
    std::tuple<Ks...> m_other;

    static std::multimap<std::size_t, X> calc_reverse_count(std::map<X, std::size_t> x) {
    ...
    static auto calc_min_count(std::map<X, std::size_t> map, std::size_t max_size_buffer) {
   ...
   public:
    constexpr Memoiza_closed_values(std::vector<X> x_values, std::size_t size_buffer)
    ...
    void calc_all(F&& f, Ks&&... ks) {
        ...
        for (auto& e : m_index)
            m_values[e.second] = std::apply(
                [&f, &e](auto&... ks) { return std::invoke(std::forward<F>(f), e.first, ks...); },
                m_other);
    }
    ...
    auto& get_this(F&& f, const X& x, Ks&&... ks) {
        ...
        if (auto it = m_index.find(x); it != m_index.end())
            ...
            m_last_value = std::apply(
                [&f, &x](auto&... ks) { return std::invoke(std::forward<F>(f), x, ks...); },
                m_other);
            return m_last_value;
        }
    }
...
class Single_Thread_Memoizer<Id, F<Id, Fun...>, Memoizer> {
   ...
   public:
    ...
    template <class... Ts>
    friend auto apply_F(Single_Thread_Memoizer& me, Ts&&... ts) {
...
template <class, class, class>
class Thread_Memoizer;

template <class Id, class... Fun, class Y, class... Xs, template <class...> class F,
   ...
   public:
    ...
    auto& operator()(I_thread i_thread, Ts&&... ts) {
        if constexpr (m_f.is_threadable)
            return m_memoiza[i_thread.i % m_n_threads].get_or_calc(m_f, i_thread,
                                                                   std::forward<Ts>(ts)...);
        else
    ...
    template <class... Ts>
    friend auto apply_F(Thread_Memoizer& me, I_thread i_thread, Ts&&... ts) {
