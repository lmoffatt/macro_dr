 ...
 * @brief Parallel Tempering MCMC and evidence integration (log Z).
 *
 * - thermo_mcmc<Parameters>: runs MCMC at multiple betas.
 * - mean_logL/var_logL: statistics of log-likelihood per beta.
 * - calculate_Evidence: numerical integration via trapezoidal rule.
 */
...
template <class T>
using ensemble = std::vector<T>;
template <class T>
using by_beta = std::vector<T>;
template <class T>
using by_iteration = std::vector<T>;
...
class Saving_intervals
    : public var::Var<
          Saving_intervals,
          var::Vector_Space<Save_Evidence_every, Save_Likelihood_every, Save_Parameter_every,
                            Save_RateParameter_every, Save_Predictions_every>> {};
...
auto stretch_move(mt_64i&, const Parameters& Xk, const Parameters& Xj, double z) {
    ...
    auto out = Xj;
    for (std::size_t i = 0; i < Xj.size(); ++i) out[i] += z * (Xk[i] - Xj[i]);
    return out;
}

template <class Parameters>
auto stretch_move(mt_64i& mt, std::uniform_real_distribution<double>& rdist, const Parameters& Xk,
                  const Parameters& Xj, double a = 2) {
    assert((Xj.size() == Xk.size()) && "sum of vector fields of different sizes");
...
inline auto init_mts(mt_64i& mt, std::size_t n) {
    ...
    out.reserve(n);
    for (std::size_t i = 0; i < n; ++i) out.emplace_back(useed(mt));
    return out;
}

inline auto get_beta_list(double n_points_per_decade, double stops_at, bool includes_zero) {
    std::size_t num_beta = std::ceil(-std::log10(stops_at) * n_points_per_decade) + 1;
...
inline auto new_get_beta_list(std::size_t beta_size, std::size_t beta_upper_size,
                              std::size_t beta_medium_size, double beta_upper_value,
                              double beta_medium_value, double stops_at, bool includes_zero) {
    ...
    std::size_t beta_inferior_size = num_beta - beta_upper_size - beta_medium_size - 1;
    auto out = std::vector<double>(beta_size, 0.0);

    double n_points_per_decade_high = -(1.0 * beta_upper_size / std::log10(beta_upper_value));
    double n_points_per_decade_med =
        -(1.0 * beta_medium_size / std::log10(beta_medium_value / beta_upper_value));
    ...
    for (std::size_t i = 0; i < beta_upper_size; ++i)
        out[beta_size - 1 - i] = std::pow(10.0, -(1.0 * i) / n_points_per_decade_high);
    ...
    for (std::size_t i = 0; i < beta_inferior_size; ++i)
        out[beta_size - 1 - i - beta_upper_size - beta_medium_size] =
            beta_medium_value * std::pow(10.0, -(1.0 * i) / n_points_per_decade_low);
    out[beta_size - num_beta] = stops_at;
    assert(includes_zero ? (out[0] == 0) && (out[1] == stops_at) : out[0] == stops_at);
    assert(out.back() == 1.0);
    assert(out[beta_size - beta_upper_size - 1] == beta_upper_value);
    assert(out[beta_size - beta_upper_size - beta_medium_size - 1] == beta_medium_value);
...
#pragma omp parallel for
    for (std::size_t i = 0; i < sta.size(); ++i) out[i] = calculate_logL_mean(sta[i]);
    return out;
}

inline logL_statistics calculate_across_sta(ensemble<logL_statistics> const& sta) {
    logL_statistics across(get<mean<logL>>(sta[0]()())());
    for (std::size_t i = 1; i < sta.size(); ++i) across() &= get<mean<logL>>(sta[i]()())();
    return across;
}
inline auto calculate_across_sta(by_beta<ensemble<logL_statistics>> const& sta) {
    by_beta<logL_statistics> across(sta.size());
    for (std::size_t i = 0; i < sta.size(); ++i) across[i] = calculate_across_sta(sta[i]);
    return across;
}
...
inline auto calculate_within_sta(by_beta<ensemble<logL_statistics>> const& sta) {
    ...
    for (std::size_t i = 0; i < sta.size(); ++i) within[i] = calculate_within_sta(sta[i]);
    return within;
}
...
inline auto calculate_effective_sample_size(by_beta<logL_statistics> const& across,
...
template <class Parameters>
ensemble<logL_statistics> make_logL_statistics(ensemble<mcmc<Parameters>> walkers) {
...
template <class Parameters>
by_beta<ensemble<logL_statistics>> make_logL_statistics(
    by_beta<ensemble<mcmc<Parameters>>> walkers) {
...
auto var_logL(thermo_mcmc<Parameters> const& mcmc, by_beta<logLs> const& mean) {
    ...
    for (std::size_t iwalker = 0; iwalker < num_walkers(mcmc); ++iwalker)
        for (std::size_t ibeta = 0; ibeta < num_betas(mcmc); ++ibeta)
            out[ibeta] =
                out[ibeta] + pow(mcmc.walkers[ibeta][iwalker].logL - mean[ibeta], 2) / n_walkers;
    return out;
}
    ...
    for (std::size_t i = 0; i < num_samples(series); ++i)
        for (std::size_t ibeta = 0; ibeta < num_betas(series[0]); ++ibeta)
            for (std::size_t iwalker = 0; iwalker < num_walkers(series[0]); ++iwalker)
                out[ibeta] += series[i].walkers[ibeta][iwalker].logL / n_iters / n_walkers;
    return out;
}
...
auto mean_logP(by_iteration<thermo_mcmc<Parameters>> const& series) {
    ...
    for (std::size_t i = 0; i < num_samples(series); ++i)
        for (std::size_t iwalker = 0; iwalker < num_walkers(series[0]); ++iwalker)
            for (std::size_t ibeta = 0; ibeta < num_betas(series[0]); ++ibeta)
                out[ibeta] += series[i].walkers[ibeta][iwalker].logP / n_iters / n_walkers;
    return out;
}

template <class Parameters>
auto var_logL(by_iteration<thermo_mcmc<Parameters>> const& series, by_beta<logLs> const& mean) {
    ...
    for (std::size_t i = 0; i < num_samples(series); ++i)
        for (std::size_t iwalker = 0; iwalker < num_walkers(series[0]); ++iwalker)
            for (std::size_t ibeta = 0; ibeta < num_betas(series[0]); ++ibeta)
                out[ibeta] += pow(series[i].walkers[ibeta][iwalker].logL - mean[ibeta], 2) /
                              (n_iters * n_walkers);
    return out;
}

inline auto derivative_var_ratio_beta(by_beta<double> const& mean, by_beta<double> const& var,
    ...
    for (std::size_t i = 0; i < mean.size() - 1; ++i) {
        ...
        auto dL0 = var[i + 1];
        auto dL1 = var[i];
        out[i] = (var[i + 1] + var[i]) / (2 * (mean[i + 1] - mean[i])) * (beta[i + 1] - beta[i]);
    }
    return out;
}

template <class Parameters>
auto derivative_var_ratio(by_beta<double> const& mean, by_beta<double> const& var,
...
auto mean_logL_walker(by_iteration<thermo_mcmc<Parameters>> const& series) {
    auto out =
        ensemble<by_beta<double>>(nun_walkers(series[0]), by_beta<double>(num_betas(series[0]), 0));
    for (std::size_t i = 0; i < num_samples(series); ++i)
        for (std::size_t iwalker = 0; iwalker < num_walkers(series[0]); ++iwalker)

            for (std::size_t ibeta = 0; ibeta < num_beta(series[0]); ++ibeta)
                out[iwalker][ibeta] += series[i].walkers[ibeta][iwalker].logL / num_samples(series);
    return out;
}
...
inline double calcEvidence_(double b1, double b2, double L1, double L2) {
    ...
    return L1 * db + dL / (std::log(b2) - std::log(b1)) *
                         (b2 * std::log(b2) - b1 * std::log(b1) - b2 + b1 - std::log(b1) * db);
}

inline double calcEvidence(double b1, double b2, double L1, double L2, double dL1, double dL2) {
    auto dL = dL2 - dL1;
    auto db = b2 - b1;
...
inline double calcEvidence(double b0, double L0) {
    return L0 * b0;
}

inline double calculate_Evidence(by_beta<double> const& beta, by_beta<double> const& meanLik) {
    auto nb = beta.size();
...
inline double calculate_Evidence(by_beta<double> const& beta, by_beta<double> const& meanLik,
    ...
    if (beta[0] > beta[nb - 1]) {
        ...
        for (std::size_t i = 1; i < beta.size(); ++i)
            sum += calcEvidence(beta[i], beta[i - 1], meanLik[i], meanLik[i - 1], varLik[i],
                                varLik[i - 1]);
        return sum;
    } else {
        double sum = calcEvidence(beta[0], meanLik[0], varLik[0]);
        for (std::size_t i = 1; i < beta.size(); ++i)
            sum += calcEvidence(beta[i - 1], beta[i], meanLik[i - 1], meanLik[i], varLik[i - 1],
                                varLik[i]);
        return sum;
    }
}
...
auto var_logL_walker(by_iteration<thermo_mcmc<Parameters>> const& series,
    ...
    auto out =
        by_beta<ensemble<double>>(nun_walkers(series[0]), by_beta<double>(num_betas(series[0]), 0));
    for (std::size_t i = 0; i < num_samples(series); ++i)
        for (std::size_t iwalker = 0; iwalker < num_walkers(series[0]); ++iwalker)
            for (std::size_t ibeta = 0; ibeta < num_beta(series[0]); ++ibeta)
...
auto mean_var_logL_walker(ensemble<by_beta<double>> const& var_logL_walker) {
    ...
    auto out = by_beta<double>(n_beta, 0.0);
    for (std::size_t i = 0; i < n_beta; ++i)
        for (std::size_t j = 0; j < n_walkers; ++j) out[i] += var_logL_walker[j][i] / n_walkers;
    return out;
}

template <class Parameters>
auto mixing_var_ratio(by_beta<double> const& mean_var, by_beta<double> const& var_mean) {
    by_beta<double> out(mean_var.size());
    for (std::size_t i = 0; i < mean_var.size(); ++i) out[i] = var_mean[i] / mean_var[i];
    return out;
}
...
auto init_thermo_mcmc(FunctionTable& f, std::size_t n_walkers, by_beta<double> const& beta,
    ...
    by_beta<ensemble<mcmc<Parameters>>> walker(beta.size(), by_beta<mcmc<Parameters>>(n_walkers));
...
#pragma omp parallel for  // collapse(2)
    ...
    return thermo_mcmc<Parameters>{beta, walker, walker_sta, i_walker, emcee_stat, thermo_stat};
}
...
void insert_high_temperture_beta(FunctionTable& f, std::size_t tested_index,
                                 ...
                                 double new_beta, ensemble<mt_64i>& mt, Prior const& prior,
...
template <class Parameters>
void remove_high_temperture_beta(thermo_mcmc<Parameters>& current, by_beta<double>& beta,
...
auto initial_beta_dts(thermo_mcmc<Parameters>& initial_data) {
    ...
    if (initial_data.beta[0] == 1.0) {
        for (std::size_t i = 0; i < n - 1; ++i) beta[i] = std::pow(10.0, i * dlog_beta);
    } else {
        for (std::size_t i = 0; i < n - 1; ++i) beta[n - 1 - i] = std::pow(10.0, i * dlog_beta);
    }
...
template <class Prior, class Parameters = std::decay_t<decltype(sample(std::declval<mt_64i&>(),
                                                                       std::declval<Prior&>()))>>
auto create_thermo_mcmc(std::size_t n_walkers, by_beta<double> const& beta, mt_64i& mt,
                        Prior const& pr) {
    ...
    by_beta<ensemble<logL_statistics>> walker_sta(beta.size(), by_beta<logL_statistics>(n_walkers));
...
template <class Parameters>
std::pair<std::pair<std::size_t, std::size_t>, bool> check_iterations(
    std::pair<std::size_t, std::size_t> current_max, const thermo_mcmc<Parameters>&) {
...
struct no_save {
    ...
    friend void report(FunctionTable&&, std::size_t, no_save&, thermo_mcmc<Parameters> const&,
                       T const&...) {
    }
...
class less_than_max_iteration {
   ...
   public:
    less_than_max_iteration(std::size_t max_iter_warming, std::size_t max_iter_final)
        : current_iteration_{0ul},
          max_iter_warming_{max_iter_warming},
          max_iter_final_{max_iter_final},
          num_betas_{1ul},
          is_final_{false} {
    }
    ...
    std::size_t max_iteration_warming() const {
        return max_iter_warming_;
    }
    std::size_t max_iteration_final() const {
        return max_iter_final_;
    }

    std::size_t betas_number() const {
        return num_betas_;
    }
    ...
    friend auto checks_convergence(less_than_max_iteration&& c, const Anything& mcmc) {
...
bool compare_to_max_ratio(Beta const& beta, Var_ratio const& mean_logL, Var_ratio const& var_ratio,
                          double max_ratio);

inline bool compare_to_max_ratio(by_beta<double> const& beta, by_beta<double> const& mean_logL,
...
class checks_derivative_var_ratio {
   ...
   public:
    checks_derivative_var_ratio(std::size_t sample_size, double max_ratio = 2)
        : current_iteration_(0ul), max_ratio_{max_ratio}, curr_samples_{sample_size} {
    }
    ...
    friend auto checks_convergence(checks_derivative_var_ratio&& c,
        ...
        if ((c.current_iteration_ > 0) && (num_betas(c.current_samples()[0]) < num_betas(mcmc))) {
            c.current_iteration_ = 0;
        }
        c.add(mcmc);
        if (c.converges()) {
...
void observe_step_stretch_thermo_mcmc(Observer& obs, std::size_t j, double z, double r,
                                      const Parameters& current, const Parameters& candidate,
                                      double currlogP, Maybe_error<double> const calogP,
                                      double culogL, Maybe_error<double> const& calogL,
                                      bool doesChange) {
}

template <class Observer, class Parameters>
void observe_thermo_jump_mcmc(Observer& obs, std::size_t jlanding, const Parameters& current,
                              const Parameters& candidate, double culogL, double calogL,
                              double deltabeta, double logA, double pJump, double r,
                              bool doesChange) {
}
...
void step_stretch_thermo_mcmc(FunctionTable& f, std::size_t& iter, var::Event_Timing<N>& dur,
                              ...
                              const by_beta<double>& beta, ensemble<mt_64i>& mt, Prior const& prior,
                              Likelihood const& lik, const DataType& y, const Variables& x,
        ...
        for (std::size_t i_thread = 0; i_thread < num_threads; ++i_thread) {
            for (std::size_t iwb = i_thread * walkers_per_thread;
                ...
                if (!((ca_logP.valid()) && (ca_logL.valid()))) {
                    ...
                    auto dthLogL = ca_logP.value() - current.walkers[ib][iw].logP +
...
template <class FunctionTable, std::size_t N, class Observer, class Prior, class Likelihood,
    ...
    requires(is_of_this_template_type_v<std::decay_t<FunctionTable>, var::FuncMap_St> &&
             is_prior<Prior, Parameters, Variables, DataType> &&
             is_likelihood_model<FunctionTable, Likelihood, Parameters, Variables, DataType>)
void step_stretch_thermo_mcmc_old(FunctionTable& f, std::size_t& iter, var::Event_Timing<N>& dur,
                                  thermo_mcmc<Parameters>& current, Observer& obs,
                                  const by_beta<double>& beta, ensemble<mt_64i>& mt,
                                  Prior const& prior, Likelihood const& lik, const DataType& y,
                                  const Variables& x, double alpha_stretch = 2) {
    dur.record("stretch_start");
    assert(beta.size() == num_betas(current));
    auto n_walkers = num_walkers(current);
    auto n_beta = beta.size();
...
#pragma omp parallel for  // collapse(2)
    for (std::size_t i_thread = 0; i_thread < std::min(n_beta, num_threads); ++i_thread) {
        for (std::size_t ib = i_thread * n_beta_f; ib < std::min(i_thread * (n_beta_f + 1), n_beta);
             ++ib) {
...
inline double calc_logA(double betai, double betaj, logLs const& logLi, logLs const& logLj) {
...
void thermo_jump_mcmc(std::size_t iter, thermo_mcmc<Parameters>& current, Observer& obs,
                      const by_beta<double>& beta, mt_64i& mt, ensemble<mt_64i>& mts,
...
#pragma omp parallel for  // collapse(2)
        for (std::size_t i_thread = 0; i_thread < num_threads; ++i_thread) {
            for (std::size_t iwb = i_thread * walkers_per_thread;
                ...
                double logA = calc_logA(beta[ib], beta[ib + 1], current.walkers[ib][iw].logL,
...
template <class Parameters, class Observer>
void thermo_jump_mcmc_old(std::size_t iter, thermo_mcmc<Parameters>& current, Observer& obs,
                          const by_beta<double>& beta, mt_64i& mt, ensemble<mt_64i>& mts,
                          std::size_t thermo_jumps_every) {
    if ((iter > 0) && (current.num_samples() > 0) && (iter % (thermo_jumps_every) == 0)) {
        std::uniform_real_distribution<double> uniform_real(0, 1);
...
template <class Parameters>
auto calculate_delta_Evidence_variance_derivative(thermo_mcmc<Parameters>& current,
                                                  by_beta<double>& beta) {
    auto across = calculate_across_sta(current.walkers_sta);
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        deltaEvidence_variance_der[i] = -(beta[i] - beta[i + 1]) / 2.0 * (var0 + var1);
...
auto calculate_Acceptance_variance(const std::vector<double>& deltaEvidence_variance,
    ...
    for (std::size_t i = 0; i < deltaEvidence_variance.size(); ++i)
        acceptance_variance[i] =
            current.thermo_stat[i]().rate().valid()
...
template <class Parameters>
auto calculate_logL_variance(const std::string& variance_approximation,
...
template <class Parameters>
std::vector<double> calculate_fixed_Acceptance_derivative(const std::string& variance_approximation,
                                                          const thermo_mcmc<Parameters>& current,
                                                          const by_beta<double>& beta) {
...
template <class Parameters>
std::vector<double> calculate_diff_Acceptance_derivative(const std::string& variance_approximation,
                                                         const thermo_mcmc<Parameters>& current,
                                                         const by_beta<double>& beta) {
    auto vlogL = calculate_logL_variance(variance_approximation, current, beta);
    auto mlogL = calculate_logL_mean(current.walkers_sta);
...
template <class Parameters>
auto calculate_controler_step(const thermo_mcmc<Parameters>& current, const by_beta<double>& beta,
    ...
    if (equalizing_paramter == "Acceptance_vfm") {
        ...
        auto dvE = calculate_delta_Evidence_variance(current, beta);
        auto A_V = calculate_Acceptance_variance(dvE, current);
        auto dA = calculate_diff_Acceptance_over_variance_derivative(variance_approximation, dvE,
                                                                     current, beta);
        auto d = std::vector<double>(A_V.size() - 1);
        for (std::size_t i = 0; i < d.size(); ++i) {
        ...
        if (std::exp(std::min(0.0, -A[tested_index])) > acceptance_upper_limit)
            ...
            if (std::exp(std::min(0.0, -A[tested_index + 1])) < acceptance_lower_limit) {
                ...
                if (beta[tested_index] == 0)
                    new_beta =
                        beta[tested_index + 1] * beta[tested_index + 1] / beta[tested_index + 2];
                else
                    new_beta = std::sqrt(beta[tested_index + 1] * beta[tested_index]);

                insert_high_temperture_beta(f, tested_index, current, beta, new_beta, mt, prior,
                                            lik, y, x);
            }
    }
}
...
auto push_back_new_beta(FunctionTable& f, std::size_t& iter, thermo_mcmc<Parameters>& current,
                        ensemble<mt_64i>& mts, by_beta<double> const& new_beta, Prior const& prior,
                        Likelihood const& lik, const DataType& y, const Variables& x) {
    auto n_walkers = current.walkers[0].size();
            ...
            for (std::size_t i_beta = 0; i_beta < num_betas(data); ++i_beta)
                for (std::size_t i_walker = 0; i_walker < num_walkers(data); ++i_walker)
                    for (std::size_t i_par = 0; i_par < num_Parameters(data); ++i_par)
                        ...
                        s.f << iter << s.sep << dur.count() << s.sep << i_beta << s.sep
                            << data.beta.size() << s.sep << data.beta[i_beta] << s.sep << i_walker
                            << s.sep << data.i_walkers[i_beta][i_walker] << s.sep << i_par << s.sep
...
class save_RateParameter {
   public:
    ...
    friend void report_title(save_RateParameter& s, thermo_mcmc<Parameters> const&, ...) {
        s.f << "iter" << s.sep << "iter_time" << s.sep << "i_beta" << s.sep << "num_beta" << s.sep
            << "beta" << s.sep << "i_walker" << s.sep << "id_walker" << s.sep << "agonist" << s.sep
            << "i_state_from" << s.sep << "i_state_to" << s.sep << "rate_value"
            << "\n";
    }

    template <class Prior, class Likelihood, class Variables, class DataType>
    friend void report_model(save_RateParameter&, const Prior&, const Likelihood&, const DataType&,
                             const Variables&, by_beta<double> const&) {
    }
 ...
 * @param data
 * @return
 * @pre data should contain at least one beta with the right parameter and
 * walkers numbers
 */
    ...
    for (std::size_t i_beta = 0; i_beta < num_betas(data); ++i_beta)
        for (std::size_t i_walker = 0; i_walker < num_walkers(data); ++i_walker)
            for (std::size_t i_par = 0; i_par < num_Parameters(data); ++i_par) {
                ...
                std::size_t v_i_par;
                double v_param_value;
                if (load_vars_line(f, iter, dur, v_beta, v_i_walker, v_walker_id, v_i_par,
                                   v_param_value)) {
...
class save_mcmc : public observer {
   ...
   public:
    ...
    save_mcmc(std::string dir, std::string filename_prefix)
        : m_m{saving{dir + filename_prefix, 1ul, 1ul}...},
          directory_{dir},
          filename_prefix_{filename_prefix} {
    }

    template <class FunctionTable, class Duration, class... T>
    friend void report(FunctionTable& f, double iter_factor, const Duration& dur, save_mcmc& smcmc,
                       thermo_mcmc<Parameters> const& data, T const&... ts) {
        (report(f, iter_factor, dur, get<saving>(smcmc.m_m), data, ts...), ..., 1);
    }
    template <class Prior, class Likelihood, class Variables, class DataType>
    friend void report_model(save_mcmc& s, Prior const& prior, Likelihood const& lik,
...
class thermo_less_than_max_iteration {
   ...
   public:
    ...
    template <class Anything>
    friend auto checks_convergence(thermo_less_than_max_iteration&& c, const Anything&) {
        if (c.stop()) {
            return std::pair(std::move(c), true);
...
class cuevi_less_than_max_iteration {
   ...
   public:
    ...
    template <class Anything>
    friend auto checks_convergence(cuevi_less_than_max_iteration&& c, const Anything&) {
        if (c.stop()) {
            return std::pair(std::move(c), true);
