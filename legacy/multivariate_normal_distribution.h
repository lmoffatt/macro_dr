...
concept Covariance =
    std::convertible_to<C, DiagPosDetMatrix<T>> || std::convertible_to<C, SymPosDefMatrix<T>>;
...
class normal_distribution {
   ...
   public:
    ...
    auto cov() const {
        return n_.stddev() * n_.stddev();
    }
    auto stddev() const {
        return n_.stddev();
    }

    double operator()(mt_64i& mt) {
        return n_(mt);
    }
...
class multivariate_normal_distribution {
   ...
   public:
    multivariate_normal_distribution(Matrix<T>&& mean, Cova&& cov, cholesky_type&& chol,
                                     Cova&& cov_inv, double logdetCov)
        : n_{},
          mean_{std::move(mean)},
          cov_{std::move(cov)},
          cho_{std::move(chol)},
          cov_inv_{std::move(cov_inv)},
...
Maybe_error<multivariate_normal_distribution<T, Cova>> make_multivariate_normal_distribution(
    Mat&& mean, Cov&& cov) {
    ...
    if (!is_valid(mean))
        ...
        if (chol) {
            ...
            return Error(chol.error()() + " cholesky fails to build a normal distribution");
    }
}
...
Maybe_error<multivariate_normal_distribution<T, Cova>>
    ...
    if (!is_valid(mean))
        ...
        if (chol_inv) {
            ...
            return Error(chol_inv.error()() + " cholesky fails to be built from precision");
    }
}
...
class multivariate_normal_distribution_by_eig {
   private:
    ...
    multivariate_normal_distribution_by_eig(Matrix<T>&& t_mean, Cova&& t_cov, Matrix<T>&& Q,
                                            DiagonalMatrix<T>&& std, Cova&& cov_inv,
                                            double logdetCov)
        : n_{},
          mean_{std::move(t_mean)},
          cov_{std::move(t_cov)},
          m_Q{std::move(Q)},
          m_std{std::move(std)},
          cov_inv_{std::move(cov_inv)},
    ...
    if (!is_valid(mean))
        ...
        if (!Maybe_eig)
            ...
            return multivariate_normal_distribution_by_eig<T, Cova>(
                std::move(meanbeta), std::move(beta_cov), std::move(r_Q), std::move(v_std),
                std::move(v_Cov_inv), logDetCov);
        }
    }
}
...
class multivariate_normal_distribution_of_probabilities
   ...
   private:
    ...
    static Maybe_error<Matrix<double>> expand(const Matrix<double>& s, std::size_t excluded_row) {
        Matrix<double> out;
        if (s.nrows() == s.size()) {
    ...
    static auto reduce_cov(const SymPosDefMatrix<double> s, std::size_t excluded_row) {
    ...
    using base_type = multivariate_normal_distribution<double, SymPosDefMatrix<double>>;

   public:
    multivariate_normal_distribution_of_probabilities(base_type&& d, std::size_t excluded_row)
    ...
    friend Maybe_error<multivariate_normal_distribution_of_probabilities>
        make_multivariate_normal_distribution_of_probabilities(Matrix<double> mean,
                                                               SymPosDefMatrix<double> cov);

    friend Maybe_error<multivariate_normal_distribution_of_probabilities>
        make_multivariate_normal_distribution_of_probabilities(Matrix<double> const& mean,
                                                               SymPosDefMatrix<double> const& cov);
    std::size_t size() const {
...
inline Maybe_error<multivariate_normal_distribution_of_probabilities>
    ...
    auto Maybe_dist = make_multivariate_normal_distribution(
        T::reduce(std::move(mean), excluded_row), T::reduce_cov(std::move(cov), excluded_row));
    if (!Maybe_dist)
...
class inverse_gamma_distribution {
   ...
   public:
    inverse_gamma_distribution(double _alpha, double _beta)
        : g_{_alpha, _beta}, cte_int_{-var::lgamma(_alpha) + _alpha * std::log(_beta)} {
    }
...
class log_inverse_gamma_distribution {
   ...
   public:
    log_inverse_gamma_distribution(double _alpha, double _beta)
        : g_{_alpha, 1.0 / _beta}, cte_int_{-var::lgamma(_alpha) + _alpha * std::log(_beta)} {
    }

    double operator()(mt_64i& mt) {
        return -std::log(g_(mt));
    }
...
template <typename T, class Cova>
    requires Covariance<T, Cova>
class multivariate_gamma_normal_distribution : private log_inverse_gamma_distribution,
   ...
   public:
    ...
    Matrix<double> operator()(mt_64i& mt) {
        ...
        for (std::size_t i = 0; i < k; ++i) out[i + 1] = sample_b[i];
        return out;
    }
    ...
    Maybe_error<double> logP(const Matrix<T>& x) const {
        ...
        if (!logPvar)
            ...
            for (std::size_t i = 0; i < k; ++i) b[i] = x[i + 1];
            auto chi_2 = m_normal::chi2(b) / var;

            double out = logPvar.value() - 0.5 * (k * log(2 * std::numbers::pi) +
...
class dirchlet_distribution {
   ...
   public:
    ...
    dirchlet_distribution(Matrix<double> alpha)
        : m_gammas{applyMap([](double a) { return std::gamma_distribution<>{a, 1.0}; }, alpha)},
          m_alpha{std::move(alpha)},
          m_alpha0{var::sum(m_alpha)} {
    }

    Matrix<double> operator()(mt_64i& mt) {
        Matrix<double> s =
            applyMap([&mt](std::gamma_distribution<>& g) { return g(mt); }, m_gammas);
        return s / var::sum(s);
    }
};
...
class multinomial_distribution {
   ...
   public:
    ...
    multinomial_distribution(Matrix<double> P) : m_P{P} {
        auto sumP = var::sum(P);
        assert(std::abs(var::sum(P) - 1.0) < std::sqrt(std::numeric_limits<double>::epsilon()));
    }
    ...
    Matrix<std::size_t> operator()(mt_64i& mt, double N) {
        ...
        for (std::size_t i = 0; i + 1 < out.size(); ++i) {
            auto n = std::binomial_distribution<std::size_t>(N_remaining, m_P[i] / p_remaining)(mt);
            N_remaining -= n;
            p_remaining -= m_P[i];
            out[i] = n;
        }
...
class multinomial_transition_distribution {
   ...
   public:
    ...
    double logP(const Matrix<std::size_t>& Nij, std::size_t N, std::size_t i) const {
        double out = var::lgamma(N + 1.0);
        for (std::size_t j = 0; j < Nij.ncols(); ++j) {
            if (Nij(i, j) > 0)
                out = out + Nij(i, j) * std::log(m_P(i, j)) - var::lgamma(Nij(i, j) + 1.0);
        }
        return out;
    }

    Maybe_error<double> logP(const Matrix<std::size_t>& Nij, Matrix<std::size_t> Ni) const {
        if (var::sum(Nij) != var::sum(Ni))
            return error_message("count mismatch");
        else {
            double out;
            for (std::size_t i = 0; i < Ni.size(); ++i) out = out + logP(Nij, Ni[i], i);
            if (std::isfinite(out))
                return out;
            else
                return error_message("not finite");
