...
template <typename T>
    requires requires(T x) {
        { x() } -> std::convertible_to<std::size_t>;
    }
auto make_Range(T min, T max) {
    std::vector<T> out(max() - min());
    for (std::size_t i = 0; i < max() - min(); ++i) out[i]() = min() + i;
    return out;
}

template <template <class...> class Container, class T, class F>
...
template <class Container, class Init, class BinaryReductionOp>
Maybe_error<Init> fold(Container c, Init init, BinaryReductionOp reduce) {
    auto run = init;
    for (auto& e : c) {
...
template <class Container, class Map, class BinaryReductionOp>
    requires requires(BinaryReductionOp b, Map f, Container c) {
        { b(f(c[0]), f(c[0])) } -> std::convertible_to<decltype(f(c[0]))>;
    }
...
auto foldMap(std::size_t i_start, std::size_t i_end, Container&& c, Map&& f,
...
template <class Init, class BinaryReductionOp>
