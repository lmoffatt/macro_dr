...
**Important — convention for \(V\):** all derivations below use \(V = V_{\text{obs}} = \varepsilon^2 + N_{\text{ch}}\,\boldsymbol{\mu}\!\cdot\!\boldsymbol{\sigma}^2\) (the measurement-noise piece at \(r=\boldsymbol{\mu}\), the value that appears in the energy / Hessian / Newton derivation), **not** the predictive variance \(V_{\text{pred}} = V_{\text{obs}} + N_{\text{ch}}\,\widetilde{\gamma^{T}\Sigma\gamma}\). An earlier version of this note (and the supplement pseudo-code) conflated the two.

**Measurement update (rank-1 quasi-Laplace)**:

Covariance — rank-1 down-date in endpoint frame:
\[
\boxed{
...
Mean — **Newton step propagated through P(t) and SM-expanded** (corrected form):
\[
\boxed{
\boldsymbol{\mu}^{\text{post}} =
\boldsymbol{\mu}^{\text{prior}}
+ \frac{\delta}{2V}\,\Bigl[\,(\widetilde{\gamma^{T}\Sigma} + \widetilde{v^{T}\Sigma})\;-\;\frac{N_{\text{ch}}}{V+N_{\text{ch}}s}\,(b' + s)\,\widetilde{v^{T}\Sigma}\,\Bigr]
}
\]
with \(b' := \widetilde{\gamma^{T}\Sigma v} = \widetilde{\gamma^{T}\Sigma\gamma} + (\delta/V)\,\widetilde{\gamma^{T}\Sigma\sigma^2}\) and \(s := \widetilde{v^{T}\Sigma v}\).

**Why the SM expansion is necessary (correction to earlier form).** A previous version of this note wrote the mean update as
\[
\boldsymbol{\mu}^{\text{post}} \stackrel{?}{=}
\boldsymbol{\mu}^{\text{prior}} + \frac{\delta}{2V}\,\boldsymbol{\Sigma}^{\text{post}}(\widetilde{\gamma^{T}\Sigma} + \widetilde{v^{T}\Sigma}),
\]
obtained by formally substituting \((\boldsymbol{\gamma}+\mathbf{v}) \to (\widetilde{\gamma^{T}\Sigma}+\widetilde{v^{T}\Sigma})\) in the M-family Newton step \(\boldsymbol{\mu} + (\delta/(2V))\,\Sigma^{\text{post}}_{\text{start}}(\boldsymbol{\gamma}+\mathbf{v})\). That substitution is incorrect because the tilde lift already absorbs a \(\Sigma\) factor: \(\widetilde{\gamma^{T}\Sigma}\) is the boundary-lifted **cross-covariance** \(\boldsymbol{\gamma}^{T}\Sigma_p\mathbf{P}\), not just \(\boldsymbol{\gamma}^{T}\). Keeping \(\Sigma^{\text{post}}\) multiplied on the right then double-counts \(\Sigma\). At \(\mathbf{v}=\boldsymbol{\gamma}\) the naive form gives a step a factor \(V_{\text{pred}}/V_{\text{obs}}\) **too small** relative to the canonical MacroIR endpoint Kalman step \((\delta/V_{\text{pred}})\,\widetilde{\gamma^{T}\Sigma}\). The correct lift propagates the M-family Newton step through \(\mathbf{P}(t)\) and expands the SM down-date in \(\Sigma_p^{\text{post}}\) **before** applying the tilde substitution; that gives the boxed form above, which reduces structurally to standard MacroIR at \(\boldsymbol{\sigma}^2=0\).
