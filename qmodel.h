...
class return_predictions
    : public var::struct_Var<return_predictions, bool> {};
...
using Patch_State_and_Evolution= Vector_Space<logL, elogL, vlogL, P_mean, P_Cov, y_mean,
                                               y_var, plogL, eplogL, vplogL,Patch_State_Evolution >;
...
public:
  ...
  //      requires U<C_Patch_Model, Patch_Model>
    ...
    auto calc_Qdt(const C_Patch_Model &m, const C_Qx_eig &t_Qx,
        ...
        auto r_gmean_ij = build<gmean_ij>(elemDivSafe(gmean_ij_tot, r_P(),t_min_P()));
        ...
        if constexpr (true) {
            r_gvar_i() =
                truncate_negative_variance(std::move(r_gvar_i()));
        }
    ...
    static auto Qn_to_Qdt(const C_Qn& x)
    {
        ...
        return build<Qdt>(get<number_of_samples>(x), get<min_P>(x), get<P>(x), build<gmean_i>(r_gmean_i), build<gtotal_ij>(r_gtotal_ij),
                          build<gmean_ij>(r_gmean_ij),build<gtotal_sqr_ij>(r_gtotal_sqr_ij), build<gsqr_i>(r_gsqr_i), build<gvar_i>(r_gvar_i), build<gtotal_var_ij>(r_gtotal_var_ij),
    ...
    auto calc_Qdt(const C_Patch_Model &m, const ATP_step &t_step, double fs)
        ...
        if (!t_Qx)
            ...
            return calc_Qdt(m, t_Qx.value(), get<number_of_samples>(t_step), dt);
        }
    }
    ...
    auto calc_Qdt(const C_Patch_Model &m, const std::vector<ATP_step> &t_step, double fs)
        ...
        {
            auto v_Qdt0=calc_Qdt(m,t_step[0],fs);
            if (!v_Qdt0)
                return v_Qdt0.error();
            else
            {
                ...
                {
                    auto v_Qdti=calc_Qdt(m,t_step[i],fs);
                    if (!v_Qdti)
                        return v_Qdti.error();
                    else
  ...
  */
            ...
            if(std::isnan(primitive(r_plogL())))
                ...
                if constexpr(U<C_Patch_State,Patch_State>)
                    ...
                    ev().push_back(build<Patch_State>(build<logL>(get<logL>(t_prior)() + r_plogL()),
                                                      ...
                                                      r_P_mean,
                                                      r_P_cov,r_y_mean,
                                                      r_y_var, r_plogL, r_eplogL, r_vlogL));
                    return build<Patch_State_and_Evolution>(
                        ...
                        std::move(r_P_mean),
                        std::move(r_P_cov), std::move(r_y_mean),
                        std::move(r_y_var), r_plogL, r_eplogL, r_vlogL,std::move(ev));
                }
    ...
    auto log_Likelihood(const Model &model, const C_Parameters &par,
        ...
        -> Maybe_error<
            Transfer_Op_to<C_Parameters, std::conditional_t<predictions.value,Patch_State_Evolution,
                                                            Vector_Space<logL, elogL, vlogL>>>> {
        
        using Transf = transformation_type_t<C_Parameters>;
        using C_Patch_State = Op_t<Transf, std::conditional_t<predictions.value,Patch_State_and_Evolution,Patch_State>>;
        auto m = model(par);
                    ...
                    if (!Maybe_t_Qdt)
                        ...
                        if constexpr (false) {
                            auto test_der_t_Qdt = var::test_Derivative(
                                [this, &t_step, &fs, &gege](auto const &l_m,
                                                            auto const &l_Qx) {
                                    return calc_Qdt(
                                        l_m, t_step, fs);
                                },
    ...
    Maybe_error<Simulated_Sub_Step> sub_sub_sample(std::mt19937_64 &mt, Simulated_Sub_Step &&t_sim_step,
        ...
        auto t_Qx=calc_eigen(m,get<ATP_concentration>(t_s));
        if (!t_Qx) return t_Qx.error();
        
        auto dt = n_samples / fs;
        auto sub_dt = dt / n_sub;
        
        double sub_sample=1.0*n_samples/n_sub;
        auto t_P=calc_P(m,t_Qx.value(),sub_dt,get<min_P>(m)());
    ...
    Maybe_error<Simulated_Sub_Step> sub_sub_sample(std::mt19937_64 &mt, Simulated_Sub_Step &&t_sim_step,
        ...
        {
            auto Maybe_sub_step= sub_sub_sample(mt,std::move(t_sim_step),m,t_s[i],n_sub,fs);
            if (!Maybe_sub_step)
    ...
    sample(std::mt19937_64 &mt, const Model &model, const Parameters<Id> &par,
        ...
        if (!ini)
            ...
            auto run =
                fold(get<Recording_conditions>(e)(), ini.value(),
...
auto make_Likelihood_Model(const Model &m, Number_of_simulation_sub_steps n) {
    return Likelihood_Model<adaptive,recursive, averaging, variance, Model>(m, n);
}

template <uses_adaptive_aproximation adaptive,
         uses_recursive_aproximation recursive,
         uses_averaging_aproximation averaging,
         uses_variance_aproximation variance, class Model, class Parameters,
         class Variables, class DataType>
Maybe_error<double> logLikelihood(
    const Likelihood_Model<adaptive,recursive, averaging, variance, Model> &lik,
    Parameters const &p, const Variables &var, const DataType &y) {
    auto v_logL = Macro_DMR{}.log_Likelihood<adaptive,recursive, averaging, variance,return_predictions(false)>(
        lik.m, p, y, var);
    if (!v_logL)
        return v_logL.error();
    else
...
Maybe_error<Patch_State_Evolution> logLikelihoodPredictions(
    const Likelihood_Model<adaptive,recursive, averaging, variance, Model> &lik,
    Parameters const &p, const Variables &var, const DataType &y) {
    return  Macro_DMR{}.log_Likelihood<adaptive,recursive, averaging, variance,return_predictions(true)>(
        lik.m, p, y, var);
}
    ...
    if (x.empty())
        ...
        for (std::size_t i=1; i<x.size(); ++i)
        {
            ...
            if ((get<ATP_concentration>(out.back())()!=get<ATP_concentration>(x[i])())&&
                ((get<ATP_concentration>(out.back())()==0)||(get<ATP_concentration>(x[i])()==0)))
            {
                out.push_back(x[i]);
            }
            else
            {
                ...
                auto new_ATP=(get<ATP_concentration>(out.back())()*n1+get<ATP_concentration>(x[i])()*n2)/(n1+n2);
...
{
    return std::visit([&x](auto&& a){
        
        return std::visit([&a](auto&& ax) {
...
public:
    experiment_fractioner(const std::vector<std::size_t>& t_segments, std::size_t t_min_number_of_samples): segments{t_segments}, min_number_of_samples{t_min_number_of_samples}{}
    
    static auto average_Recording(const Recording_conditions &e,
                                  const Recording &y,const std::vector<std::size_t>& indexes0,
    ...
    auto operator()(const Recording &y,
                    const Experiment &x,
                    std::mt19937_64 &mt,
        ...
        by_fraction<Experiment> x_out(n_frac,Experiment(Recording_conditions{},get<Frequency_of_Sampling>(x), get<initial_ATP_concentration>(x)));
        y_out[n_frac-1]=y;
        x_out[n_frac-1]=x;
        
        for (std::size_t i = n_frac - 1; i >0 ; --i) {
            std::tie(get<Recording_conditions>(x_out[i-1]), y_out[i-1]) =
                average_Recording(get<Recording_conditions>(x_out[i]), y_out[i],indexes[i],indexes[i-1]);
        }
...
void report_title(save_Predictions<Parameters<Id>> &s, cuevi_mcmc<Parameters<Id>> const &, const Ts&...t) {
...
template<class Id>
void report_title(save_Predictions<Parameters<Id>> &s, thermo_mcmc<Parameters<Id>> const &,...) {
...
void report_title(save_Predictions<Matrix<double>> &s, thermo_mcmc<Matrix<double>> const &,...) {
}
...
{
    return std::to_string(get<number_of_samples>(ev)())+"  "+ std::to_string(get<ATP_concentration>(ev)());
}
...
template <class Prior, class Likelihood, class Variables,
         class DataType,
         class Parameters>
        ...
        for (std::size_t i_frac = 0; i_frac < size(data.beta); ++i_frac)
            for (std::size_t i_beta = 0; i_beta < size(data.beta[i_frac]); ++i_beta)
                for (std::size_t i_walker = 0; i_walker < size(data.walkers);
                     ++i_walker){
                    Maybe_error<Patch_State_Evolution> prediction=logLikelihoodPredictions(lik,data.walkers[i_walker][i_frac][i_beta].parameter,ys[i_frac],xs[i_frac]);
                    if (is_valid(prediction))
                    {
                        auto& predictions=prediction.value();
                        for (std::size_t i_x = 0;
                             i_x < size(ys[i_frac]);
                             ++i_x)
                        {
                            auto v_ev=get<ATP_evolution>(get<Recording_conditions>(xs[i_frac])()[i_x]);
                            ...
                            s.f << size(data.beta) << s.sep
                                ...
                                << i_x<< s.sep
                                <<get<Time>(get<Recording_conditions>(xs[i_frac])()[i_x])<<s.sep
                                <<get_num_samples(v_ev)<<s.sep
                                <<ToString(average_ATP_step(v_ev))<<s.sep
                                <<ToString(v_ev)<<s.sep
                                <<ys[i_frac]()[i_x]()<<s.sep
...
template<class Id>
auto cuevi_Model_by_convergence(std::string path, std::string filename,
                                const std::vector<std::size_t>& t_segments, std::size_t t_min_number_of_samples,
...
template<class Id>
auto cuevi_Model_by_iteration(std::string path, std::string filename,
                              const std::vector<std::size_t>& t_segments, std::size_t t_min_number_of_samples,
