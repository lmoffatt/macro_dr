...
Maybe_error<bool> load_simulation(std::string const &fname,
    ...
    while ((extract_double(ss >> i_step >> septr(separator), val) >>
        ...
        if (i_step_prev == i_step)
        {
            if (i_state!=N_state.size())
                    return error_message("i_state missmatch expected ="+
                                     std::to_string(N_state.size())+" found "+std::to_string(i_state));
            N_state.push_back(N);
  ...
  template <typename CQx>
    requires(var::U<CQx, Qx>)
  Maybe_error<Transfer_Op_to<CQx, P>> expm_taylor_scaling_squaring(
      const CQx &x, std::size_t order = 6) {
    {
  ...
  class Macro_DMR {
    ...
    template <class C_double>
      requires U<C_double, double>
    static C_double E111(C_double const &x, C_double const &y,
                         C_double const &z, C_double const &exp_x,
                         C_double const &exp_y, C_double const &exp_z) {
  ...
  public:
    ...
    auto calc_Qdt_old(const Patch_Model &m, const Qx_eig &t_Qx,
      ...
      for (std::size_t i = 0; i < N; ++i)
        for (std::size_t j = 0; j < i + 1; ++j)
          E2m.set(i, j,
                  Ee(ladt[i], ladt[j], exp_ladt[i], exp_ladt[j], t_min_P));
      ...
      auto v_gtotal_ij =
          gtotal_ij(t_Qx[Var<V>{}]() * WgV_E2 * t_Qx[Var<W>{}]());
      ...
      auto v_gmean_ij = gmean_ij(
          zip([](auto x, auto y) { return x / y; }, gmean_ij_tot, P_p));
      auto v_gtotal_var_ij = gtotal_var_ij(
          v_gtotal_sqr_ij() - zip([](auto x, auto y) { return x * y; },
                                  v_gtotal_ij(), v_gmean_ij()));
    ...
    calc_Qdt_eig(FunctionTable &&, const C_Patch_Model &m, const C_Qx_eig &t_Qx,
      ...
      if (!Maybe_r_P)
        ...
        auto r_gtotal_sqr_ij =
            build<gtotal_sqr_ij>(t_V() * WgV_E3 * t_W() * 2.0);
    ...
    auto calc_Qdt_bisection(FunctionTable &&f, const C_Patch_Model &m,
                            ...
                            std::size_t order)
        -> Maybe_error<Transfer_Op_to<C_Patch_Model, Qdt>> {
      auto maybe_Qn = calc_Qn_bisection(std::forward<FunctionTable>(f), m,
                                        t_step, fs, order);
      if (!maybe_Qn)
    ...
    auto calc_Qdt(FunctionTable &&f, const C_Patch_Model &m,
      ...
      if (t_step.empty())
        ...
        auto v_Qdt0 =
            calc_Qdt(std::forward<FunctionTable>(f), m, t_step[0], fs);
        if (!v_Qdt0)
          return v_Qdt0.error();
        else {
    ...
    */
    ...
    Macror_old(FunctionTable &, C_Patch_State &&t_prior, C_Qdt const &t_Qdt,
      ...
      Op_t<Transf, eplogL> r_eplogL(-0.5 *
                                        log(2 * std::numbers::pi * r_y_var()) -
                                    0.5); // e_mu+N*gSg"-N*zeta*sqr(sSg)"
      // double chilogL=(eplogL-plogL)/std::sqrt(0.5);
      ...
      if constexpr (false) {
        if (!r_test) {
          ...
          return error_message(
              "\nfails in trace!!!; error=" + r_test.error()() + ss.str());
        }
    ...
    Maybe_error<C_Patch_State>
    Macror(FunctionTable &, C_Patch_State &&t_prior, C_Qdt const &t_Qdt,
           C_Patch_Model const &m, C_double const &Nch,
           const Patch_current &p_y, double fs) const {
      ...
      Op_t<Transf, eplogL> r_eplogL(
          -0.5 * log(2 * std::numbers::pi * r_y_var()) - 0.5);
      vplogL r_vlogL(0.5);
    ...
    template <uses_adaptive_aproximation adaptive,
              ...
              uses_variance_aproximation variance,
              uses_variance_correction_aproximation variance_correction,
              return_predictions predictions, class FuncTable,
        ...
        if (!ini)
          ...
          auto run = fold(
              ...
              [this, &f, &m, fs, &e, &y, &gege](C_Patch_State &&t_prior,
                ...
                std::size_t i_segment = std::min(
                    Nchs.size() - 1.0, std::floor(time / time_segment));
                ...
                if (!Maybe_t_Qdt)
                  ...
                  if (false) {
                    auto test_Qdt = test(
                        t_Qdt, get<Conductance_variance_error_tolerance>(m));
              ...
              [this, &f_local, &m, fs, &e, &y, &gege](C_Patch_State &&t_prior,
                ...
                if (!Maybe_t_Qdt)
                  ...
                  if constexpr (false) {
                    auto test_der_t_Qdt = var::test_Derivative(
                        [this, &t_step, &fs, &gege,
                         &f_local](auto const &l_m, auto const &l_Qx) {
    ...
    sub_sample(mt_64i &mt, Simulated_Step<keep_N_state> &&t_sim_step,
      ...
      auto Maybe_t_sub_step = std::visit(
          [this, &mt, &m, n_sub_dt, &t_sub_step, fs](auto const &a) {
            return sub_sub_sample(mt, std::move(t_sub_step), m, a, n_sub_dt,
                                  fs);
          },
          t_s());
    ...
    Simulated_Step<keep_N_state> init_sim(mt_64i &mt, const Patch_Model &m,
      ...
      return Simulated_Step<keep_N_state>(Vector_Space(
          std::move(N_state), Simulated_Recording<keep_N_state>{}));
    }
    ...
    sample_N(mt_64i &mt, const Model &model, const Parameters<Id> &par,
             const Experiment &e, const Simulation_Parameters &sim,
             const Recording &r = Recording{}) {
      return sample_<includes_N_state_evolution(true)>(mt, model, par, e, sim,
                                                       r);
    }
  };
  ...
  struct Likelihood_Model_v {
    ...
    template <uses_adaptive_aproximation adaptive,
              ...
              uses_variance_correction_aproximation variance_correction,
              class Model>
    auto template_op(::V<adaptive>, ::V<recursive>, ::V<averaging>,
  ...
  template <uses_adaptive_aproximation adaptive,
            ...
            uses_variance_aproximation variance,
            uses_variance_correction_aproximation variance_correction,
            class FuncTable, class Model, class Parameters, class Variables,
            class DataType>
  ...
  static Experiment_step average_Experimental_step(Experiment_step const &x) {
    return Experiment_step(get<Time>(x),
                           average_ATP_step(get<ATP_evolution>(x)));
  }
  ...
  static ATP_evolution add_ATP_step_i(ATP_step && x,
                                      std::vector<ATP_step> && y) {
  ...
  template <includes_N_state_evolution keep_N_state>
  void save_Likelihood_Predictions(
 ...
 inline void save_Likelihood_Predictions(
  ...
  void report(
      FunctionTable && f, std::size_t iter, save_Predictions<Parameters> & s,
          ...
          for (std::size_t i_walker = 0; i_walker < size(data.walkers);
            ...
            Maybe_error<Patch_State_Evolution> prediction =
                logLikelihoodPredictions(
                    ...
                    data.walkers[i_walker][i_frac][i_beta].parameter,
                    ys[i_frac], xs[i_frac]);
            if (is_valid(prediction)) {
              ...
              for (std::size_t i_x = 0; i_x < size(ys[i_frac]); ++i_x) {
                ...
                s.f << size(data.beta) << s.sep << size(data.beta[i_frac])
                    ...
                    << ToString(v_ev) << s.sep << ys[i_frac]()[i_x]() << s.sep
    ...
    if (iter % s.save_every == 0) {
      ...
      for (std::size_t i_cu = 0; i_cu < data.get_Cuevi_Temperatures_Number();
        ...
        for (std::size_t half = 0; half < 2; ++half)
          ...
          for (std::size_t iiw = 0; iiw < data.get_Walkers_number() / 2;
               ++iiw) {
            ...
            auto prediction =
                logLikelihoodPredictions(f.fork(var::I_thread(iiw)), lik, par,
                                         ys[i_frac()], xs[i_frac()]);
            if (is_valid(prediction)) {
              auto &predictions = prediction.value();
              for (std::size_t i_x = 0; i_x < size(ys[i_frac()]); ++i_x) {
  ...
  void report_title(save_Predictions<ParameterType> & s,
    ...
    s.f << "iter" << s.sep << "i_cu" << s.sep << "i_frac" << s.sep << "nsamples"
        ...
        << s.sep << "i_x" << s.sep << "Time" << s.sep << "num_samples" << s.sep
  ...
  auto cuevi_Model_by_convergence(
    ...
    return cuevi_integration(
        ...
        save_mcmc<Parameters<Id>, save_likelihood<Parameters<Id>>,
                  ...
                  save_Predictions<Parameters<Id>>>(path, filename, 100ul,
                                                    100ul, 100ul, 100ul),
  ...
  new_cuevi_Model_by_iteration(
    ...
    return cuevi::Cuevi_Algorithm(
        ...
        cuevi::Fractions_Param(Vector_Space(
            cuevi::Min_value(min_fraction),
  ...
  Maybe_error<bool> load_Simulated_Recording(
      std::string const &filename, const std::string &separator,
  ...
  template <class Id>
  void report_model(save_Parameter<Parameters<Id>> & s,
  ...
  template <class Id>
  Maybe_error<Parameters<Id>> load_Parameters(save_Parameter<Parameters<Id>> &
                                              s) {
