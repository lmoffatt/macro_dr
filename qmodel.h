...
class uses_variance_aproximation : public var::struct_Var<uses_variance_aproximation, bool> {};

class uses_variance_correction_aproximation
    : public var::struct_Var<uses_variance_correction_aproximation, bool> {};

class uses_adaptive_aproximation : public var::struct_Var<uses_adaptive_aproximation, bool> {};
class uses_recursive_aproximation : public var::struct_Var<uses_recursive_aproximation, bool> {};
class uses_averaging_aproximation : public var::struct_Var<uses_averaging_aproximation, int> {};

class return_predictions : public var::struct_Var<return_predictions, int> {};
...
template <uses_recursive_aproximation recursive, uses_averaging_aproximation averaging,
...
using Patch_State = Vector_Space<logL, elogL, vlogL, P_mean, P_Cov, y_mean, y_var, plogL, eplogL,
                                 vplogL, macror_algorithm>;
...
using Patch_State_and_y_Evolution =
    Vector_Space<logL, elogL, vlogL, P_mean, P_Cov, y_mean, y_var, plogL, eplogL, vplogL,
                 macror_algorithm, ymean_Evolution, yvar_Evolution>;
...
class includes_N_state_evolution : public var::struct_Var<includes_N_state_evolution, bool> {};
...
class Macro_DMR {
   ...
   public:
    ...
    template <class C_Qx>
    auto calc_eigen(const C_Qx& v_Qx) -> Maybe_error<Transfer_Op_to<C_Qx, Qx_eig>> {
    ...
    Maybe_error<Qx_eig const*> get_eigen(Vs& buffer_calc, const Patch_Model& m,
                                         ATP_concentration x) {
        auto Maybe_eigen =
            buffer_calc[var::Vector_Map<Qx_eig>{}][var::Vector_Space<ATP_concentration>(x)];
        if (Maybe_eigen)
            return Maybe_eigen;
        else {
            auto Maybe_new_eigen = calc_eigen(m, x);
            if (Maybe_new_eigen) {
                buffer_calc[var::Vector_Map<Qx_eig>{}].emplace(x,
                                                               std::move(Maybe_new_eigen.value()));
    ...
    auto calc_Peq(C_Qx const& t_Qx, N_St nstates) -> Transfer_Op_to<C_Qx, P_mean> {
        auto p0 = Matrix<double>(1ul, nstates(), 1.0 / nstates());
        auto v_eig_Qx = calc_eigen(t_Qx);
        if (v_eig_Qx) {
            auto& landa = get<lambda>(v_eig_Qx.value())();
    ...
    auto calc_P(const Patch_Model& m, const Qx_eig& t_Qx, double dt, double t_min_P) {
        ...
        //    t_min_P);
        return to_Transition_Probability(get<V>(t_Qx)() * exp_ladt * get<W>(t_Qx)());
    }

    template <class Patch_Model>
    Maybe_error<Transfer_Op_to<Patch_Model, P>> calc_P(const Patch_Model& m, const Qx& t_Qx,
    ...
    template <class Patch_Model>
    auto calc_Qdt_old(const Patch_Model& m, const Qx_eig& t_Qx, number_of_samples ns, double dt) {
        ...
        auto v_gtotal_sqr_ij = gtotal_sqr_ij(t_Qx[Var<V>{}]() * WgV_E3 * t_Qx[Var<W>{}]() * 2.0);
        for (std::size_t i = 0; i < N; ++i)
 ...
 M_Matrix<double> WgV_Wg_E2(k_u,k_u);
    ...
    Maybe_error<Transfer_Op_to<C_Patch_Model, Qdt>> calc_Qdt_eig(FunctionTable&&,
        ...
        if (!Maybe_r_P)
            ...
            auto r_gvar_ij = build<gvar_ij>(elemDivSafe(r_gtotal_var_ij(), r_P(), t_min_P()));
    ...
    auto calc_Qdtg_ATP_step(FunctionTable& f, const C_Patch_Model& m, const ATP_step& t_step,
        ...
        return calc_Qdtg_taylor(m, t_Qx, get<number_of_samples>(t_step), dt);
    }
    ...
    auto calc_Qdtm_ATP_step(FunctionTable& f, const C_Patch_Model& m, const ATP_step& t_step,
        ...
        if (t_Qeig) {
            auto Maybe_Qdt =
                calc_Qdtm_eig(f, m, t_Qeig.value(), get<number_of_samples>(t_step), dt);
            if (Maybe_Qdt)
    ...
    auto calc_Qdt_ATP_step(FunctionTable& f, const C_Patch_Model& m, const ATP_step& t_step,
        ...
        auto t_Qeig = f.fstop(Calc_eigen{}, m, get<ATP_concentration>(t_step));
        if (t_Qeig) {
            auto Maybe_Qdt = calc_Qdt_eig(f, m, t_Qeig.value(), get<number_of_samples>(t_step), dt);
            if (Maybe_Qdt)
                return Maybe_Qdt;
        }
        auto t_Qx = build<Qx>(calc_Qx(m, get<ATP_concentration>(t_step)));
    ...
    C_Matrix truncate_negative_variance(C_Matrix&& var) {
        for (std::size_t i = 0; i < var.nrows(); ++i)
            for (std::size_t j = 0; j < var.ncols(); ++j) var.set(i, j, max(0.0, var(i, j)));
        return var;
    }
    ...
    template <uses_recursive_aproximation recursive, uses_averaging_aproximation averaging,
              uses_variance_aproximation variance,
              uses_variance_correction_aproximation variance_correction, class C_Patch_State,
        ...
        if constexpr (averaging.value == 0) {
            ...
            if constexpr (variance_correction.value) {
                ...
                if (!std::isfinite(primitive(N * ms0 + N * gSg - N * zeta * sqr(sSg))) ||
                    ...
                    return safely_calculate_yvar<recursive, averaging, variance,
                                                 uses_variance_correction_aproximation(false)>(
                        t_Qdt, m, fs, N, p_P_mean, SmD, y, y_baseline);
                ...
                if (!all_Probability_elements(primitive(r_P_mean())) ||
                    ...
                    return safely_calculate_Pmean_Pcov<recursive, averaging, variance,
                                                       uses_variance_correction_aproximation(
                                                           false)>(is_y_nan, std::move(r_y_mean),
                ...
                if constexpr (variance.value) {
                    ...
                    if (std::isfinite(primitive(ms)) && primitive(ms) >= 0) {
                        ...
                        return safely_calculate_Pmean_Pcov<recursive, averaging,
    ...
    auto calculate_logL(bool y_is_nan, C_y_var const& r_y_var, auto& chi2, auto& p_logL,
        ...
        if (y_is_nan)
            ...
            if (get<Proportional_Noise>(m).value() == 0) {
                auto r_plogL =
                    build<plogL>(-0.5 * log(2 * std::numbers::pi * r_y_var()) - 0.5 * chi2);
                if constexpr (var::is_derivative_v<std::decay_t<decltype(r_plogL)>>)
                    if (std::isnan(var::derivative(r_plogL())()[0]))
                        std::cerr << "nan der\n";
                auto r_pelogL = build<eplogL>(-0.5 * log(2 * std::numbers::pi * r_y_var()) - 0.5);
                auto r_logL = build<logL>(p_logL() + r_plogL());
                auto r_elogL = build<elogL>(p_elogL() + r_pelogL());

                return std::tuple(std::move(r_plogL), std::move(r_pelogL), vplogL(0.5),
                ...
                return std::tuple(std::move(r_plogL), std::move(r_pelogL), vplogL(0.5),
                                  std::move(r_logL), std::move(r_elogL), vlogL(p_vlogL() + 0.5));
            }
        }
    }
    ...
    Maybe_error<std::tuple<Transfer_Op_to<C_Qdt, y_mean>, Transfer_Op_to<C_Qdt, y_var>,
                           Transfer_Op_to<C_Qdt, P_mean>, Transfer_Op_to<C_Qdt, P_Cov>,
                           Transfer_Op_to<C_Qdt, double>, macror_algorithm>>
        safely_calculate_Pmean_Pcov(bool y_is_nan, Transfer_Op_to<C_Qdt, y_mean>&& r_y_mean,
                                    Transfer_Op_to<C_Qdt, y_var>&& r_y_var,
                                    C_Patch_State const& t_prior, C_Qdt const& t_Qdt,
                                    C_Patch_Model const& m, C_double const& N,
                                    const Patch_current& p_y, double fs, auto& SmD) const {
        auto& y = p_y.value();
        auto dy = y - r_y_mean();
        auto chi = dy / r_y_var();
        auto chi2 = dy * chi;
        ...
        auto r_macro_algo = macror_algorithm(ToString(
            MacroR2<::V<recursive>, ::V<averaging>, ::V<variance>, ::V<variance_correction>>{}));
            ...
            if constexpr (!variance_correction.value) {
                if constexpr (averaging.value == 2) {
                    ...
                    if (!all_Probability_elements(primitive(r_P_mean())) ||
                        ...
                        return std::tuple(std::move(r_y_mean), std::move(r_y_var),
                                          std::move(r_P_mean), std::move(r_P_cov), std::move(chi2),
                                          std::move(r_macro_algo));

                } else if constexpr (averaging.value == 1) {
                    auto& t_gmean_i = get<gmean_i>(t_Qdt);
                    auto gS = TranspMult(t_gmean_i(), SmD) * t_P();
    ...
    template <uses_recursive_aproximation recursive, uses_averaging_aproximation averaging,
              uses_variance_aproximation variance,
              uses_variance_correction_aproximation variance_correction, class FunctionTable,
    ...
    Maybe_error<C_Patch_State> Macror(FunctionTable&, C_Patch_State&& t_prior, C_Qdt const& t_Qdt,
                                      C_Patch_Model const& m, C_double const& Nch,
                                      const Patch_current& p_y, double fs) const {
        using Transf = transformation_type_t<C_Qdt>;

        auto Maybe_all = safely_calculate_y_mean_yvar_Pmean_PCov<recursive, averaging, variance,
                                                                 variance_correction>(
            t_prior, t_Qdt, m, Nch, p_y, fs);

        if (!Maybe_all)
            return Maybe_all.error();

        auto [r_y_mean, r_y_var, r_P_mean, r_P_cov, chi2, r_algo] = std::move(Maybe_all.value());
        auto y_baseline = get<Current_Baseline>(m);
        ...
        auto [r_plogL, r_eplogL, r_vplogL, r_logL, r_elogL, r_vlogL] =
            calculate_logL(std::isnan(y), r_y_var, chi2, get<logL>(t_prior), get<elogL>(t_prior),
                           get<vlogL>(t_prior), m);

        if (!std::isnan(y) && std::isnan(primitive(r_plogL()))) {
            std::stringstream ss;
            ...
            return build<Patch_State_and_Hessian>(r_logL, r_elogL, r_vlogL, r_P_mean, r_P_cov,
                                                  r_y_mean, r_y_var, r_plogL, r_eplogL, r_vplogL,
                                                  r_algo, FIM(get<FIM>(t_prior)() + r_FIM));
        } else {
            return build<Patch_State>(r_logL, r_elogL, r_vlogL, r_P_mean, r_P_cov, r_y_mean,
                                      r_y_var, r_plogL, r_eplogL, r_vplogL, r_algo);
        }
    }
    ...
    Maybe_error<C_Patch_State> Macror_old(FunctionTable&, C_Patch_State&& t_prior,
        ...
        if (std::isnan(y)) {
            get<macror_algorithm>(t_prior)() =
                ToString(MacroR2<::V<uses_recursive_aproximation(false)>, ::V<averaging>,
        ...
        if (std::isnan(primitive(r_plogL()))) {
            ...
            ev().push_back(build<Patch_State>(build<logL>(get<logL>(t_prior)() + r_plogL()),
                                              ...
                                              r_P_mean, r_P_cov, r_y_mean, r_y_var, r_plogL,
            ...
            return build<Patch_State_and_Evolution>(
                ...
                std::move(r_P_cov), std::move(r_y_mean), std::move(r_y_var), r_plogL, r_eplogL,
                r_vlogL, get<macror_algorithm>(t_prior), std::move(ev));
        } else if constexpr (U<C_Patch_State, Patch_State_and_y_Evolution>) {
            auto& yev = get<ymean_Evolution>(t_prior);
            yev().push_back(r_y_mean);
            auto& yvev = get<yvar_Evolution>(t_prior);
            ...
            return build<Patch_State_and_y_Evolution>(
                ...
                build<vlogL>(get<vlogL>(t_prior)() + r_vlogL()), std::move(r_P_mean),
                std::move(r_P_cov), std::move(r_y_mean), std::move(r_y_var), r_plogL, r_eplogL,
                r_vlogL, get<macror_algorithm>(t_prior), std::move(yev), std::move(yvev));
        } else if constexpr (U<C_Patch_State, Patch_State_and_Hessian>) {
            auto r_J = derivative(r_y_mean)();
            auto r_JS = derivative(r_y_var)();
            ...
            return build<Patch_State_and_Hessian>(
                ...
                build<vlogL>(get<vlogL>(t_prior)() + r_vlogL()), std::move(r_P_mean),
                std::move(r_P_cov), std::move(r_y_mean), std::move(r_y_var), r_plogL, r_eplogL,
                r_vlogL, get<macror_algorithm>(t_prior), FIM(get<FIM>(t_prior)() + r_FIM));
        } else {
    ...
    auto init(const C_Patch_Model& m, initial_ATP_concentration initial_x)
        ...
        if constexpr (var::is_derivative_v<C_Patch_Model>) {
            if constexpr (predictions.value == 0) {
                auto& r_Q0 = get<Q0>(m);
                auto n_par = r_Q0.dx().size();
                return Transfer_Op_to<C_Patch_Model, Patch_State_and_Hessian>(
    ...
    template <uses_adaptive_aproximation adaptive, uses_recursive_aproximation recursive,
              ...
              uses_variance_correction_aproximation variance_correction,
              return_predictions predictions, class FuncTable, class C_Parameters, class Model>
    ...
    auto log_Likelihood(FuncTable& f, const Model& model, const C_Parameters& par,
        ...
        -> Maybe_error<std::conditional_t<
            ...
            std::conditional_t<predictions.value == 2, Patch_State_Evolution,
        ...
        using C_Patch_State = std::conditional_t<
            var::is_derivative_v<C_Parameters>,
            Op_t<Transf, std::conditional_t<
                             predictions.value == 2, Patch_State_and_Evolution,
                             std::conditional_t<predictions.value == 1, Patch_State_and_y_Evolution,
                                                Patch_State_and_Hessian>>>,
        ...
        if (!is_valid(Maybe_m))
            ...
            if (!ini)
                ...
                auto run = std::move(Maybe_run.value());
                if constexpr (var::is_derivative_v<C_Parameters>) {
                    ...
                    if constexpr (predictions.value == 2)
                        return get<Patch_State_Evolution>(run);
                    else if constexpr (predictions.value == 1)
                        return Vector_Space<logL, elogL, vlogL, ymean_Evolution, yvar_Evolution>(
                            get<logL>(run), get<elogL>(run), get<vlogL>(run),
                            get<ymean_Evolution>(run), get<yvar_Evolution>(run));
                    else
...
struct MacroR2 {
    ...
    auto operator()(T&& x, Ts&&... xs) {
        auto m = Macro_DMR{};
        auto l1 =
            m.Macror<recursive{}.value, averaging{}.value, variance{}.value,
                     variance_correction{}.value>(std::forward<T>(x), std::forward<Ts>(xs)...);

        if constexpr (false) {
            auto l2 = m.Macror_old<recursive{}.value, averaging{}.value, variance{}.value,
...
auto make_Likelihood_Model(const Model& m, Simulation_n_sub_dt n_sub_dt) {
    return Likelihood_Model<adaptive, recursive, averaging, variance, variance_correction, Model>(
        m, n_sub_dt);
}

template <uses_adaptive_aproximation adaptive, uses_recursive_aproximation recursive,
          uses_averaging_aproximation averaging, uses_variance_aproximation variance,
          uses_variance_correction_aproximation variance_correction, class FuncTable, class Model,
          class Parameters, class Recoding, class Experiment>
Maybe_error<logLs> logLikelihood(FuncTable& f,
    ...
    auto v_logL = Macro_DMR{}
                      .log_Likelihood<adaptive, recursive, averaging, variance, variance_correction,
                                      return_predictions(0)>(f, lik.m, p, y, e);
    if (!v_logL)
        return v_logL.error();
    else
...
Maybe_error<dlogLs> dlogLikelihood(FuncTable& f,
                                   const Likelihood_Model<adaptive, recursive, averaging, variance,
                                                          variance_correction, Model>& lik,
                                   var::Parameters_transformed const& p, const DataType& y,
                                   const Variables& var) {
    auto dp = var::selfDerivative(p);
    auto dpp = dp.to_value();
    auto v_logL = Macro_DMR{}
                      .log_Likelihood<adaptive, recursive, averaging, variance, variance_correction,
                                      return_predictions(0)>(f, lik.m, dpp, y, var);
    if (!v_logL)
        return v_logL.error();
    else
...
Maybe_error<dlogLs> diff_logLikelihood(
    FuncTable& f,
    const Likelihood_Model<adaptive, recursive, averaging, variance, variance_correction, Model>&
        lik,
    ...
    auto Maybe_logL =
        Macro_DMR{}
            .log_Likelihood<adaptive, recursive, averaging, variance, variance_correction,
                            return_predictions(1)>(f, lik.m, v_p, y, var);
    if (!Maybe_logL)
    ...
    for (std::size_t i = 0; i < ny; ++i) {
        auto yvar = get<yvar_Evolution>(v_logL)()[i]();
        auto ymean = get<ymean_Evolution>(v_logL)()[i]();
        if (std::isfinite(yvar) && std::isfinite(ymean) && (yvar > 0)) {
            t_yvar[i] = 1.0 / yvar;
    ...
    for (std::size_t ipar = 0; ipar < npar; ++ipar) {
        ...
        auto Maybe_logL_y =
            Macro_DMR{}
                .log_Likelihood<adaptive, recursive, averaging, variance, variance_correction,
                                return_predictions(1)>(f, lik.m, v_pi, y, var);
        if (!Maybe_logL_y)
            return Maybe_logL_y.error();
        auto logL_y = std::move(Maybe_logL_y.value());
        G[ipar] = (get<logL>(logL_y)() - get<logL>(v_logL)()) / delta_par;
        for (std::size_t i = 0; i < ny; ++i) {
            J_y(i, ipar) =
                (get<ymean_Evolution>(logL_y)()[i]() - get<ymean_Evolution>(v_logL)()[i]()) /
                delta_par;
            J_v(i, ipar) =
                (get<yvar_Evolution>(logL_y)()[i]() - get<yvar_Evolution>(v_logL)()[i]()) /
                delta_par;
        }
    }
    ...
    return dlogLs(get<logL>(v_logL), get<elogL>(v_logL), get<vlogL>(v_logL), Grad(std::move(G)),
                  FIM(std::move(r_FIM)));
}
...
Maybe_error<logL_y_yvar> logLikelihood_Y_Predictions(
    ...
    return Macro_DMR{}
        .log_Likelihood<adaptive, recursive, averaging, variance, variance_correction,
                        return_predictions(1)>(f, lik.m, p, y, var);
}
...
Maybe_error<std::vector<Patch_State_Evolution>> fractioned_logLikelihoodPredictions(
    ...
    for (std::size_t i = 0; i < y.size(); ++i) {
        auto Maybe_e =
            Macro_DMR{}
                .log_Likelihood<adaptive, recursive, averaging, variance, variance_correction,
                                return_predictions(2)>(f, lik.m, p, get<Recording>(y[i]()), var[i]);
        if (Maybe_e)
...
template <uses_adaptive_aproximation adaptive, uses_recursive_aproximation recursive,
          uses_averaging_aproximation averaging, uses_variance_aproximation variance,
          uses_variance_correction_aproximation variance_correction, class Model, class Parameters,
          class Variables>
    requires(!is_of_this_template_type_v<Variables, std::vector>)
auto simulate(mt_64i& mt,
              const Likelihood_Model<adaptive, recursive, averaging, variance, variance_correction,
                                     Model>& lik,
...
inline Maybe_error<std::tuple<std::string, std::string, double, double>> calc_simulation_fractions(
    std::string save_name, std::string simulation, experiment_type experiment,
    fraction_algo_type fraction_algo, Maybe_error<std::size_t> Maybe_num_param,
    std::size_t i_seed) {
    if (!Maybe_num_param)
        return Maybe_num_param.error();
