...
*/
  ...
  std::pair<C_Patch_Model, double>
  remove_Patch_inactivation(C_Patch_Model const &mi) {
    auto m = mi;
  ...
  using logL_y_yvar =
  var::Vector_Space<logL, elogL, vlogL, ymean_Evolution, yvar_Evolution>;
  ...
  Vector_Space<logL, elogL, vlogL, P_mean, P_Cov, y_mean, y_var, plogL,
  eplogL, vplogL, macror_algorithm, ymean_Evolution,
  yvar_Evolution>;
...
class Macro_DMR {
  ...
  static C_double E1(C_double const &x,
  ...
  template <class C_double>
  requires U<C_double, double>
  static C_double E2(C_double const &x, C_double const &y,
...
public:
  ...
  Maybe_error<Transfer_Op_to<C_Patch_Model, Qdtg>>
                                                   calc_Qdtg_eig(FunctionTable &&, const C_Patch_Model &m, const C_Qx_eig &t_Qx,
                                                                 number_of_samples ns, double dt) {
...
for (std::size_t j = 0; j < i + 1; ++j) {
set(E2_sqr, i, j,
E2(v_ladt[i], v_ladt[j], v_exp_ladt[i], v_exp_ladt[j]));
}
}
...
requires(/*U<C_Patch_Model, Patch_Model> && */
         U<C_Qx, Qx>)
Maybe_error<Transfer_Op_to<C_Patch_Model, Qdtg>>
                                                 calc_Qdtg_taylor(const C_Patch_Model &m, const C_Qx &t_Qx,
    ...
    to_Transition_Probability(expm_taylor(t_Qrun_sub, order));
                                                 ...
                                                 return build<Qdtg>(ns, get<min_P>(m), std::move(Maybe_P_sub.value()),
                                                                    get<g>(m));
                                                 }
                                                 }
...
static bool crude_gmean_violation(gmean_i const &v_gm, const g &v_g) {
  ...
  if (!((max_g_m <= max_g) && (min_g_m >= min_g))) {
      std::cerr << "max_g_m=" << max_g_m << " max_g=" << max_g
                << " min_g_m=" << min_g_m << " min_g=" << min_g << "\n";
      return true;
    } else
    return false;
}
...
static auto Qn_to_Qdt(const C_Qn &x, const C_Patch_Model &m) {
  ...
  auto b_gtotal_ij =
      force_gmean_in_range(build<gtotal_ij>(r_gtotal_ij), get<g>(m));
  
  auto r_gmean_ij = elemDivSafe(b_gtotal_ij(), get<P>(x)(), get<min_P>(x)());
  auto b_gmean_ij =
      force_gmean_in_range(build<gmean_ij>(r_gmean_ij), get<g>(m));
  
  auto r_gtotal_var_ij =
      r_gtotal_sqr_ij - elemMult(b_gtotal_ij(), b_gmean_ij());
...
// requires(U<C_Patch_Model, Patch_Model>)
auto calc_Qdtg_ATP_step(FunctionTable &f, const C_Patch_Model &m,
...
-> Maybe_error<Transfer_Op_to<C_Patch_Model, Qdtg>> {
  ...
  if (t_Qeig) {
      auto Maybe_Qdt = calc_Qdtg_eig(f, m, t_Qeig.value(),
...
// requires(U<C_Patch_Model, Patch_Model>)
auto
calc_Qdtg(FunctionTable &f, const C_Patch_Model &m, const ATP_step &t_step,
...
// requires(U<C_Patch_Model, Patch_Model> )
auto
calc_Qdtg(FunctionTable &f, const C_Patch_Model &m,
          const std::vector<ATP_step> &t_step,
          double fs) -> Maybe_error<Transfer_Op_to<C_Patch_Model, Qdtg>> {
  if (t_step.empty())
    return error_message("Emtpy ATP step");
  ...
  for (std::size_t i = 1; i < t_step.size(); ++i) {
      ...
      if (!v_Qdti)
        return v_Qdti.error();
          ...
          auto Maybe_v_Prun =
              to_Transition_Probability(v_Prun() * get<P>(v_Qdti.value())());
          if (!Maybe_v_Prun)
  ...
  return build<Qdtg>(number_of_samples(v_ns), get<min_P>(m),
                     std::move(v_Prun), get<g>(m));
}
...
template <uses_recursive_aproximation recursive,
          ...
          class C_double>
Maybe_error<
std::tuple<Transfer_Op_to<C_Qdt, y_mean>, Transfer_Op_to<C_Qdt, y_var>,
Transfer_Op_to<C_Qdt, P_mean>, Transfer_Op_to<C_Qdt, P_Cov>,
Transfer_Op_to<C_Qdt, double>, macror_algorithm>>
safely_calculate_y_mean_yvar_Pmean_PCov(
    C_Patch_State const &t_prior, C_Qdt const &t_Qdt, C_Patch_Model const &m,
    C_double const &N, const Patch_current &p_y, double fs) const {
  ...
  if constexpr (averaging.value == 0) {
    auto &t_g = get<g>(m);
    auto r_y_mean =
        build<y_mean>(N * getvalue(p_P_mean() * t_g()) + y_baseline());
    ...
    if constexpr (variance_correction.value) {
      ...
      auto sSs = getvalue(TranspMult(t_gvar_i(), SmD) * t_gvar_i()) +
          getvalue(p_P_mean() *
                   (elemMult(t_gtotal_var_ij(), t_gvar_ij()) * u));
      ...
      if (!std::isfinite(
            primitive(N * ms0 + N * gSg - N * zeta * sqr(sSg))) ||
        ...
        return safely_calculate_yvar<recursive, averaging, variance,
            uses_variance_correction_aproximation(
              false)>(t_Qdt, m, fs, N, p_P_mean,
                      SmD, y, y_baseline);
      
      auto r_y_var =
          build<y_var>(e + N * ms0 + N * gSg - N * zeta * sqr(sSg));
      ...
      auto sS = TranspMult(t_gvar_i(), SmD) * t_P() +
          p_P_mean() * t_gtotal_var_ij();
      
      auto Maybe_r_P_mean =
          to_Probability(p_P_mean() * t_P() + chi * gS -
                         (chi * zeta * sSg + 0.5 / e_mu) * sS);
      ...
      if (!all_Probability_elements(primitive(r_P_mean())) ||
        ...
        return safely_calculate_Pmean_Pcov<
            ...
            uses_variance_correction_aproximation(false)>(
              is_y_nan, std::move(r_y_mean), std::move(r_y_var), t_prior, t_Qdt,
              m, N, p_y, fs, SmD);
      else {
          ...
          return std::tuple(std::move(r_y_mean), std::move(r_y_var),
                            std::move(r_P_mean), std::move(r_P_cov),
                            std::move(chi2), std::move(r_macro_algo));
        }
      ...
      if (!std::isfinite(primitive(gSg)) || primitive(gSg) <= 0)
        return safely_calculate_y_mean_yvar_Pmean_PCov<
            recursive, uses_averaging_aproximation(1), variance,
            variance_correction>(t_prior, t_Qdt, m, N, p_y, fs);
      auto r_y_mean = build<y_mean>(N * getvalue(p_P_mean() * t_gmean_i()) +
                                    y_baseline());
      if constexpr (PoissonDif)
          e = e + get<Proportional_Noise>(m).value() * abs(y - r_y_mean());
      ...
      if constexpr (variance.value) {
        ...
        if (std::isfinite(primitive(ms)) && primitive(ms) >= 0) {
            ...
            return safely_calculate_Pmean_Pcov<
                ...
                variance_correction>(is_y_nan, std::move(r_y_mean),
    ...
    if (!std::isfinite(primitive(gSg)) || primitive(gSg) <= 0)
      return safely_calculate_y_mean_yvar_Pmean_PCov<
          recursive, uses_averaging_aproximation(0),
          uses_variance_aproximation(false),
          uses_variance_correction_aproximation(false)>(t_prior, t_Qdt, m, N,
                                                        p_y, fs);
    auto r_y_mean =
...
auto calculate_logL(bool y_is_nan, C_y_var const &r_y_var, auto &chi2,
                    auto &p_logL, auto &p_elogL, auto &p_vlogL,
                    auto &m) const {
  if (y_is_nan)
    return std::tuple(Transfer_Op_to<C_y_var, plogL>(NaN),
                      Transfer_Op_to<C_y_var, eplogL>(NaN), vplogL(0.0),
                      p_logL, p_elogL, p_vlogL);
  else {
      
      if (get<Proportional_Noise>(m).value() == 0) {
          auto r_plogL = build<plogL>(
                -0.5 * log(2 * std::numbers::pi * r_y_var()) - 0.5 * chi2);
          if constexpr (var::is_derivative_v<std::decay_t<decltype(r_plogL)>>)
              if (std::isnan(var::derivative(r_plogL())()[0]))
              std::cerr << "nan der\n";
          auto r_pelogL =
              build<eplogL>(-0.5 * log(2 * std::numbers::pi * r_y_var()) - 0.5);
          ...
          auto r_plogL =
              build<plogL>(-log(Poisson_noise_normalization(
                                  primitive(r_y_var()),
                                  primitive(get<Proportional_Noise>(m).value()))) -
                           0.5 * chi2);
          auto r_pelogL =
              Transfer_Op_to<C_y_var, eplogL>(Poisson_noise_expected_logL(
                                                primitive(r_y_var()),
                                                primitive(get<Proportional_Noise>(m).value())));
          auto r_logL = build<logL>(p_logL() + r_plogL());
          auto r_elogL = build<elogL>(p_elogL() + r_pelogL());
...
Transfer_Op_to<C_Qdt, P_mean>, Transfer_Op_to<C_Qdt, P_Cov>,
Transfer_Op_to<C_Qdt, double>, macror_algorithm>>
safely_calculate_Pmean_Pcov(bool y_is_nan,
                            Transfer_Op_to<C_Qdt, y_mean> &&r_y_mean,
  ...
  if constexpr (!recursive.value) {
    ...
    auto Maybe_r_P_cov =
        to_Covariance_Probability(r_P_cov() + diag(r_P_mean()));
...
requires(
    /*(U<std::decay_t<C_Patch_State>,
                                                   ...
                                                   &&*/
    U<C_double, double> &&
    (U<C_Qdt, Qdt> || U<C_Qdt, Qdtm> || U<C_Qdt, Qdtg>))
...
Maybe_error<C_Patch_State> Macror(FunctionTable &, C_Patch_State &&t_prior,
  ...
  auto [r_y_mean, r_y_var, r_P_mean, r_P_cov, chi2, r_algo] =
      std::move(Maybe_all.value());
  auto y_baseline = get<Current_Baseline>(m);
  ...
  auto [r_plogL, r_eplogL, r_vplogL, r_logL, r_elogL, r_vlogL] =
      calculate_logL(std::isnan(y), r_y_var, chi2, get<logL>(t_prior),
                     get<elogL>(t_prior), get<vlogL>(t_prior), m);
  ...
  if (!std::isnan(y) && std::isnan(primitive(r_plogL()))) {
    ...
    ev().push_back(build<Patch_State>(r_logL, r_elogL, r_vlogL, r_P_mean,
                                      r_P_cov, r_y_mean, r_y_var, r_plogL,
                                      r_eplogL, r_vplogL, r_algo));
    return build<Patch_State_and_Evolution>(
          r_logL, r_elogL, r_vlogL, r_P_mean, r_P_cov, r_y_mean, r_y_var,
          r_plogL, r_eplogL, r_vplogL, r_algo, std::move(ev));
    ...
    return build<Patch_State_and_y_Evolution>(
          r_logL, r_elogL, r_vlogL, r_P_mean, r_P_cov, r_y_mean, r_y_var,
          r_plogL, r_eplogL, r_vplogL, r_algo, std::move(yev), std::move(yvev));
    ...
    return build<Patch_State_and_Hessian>(r_logL, r_elogL, r_vlogL, r_P_mean,
                                          r_P_cov, r_y_mean, r_y_var, r_plogL,
                                          r_eplogL, r_vplogL, r_algo,
                                          FIM(get<FIM>(t_prior)() + r_FIM));
  } else {
    return build<Patch_State>(r_logL, r_elogL, r_vlogL, r_P_mean, r_P_cov,
...
Macror_old(FunctionTable &, C_Patch_State &&t_prior, C_Qdt const &t_Qdt,
  ...
  if (std::isnan(y)) {
      ...
      if constexpr (U<C_Patch_State, Patch_State_and_Evolution>) {
        ...
        ev().push_back(build<Patch_State>(build<logL>(get<logL>(t_prior)()),
                                          build<elogL>(get<elogL>(t_prior)()),
                                          build<vlogL>(get<vlogL>(t_prior)()),
                                          r_P_mean, r_P_cov, r_y_mean, r_y_var,
                                          plogL(NaN), eplogL(NaN), vplogL(NaN),
                                          get<macror_algorithm>(t_prior)));
        return build<Patch_State_and_Evolution>(
              build<logL>(get<logL>(t_prior)()),
              build<elogL>(get<elogL>(t_prior)()),
  ...
  if (std::isnan(primitive(r_plogL()))) {
    ...
    return build<Patch_State_and_y_Evolution>(
          ...
          build<vlogL>(get<vlogL>(t_prior)() + r_vlogL()), std::move(r_P_mean),
          std::move(r_P_cov), std::move(r_y_mean), std::move(r_y_var), r_plogL,
          r_eplogL, r_vlogL, get<macror_algorithm>(t_prior), std::move(yev),
          std::move(yvev));
  } else if constexpr (U<C_Patch_State, Patch_State_and_Hessian>) {
...
Patch_State_and_y_Evolution, Patch_State>>>> {
  ...
  if constexpr (var::is_derivative_v<C_Patch_Model>) {
    ...
    return Transfer_Op_to<C_Patch_Model, Patch_State_and_y_Evolution>(
          logL(0.0), elogL(0.0), vlogL(0.0), std::move(r_P_mean),
          std::move(r_P_cov), y_mean(NaN), y_var(NaN), plogL(NaN), eplogL(NaN),
          vplogL(NaN), macror_algorithm(""), ymean_Evolution(),
          yvar_Evolution());
  } else if constexpr (predictions.value == 2) {
    return Transfer_Op_to<C_Patch_Model, Patch_State_and_Evolution>(
          logL(0.0), elogL(0.0), vlogL(0.0), std::move(r_P_mean),
          std::move(r_P_cov), y_mean(NaN), y_var(NaN), plogL(NaN), eplogL(NaN),
          vplogL(NaN), macror_algorithm(""), Patch_State_Evolution());
  } else {
    return Transfer_Op_to<C_Patch_Model, Patch_State>(
          logL(0.0), elogL(0.0), vlogL(0.0), std::move(r_P_mean),
          std::move(r_P_cov), y_mean(NaN), y_var(NaN), plogL(NaN), eplogL(NaN),
...
std::conditional_t<
predictions.value == 2, Patch_State_Evolution,
        ...
        return ini.error();
          ...
          if constexpr (var::is_derivative_v<C_Parameters>) {
            ...
            if constexpr (predictions.value == 2)
                ...
                return Vector_Space<logL, elogL, vlogL, ymean_Evolution,
                yvar_Evolution>(
...
Maybe_error<Simulated_Recording<includes_N_state_evolution(false)>>
                                                                    ...
                                                                    sample_N(mt_64i &mt, const Model &model, const var::Parameters_values &par,
                                                                             const Experiment &e, const Simulation_Parameters &sim,
                                                                             const Recording &r = Recording{}) {
  ...
  template <class T, class... Ts> auto operator()(T &&x, Ts &&...xs) {
    ...
    if constexpr (false) {
      auto l2 = m.Macror_old<recursive{}.value, averaging{}.value,
          variance{}.value, variance_correction{}.value>(
...
Maybe_error<dlogLs> diff_logLikelihood(
  ...
  for (std::size_t ipar = 0; ipar < npar; ++ipar) {
      ...
      for (std::size_t i = 0; i < ny; ++i) {
          J_y(i, ipar) = (get<ymean_Evolution>(logL_y)()[i]() -
                          get<ymean_Evolution>(v_logL)()[i]()) /
              delta_par;
          J_v(i, ipar) = (get<yvar_Evolution>(logL_y)()[i]() -
                          get<yvar_Evolution>(v_logL)()[i]()) /
              delta_par;
        }
    }
  ...
  return dlogLs(get<logL>(v_logL), get<elogL>(v_logL), get<vlogL>(v_logL),
                Grad(std::move(G)), FIM(std::move(r_FIM)));
}
                  ...
                  if (is_valid(prediction)) {
                      ...
                      for (std::size_t i_step = 0; i_step < size(y); ++i_step) {
                          ...
                          s.f << iter << s.sep << dur.count() << s.sep << i_b << s.sep
                              ...
                              << s.sep << walker_id << s.sep << i_step << s.sep << time
                              << s.sep << num_samples << s.sep << y()[i_step]() << s.sep
                  ...
                  for (std::size_t i_from = 0; i_from < v_Q0().nrows(); ++i_from)
                    for (std::size_t i_to = 0; i_to < v_Q0().ncols(); ++i_to) {
                        if (v_Qa()(i_from, i_to) > 0)
                          s.f << iter << s.sep << dur.count() << s.sep << i_b << s.sep
                              << beta.size() << s.sep << beta[i_b] << s.sep << i_walker
                              << s.sep << walker_id << s.sep << "agonist" << s.sep
