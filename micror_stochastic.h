...
class Micror_parameters_likelihood{
    ...
    logLikelihood(FuncTable&& f, Micror_parameters_likelihood ,
                  Micror_state const &p, const Patch_current &y, const Vector_Space<gmean_ij,gvar_ij,Current_Noise> &x)
    {
...
Micror_stochastic(FunctionTable &ftbl, C_Patch_State &&t_prior,
  ...
  auto opt = evidence(ftbl, std::move(tmi), r_prior,
                      Micror_parameters_likelihood{}, p_y, Vector_Space(get<gmean_ij>(t_Qdt),get<gvar_ij>(t_Qdt),
                                                                                                       get<Current_Noise>(m)));
}
