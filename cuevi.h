...
public:
  ...
  Walker_statistics &operator+=(const Walker_statistics &other) {
    ...
    get<Likelihood_statistics>((*this)()) +=
        get<Likelihood_statistics>(other());
    get<emcee_Step_statistics>((*this)())() +=
        get<emcee_Step_statistics>(other())();
    get<Thermo_Jump_statistics>((*this)())() +=
        get<Thermo_Jump_statistics>(other())();
    get<Cuevi_Jump_statistics>((*this)())() +=
        get<Cuevi_Jump_statistics>(other())();
    return *this;
  }
};
...
template <class ParameterType> class Cuevi_mcmc {
  ...
  class Walker_value
    ...
    thermo_jump(Th_Beta ca_beta, Fraction_Index ca_fra,
      ...
      bool current_is_big_0_is_NaN = (cu_fra() > ca_fra()) &&
                                     (!logL_cu_ca.valid()) &&
                                     (logL_cu_ca.valid());
      bool candidate_is_big_0_is_NaN =
          ((cu_fra() < ca_fra()) && (logL_cu_ca.valid()) &&
           (!logL_cu_ca.valid()));
      bool current_sum_is_NaN_after_is_not =
          (!current_sum.valid() && after_jump_sum.valid());
  ...
  static Maybe_error<double> cuevi_jump_logProb(
    ...
    auto Maybe_00 =
        calc_Relevant_Likelihoods(f, lik, y, x, t, w_0, i_0, wa_sta_0);
    auto Maybe_01 =
        calc_Relevant_Likelihoods(f, lik, y, x, t, w_0, i_1, wa_sta_0);
    auto Maybe_10 =
        calc_Relevant_Likelihoods(f, lik, y, x, t, w_1, i_0, wa_sta_1);
    auto Maybe_11 =
        calc_Relevant_Likelihoods(f, lik, y, x, t, w_1, i_1, wa_sta_1);
...
public:
  ...
  auto get_Cuevi_Statistics(Cuevi_Index i_cu) { return m_sta[i_cu]; }
...
#pragma omp parallel for// collapse(2)
  ...
  friend void report(FunctionTable &f, std::size_t iter, const Duration &dur,
                     save_likelihood<ParameterType> &s, Cuevi_mcmc &data,
                     Prior &&, t_logLikelihood &&lik,
        ...
        for (std::size_t i_walker = 0; i_walker < data.get_Walkers_number();
          ...
          for (std::size_t i_par = 0; i_par < data.get_Parameters_number();
            ...
            s.f << iter << s.sep << dur << s.sep << i_cu << s.sep << i_fra()
                ...
                << mts[i_mts].pos() << s.sep << i_par << s.sep
  ...
  friend void report_title(save_Parameter<ParameterType> &s, Cuevi_mcmc const &,
    ...
    s.f << "iter" << s.sep << "iter_time" << s.sep << "i_cu" << s.sep
        ...
        << "i_walker" << s.sep << "walker_id" << s.sep << "i_mt" << s.sep
        << "mt_pos" << s.sep << "i_par" << s.sep << "parameter_value";
  ...
  template <class FunctionTable, class Duration, class Prior,
            class t_logLikelihood, class Data, class Variables>
  friend void report(FunctionTable &f, std::size_t iter, const Duration &dur,
                     save_Evidence &s, Cuevi_mcmc &data, Prior &&,
...
class Cuevi_Algorithm_no_Fractioner
    : public var::Var<
          ...
          var::Vector_Space<
              Num_Walkers_Per_Ensemble, var::Constant<Reporter, t_Reporter>,
...
public:
  Cuevi_Algorithm_no_Fractioner(
      t_Reporter &&rep, t_Finalizer &&f, Num_Walkers_Per_Ensemble n,
      ...
      : base_type(var::Vector_Space(
            ...
            var::Constant<Finalizer, t_Finalizer>(std::move(f)),
            std::move(beta), std::move(max_iter_for_sampling),
            std::move(thermo_jumps_every), std::move(randomize),
...
auto evidence_old(FunctionTable &ff,
  ...
  if (!Maybe_current)
    ...
    while (!mcmc_run.second) {
      ...
      ++iter;
      f.f(thermo_cuevi_jump_mcmc{}, iter, current, mt, mts, prior, lik, ys, xs,
...
auto evidence_loop(FunctionTable &f, std::pair<mcmc_type, bool> &&mcmc_run,
  ...
  while (!mcmc_run.second) {
    ...
    ++iter;
    f.f(thermo_cuevi_jump_mcmc{}, iter, current, mts, prior, lik, ys, xs,
...
auto evidence(FunctionTable &f,
  ...
  auto Maybe_current = Cuevi_mcmc<Parameter_Type>::init(
      f, mts, std::forward<Prior>(prior), lik, ys, xs,
  ...
  if (!Maybe_current)
    ...
    return evidence_loop(f, std::move(mcmc_run), rep, iter, current, mts,
                         v_random_jumps, v_thermo_jump_every,
                         std::forward<Prior>(prior), lik, ys, xs);
  }
}

template <class FunctionTable, class t_Reporter, class t_Finalizer, class Prior,
          class Likelihood, class DataType, class Variables>
auto evidence_fraction(
    FunctionTable &f,
...
auto continue_evidence(
  ...
  if (!Maybe_current)
    ...
    return evidence_loop(f, v_thermo_jump_every, mcmc_run, rep, iter, current,
                         mts, std::forward<Prior>(prior), lik, ys, xs);
  }
}
...
template <class Parameters> struct cuevi_mcmc {
  ...
  friend void report_title(save_likelihood<Parameters> &s,
    ...
    s.f << "n_fractions" << s.sep << "n_betas" << s.sep << "iter" << s.sep
        ...
        << "i_walker" << s.sep << "id_walker" << s.sep << "logPa" << s.sep
        << "logP" << s.sep << "logLik"
        << "\n";
  }
  friend void report_title(save_Evidence &s, cuevi_mcmc const &, ...) {
...
void report(FunctionTable &&, std::size_t iter, const Duration &dur,
            save_likelihood<Parameters> &s, cuevi_mcmc<Parameters> const &data,
            ...) {
  if (iter % s.save_every == 0)
        ...
        if (data.is_active[i_frac][i_beta] == 1)
          for (std::size_t i_walker = 0; i_walker < size(data.walkers);
            ...
            s.f << size(data.beta) << s.sep << size(data.beta[i_frac]) << s.sep
                << iter << s.sep << dur << s.sep << data.nsamples[i_frac]
                << s.sep << data.beta[i_frac][i_beta] << s.sep << i_walker
                << s.sep << data.i_walkers[i_walker][i_frac][i_beta] << s.sep
...
template <class FunctionTable, class Duration, class Parameters>
void report(FunctionTable &&, std::size_t iter, const Duration &dur,
            save_Parameter<Parameters> &s, cuevi_mcmc<Parameters> const &data,
            ...) {
  if (iter % s.save_every == 0)
    for (std::size_t i_frac = 0; i_frac < size(data.beta); ++i_frac)
      for (std::size_t i_beta = 0; i_beta < size(data.beta[i_frac]); ++i_beta)
...
void report(FunctionTable &&, std::size_t iter, const Duration &dur,
            save_Evidence &s, cuevi_mcmc<Parameters> const &data,
            T const &...) {
  if (iter % s.save_every == 0) {
...
template <>
inline bool
compare_to_max_ratio(deprecated::by_fraction<by_beta<double>> const &beta,
