...
TEST_CASE("calculation of the likelihood of  fractions of a simulation with N_states",
          "[likelihood of fraction]") {
    ...
    macrodr::cmd::parameters_value_type par =
        macrodr::cmd::load_Parameter_value("../macro_dr/models/scheme_4_inact_par.csv", ",");
    ...
    auto Maybe_frac =
        calc_simulation_fractions(save_name, fsimulation, fexperiment, fraction_algo, 8, 1);
    REQUIRE(Maybe_frac);
    ...
    auto lik_algo = set_Likelihood_algorithm(
        adaptive_aproximation, recursive_approximation, averaging_approximation,
    ...
    auto Maybe_lik = calc_fraction_likelihood(lik_name, model, par, Maybe_frac, lik_algo, ft);

    REQUIRE(Maybe_lik);
}

TEST_CASE(
    "simulate a model and compare the partial likelihood with the expected partial likelihood",
    "[likelihood of fraction]") {
    using namespace macrodr;
    using namespace cmd;
    ...
    recording_type experiment_observations =
        "../macro_dr/experiments/Moffatt_Hume_2007_ATP_time_recording.txt";

    auto experiment_cond =
        get_Experiment("../macro_dr/experiments/Moffatt_Hume_2007_ATP_time.txt", 50e3, 0);

    parameters_value_type param_file =
        load_Parameter_value("../macro_dr/models/scheme_4_inact_par.csv", ",");

    auto sim_algo = set_simulation_algorithm(true, 100);

    auto sim_file = run_simulation(filename_prefix, experiment_observations, experiment_cond, 0ul,
                                   model, param_file, sim_algo);
    ...
    auto Maybe_lik =
        calc_fraction_likelihood(lik_name, model, param_file, Maybe_frac, lik_algo, ft);
