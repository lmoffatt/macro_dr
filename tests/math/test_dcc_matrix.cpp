...
TEST_CASE("dcc_matrix identity sanity", "[dcc][matrix]") {
    auto H = make_diag_spd({1.0, 1.0, 1.0});
    auto J = make_diag_spd({1.0, 1.0, 1.0});
    auto H0 = H;
    auto J0 = J;

    auto me = dcc_matrix(H, J);
    REQUIRE(me.valid());
    auto C = me.value();
...
TEST_CASE("dcc_matrix diagonal closed form", "[dcc][matrix]") {
...
TEST_CASE("dcc_matrix matches explicit sandwich form", "[dcc][matrix]") {
    ...
    REQUIRE(me.valid());
    auto C = me.value();
    auto out_dense = to_dense(C);

    auto Hinv_maybe = inv(to_dense(H));
    REQUIRE(Hinv_maybe.valid());
    auto Hinv = Hinv_maybe.value();
    auto Cref = Hinv * to_dense(J) * Hinv;

    REQUIRE(rel_residual_1(Cref, out_dense) < 1e-10);
    REQUIRE(symmetry_defect_1(out_dense) < 1e-12);
}

TEST_CASE("dcc_matrix fails when H is not SPD", "[dcc][matrix]") {
...
TEST_CASE("dcc_matrix fails when J is not SPD", "[dcc][matrix]") {
