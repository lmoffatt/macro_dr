...
template <template <class> class Matrix>
    requires(Matrix<double>::is_Matrix)
class Derivative<Matrix<double>,
                 double>  //: public Primitive<Matrix<double>>, public d_d_<Matrix<double>,double>
{
...
class Derivative<double,
                 Matrix<double>>  //: public Primitive<double>, public d_d_<double,Matrix<double>>
{
...
class Derivative<
    Matrix<double>,
    Matrix<
        double>>  //: public Primitive<Matrix<double>>, public d_d_<Matrix<double>,Matrix<double>>
{
    using primitive_type = Matrix<double>;
   ...
   public:
    ...
    auto operator()(std::size_t i, std::size_t j) const {
        return Derivative<double, Matrix<double>>(
            primitive()(i, j), applyMap([i, j](auto const& m) { return m(i, j); }, derivative()));
    }

    auto operator[](std::size_t i) const {
        return Derivative<double, Matrix<double>>(
            primitive()[i], applyMap([i](auto const& m) { return m[i]; }, derivative()));
    }
};
...
auto build(std::size_t nrows, std::size_t ncols,
           std::vector<std::tuple<std::size_t, std::size_t, double>>&& values) {
