...
inline void calc_thermo_evidence_fraction_dts(std::string id,
                                              std::string model,
                                 std::string prior,
    ...
    if (true) {
        ...
        save_vars(f, filename, model, prior, likelihood, fname_simulation, fname_experiment,fs,iniATP,
    ...
    if (!Maybe_model_v) {
        ...
        return std::visit(
            ...
            [&filename, &ftbl3, &xs, &ys, &prior, &likelihood,
             &thermo_algorithm, &myseed, sampling_interval, max_number_of_values_per_iteration](auto model0ptr) {
                std::string sep = ",";
                ...
                if (!Maybe_param1_prior) {
                    ...
                    auto param1_prior = std::move(Maybe_param1_prior.value());
                        ...
                        std::visit(
                            [&ftbl3, &tmi, &param1_prior, &ys,
                             &xs](auto &modelLikelihood) {
                                auto opt =
                                    thermo_fraction_evidence<true>(ftbl3, std::move(tmi), param1_prior,
                                                    modelLikelihood, ys, xs);
                            },
                            modelLikelihood_v);
...
inline void calc_thermo_evidence_fraction_dts_continuation(
    std::string id, std::size_t ith, std::size_t myinit_seed) {
    ...
    if (true) {
        ...
        load_vars(f, filename, model, prior, likelihood, fname_simulation, fname_experiment,fs,iniATP,
                  thermo_algorithm, sampling_interval,max_number_of_values_per_iteration, myseed);
    }
    ...
    if (Maybe_model_v) {
        auto model_v = std::move(Maybe_model_v.value());
        return std::visit(
            [&oldfilename, &ftbl3, &xs, &ys, &prior, &likelihood,
                    ...
                    auto param1_prior = std::move(Maybe_param1_prior.value());
                        ...
                        std::visit(
                            [&oldfilename,&ftbl3, &tmi, &param1_prior, &ys,
                             &xs](auto &modelLikelihood) {
                                auto opt =
                                    thermo_fraction_evidence_continuation<true>(oldfilename,ftbl3, std::move(tmi), param1_prior,
                                                                 modelLikelihood, ys, xs);
                            },
                            modelLikelihood_v);
                    }
