...
\[
  ...
  \frac{N_{\text{ch}}\,\mathbf{g}\mathbf{g}^{\top}}
       {\sigma^2_{\overline{y}^{\text{pred}}}},
\]
is precisely the Gaussian conditioning formula once one remembers that
\(
\boldsymbol{\Sigma}^{\text{prop}}=\Cov(\boldsymbol{N}(t))/N_{\text{ch}}
\)
and
\(
\mathbf{g}=\Cov(\boldsymbol{N}(t),\overline{y}_{0\to t})/N_{\text{ch}}
\)
are stored in per-channel normalisation.
