...
The covariance update is scaled by the same \(\alpha\) so that the posterior
mean and covariance remain consistent with the same trust-region correction.
...
bad choice: when the admissible trust coefficient reaches \(\alpha=0\), the
naive substitution of \(\alpha\) into the likelihood variance produces a
singular log term and unstable likelihood values. The current implementation
therefore uses \(\alpha\) only to scale the state correction
\((\boldsymbol{\mu},\boldsymbol{\Sigma})\), while \(\log L\) and \(E[\log L]\)
continue to use the unmodified predictive variance \(\sigma^2_{\text{pred}}\).
