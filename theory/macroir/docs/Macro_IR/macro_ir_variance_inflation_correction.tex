...
\title{Trust-Region Kalman Mean Update for MacroIR}
\author{}
\date{}
...
\begin{enumerate}
  ...
  \item keeps the likelihood calculation numerically stable, since MacroIR
...
We introduce a shrink factor \(\alpha_\mu\in[0,1]\) on the Kalman mean update
and compute the maximal \(\alpha_\mu\) that keeps the mean inside the safe
region (this is the simplex trust coefficient, the only one this document is
concerned with).

\paragraph{Mean and covariance trust are decoupled.}
The current implementation does \emph{not} apply this simplex shrink
\(\alpha_\mu\) to the covariance update. The mean update uses \(\alpha_\mu\)
alone; the covariance down-date is handled independently (it takes the full
\(\alpha=1\) step in the standard rank-1 branch, guarded by a separate
positive-semidefiniteness canary, and only requires its own PSD trust
coefficient \(\alpha_\sigma\) in the variance-correction Taylor branches).
Sharing \(\alpha_\mu\) with the covariance is in fact harmful: at a
likelihood-residual zero-crossing the bound \((p_{\min}\text{-type})\) defining
\(\alpha_\mu\) becomes singular in \(\delta\), whereas the mean step
\(\alpha_\mu\,\mathbf{g}\,\delta/\sigma^2_{\text{pred}}\) vanishes there and is
harmless; carrying that singularity into the (\(\delta\)-free) covariance
down-date destabilises the per-sample numerical Fisher. The justification for
keeping the mean and covariance on the same iterate is tempered-likelihood, not
a Kalman gain identity. The remainder of this document derives \(\alpha_\mu\)
from the probability constraints; the covariance equations below are written
with their own factor \(\alpha\) and should be read with \(\alpha=1\) in the
standard branch (\(\alpha=\alpha_\sigma\) only in the Taylor branches), never
with \(\alpha=\alpha_\mu\).

\paragraph{Implementation note.}
An earlier variant interpreted this shrink factor as an effective variance
inflation in the per-interval likelihood. In practice this turned out to be a
bad choice: when the admissible trust coefficient reaches \(\alpha_\mu=0\), the
naive substitution of \(\alpha_\mu\) into the likelihood variance produces a
singular log term and unstable likelihood values. The current implementation
therefore uses \(\alpha_\mu\) only to scale the mean correction
\(\boldsymbol{\mu}\) (the covariance is updated independently; see above), while
\(\log L\) and \(E[\log L]\) continue to use the unmodified predictive variance
\(\sigma^2_{\text{pred}}\).
...
\begin{align}
  ...
  \label{eq:alpha-cov}
\end{align}
Here \(\alpha\in[0,1]\).  We write the mean and covariance with a common
symbol \(\alpha\) only to expose the formal variance-inflation interpretation
\(\sigma^2_{\text{eff}} = \sigma^2_{\text{pred}}/\alpha\) in a standard Kalman
update (see Section~\ref{subsec:scaled-update-equivalence}); as noted above,
the implementation does not tie the covariance factor to the simplex shrink
\(\alpha_\mu\). In the standard rank-1 branch the covariance takes the full
\(\alpha=1\) down-date (its positive semidefiniteness is guaranteed by
construction, see Section~\ref{sec:covariance-psd}); a separate PSD trust
\(\alpha_\sigma\) is needed only in the variance-correction Taylor branches.
...
the observation and no shrink is needed.
...
\subsection{Step 4: Final shrink factor}
...
Two limiting cases are useful (recall that \(\alpha^\star\) acts on the
\emph{mean} only):
\begin{itemize}
  \item If \(\alpha^\star=1\), we recover the standard Gaussian mean update
        with no shrink.
  \item If \(\alpha^\star=0\), we perform \emph{no} mean correction:
        \(\boldsymbol{\mu}^{\text{post}}=\boldsymbol{\mu}^{\text{prop}}\).
        The covariance down-date is unaffected by \(\alpha^\star\) and still
        proceeds (see \eqref{eq:final-cov-update}). In practice, one may treat
        such intervals as fully rejected outliers \emph{for the mean}.
\end{itemize}
...
observation leaves the mean unchanged (the covariance down-date is governed by
its own factor and is unaffected).

The implemented mean update is therefore
\begin{align}
...
\end{align}
where \(\alpha^\star\) is the simplex shrink \(\alpha_\mu\) derived above. The
covariance is down-dated independently of \(\alpha^\star\):
\begin{align}
...
\end{align}
with \(\alpha=1\) in the standard rank-1 branch (full down-date, PSD by
construction; see Section~\ref{sec:covariance-psd}) and \(\alpha=\alpha_\sigma\)
only in the variance-correction Taylor branches. In particular, even when the
mean step is fully suppressed (\(\alpha^\star=0\), an outlier interval), the
standard covariance down-date is \emph{not} suppressed.

\subsection{Step 5: Log-likelihood contribution for this interval}

In the current implementation, the trust coefficient does not alter the
per-interval likelihood variance. The Gaussian log-likelihood therefore
remains
\[
  \ell
  =
  ...
  -\frac{1}{2}
  \left[
    \log\bigl(2\pi\sigma^2_{\text{pred}}\bigr)
    +
    \frac{\delta^2}{\sigma^2_{\text{pred}}}
  \right].
\]

This choice is deliberate. We tried the variance-inflation alternative, but
it can blow up when \(\alpha^\star=0\) because the likelihood normalisation
becomes singular. The adopted policy is to use \(\alpha^\star\) only as a
trust-region parameter for the posterior state update and to record it as a
diagnostic quantity (`trust\_coefficient`).
...
\subsection{Scaled update and a discarded variance-inflation interpretation}
...
\(\sigma^2_{\text{eff}}\) at the algebraic level.
...
This remains a useful formal interpretation of the scaled update, but it is
not the current likelihood implementation. Once \(\alpha\) is allowed to reach
zero, substituting it directly into the likelihood variance can make the log
term singular, so the code keeps the raw \(\sigma^2_{\text{pred}}\) in
\(\log L\) and \(E[\log L]\).
...
\subsection{Current log-likelihood choice}

The current implementation does not feed the trust coefficient back into the
likelihood variance. The per-interval Gaussian log-likelihood is therefore
\[
  \ell
  =
  -\frac{1}{2}
...
We previously explored the variance-inflation alternative suggested by the
algebra above. In practice that route was abandoned because the trust
coefficient can legitimately reach \(\alpha=0\) when the unconstrained update
would leave the admissible probability region, and the corresponding
likelihood normalisation blows up. The adopted policy is therefore:
\begin{itemize}
  \item use \(\alpha\) to limit the posterior state correction;
  \item record \(\alpha\) as a diagnostic (`trust\_coefficient`);
  \item compute \(\log L\) and \(E[\log L]\) from the original predictive
        variance.
\end{itemize}
