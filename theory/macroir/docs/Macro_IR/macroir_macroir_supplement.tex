...
\Sigma^{\text{prop}}(t)
-
\frac{N_{\mathrm{ch}}}{\sigma^2_{\overline{y}^{\mathrm{pred}}_{0\to t}}}
\widetilde{\gamma^{T}\Sigma}^T
\widetilde{\gamma^{T}\Sigma}.
\]

Here the factor \(N_{\mathrm{ch}}\) is required because
\(
\Sigma^{\text{prop}}(t)=\Cov(N(t))/N_{\mathrm{ch}}
\)
and
\(
\widetilde{\gamma^{T}\Sigma}
=
\Cov(N(t),\overline{y}_{0\to t})/N_{\mathrm{ch}}
\)
are both stored in the per-channel normalisation used by MacroIR.
