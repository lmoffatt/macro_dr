...
UNSOURCED="$(perl -ne '
  ...
  $t =~ s/\\(sub)?section\*?\{[^}]*\}//g;
  $t =~ s/\\\\\[[\d.]+\s*[a-z]+\]//g;          # LaTeX inter-row spacing, e.g. \\[1.5ex]
  $t =~ s/(Figures?|Figs?\.?|Tables?|Sections?|Equations?|Eqs?\.?|Regions?|indices|index|param(eter)?\s+index)~?\s*\d+//gi;   # numbering and cross-refs, not claims
  $t =~ s/~\d+\b//g;                           # any remaining tie-bound cross-reference number
  $t =~ s/(figure\s+)?supplements?\s*\d+(\s*and\s*\d+)*//gi;  # eLife "Figure N--figure supplement M" cross-refs, incl. "supplements 1 and 2"
  $t =~ s/source\s+data\s*\d+//gi;             # eLife "Figure N--source data M" cross-refs (added 2026-08-25)
...
# The figure set is SIX body figures plus supplements (papers/1_method/decisions.md,
# "The figure set", revised 2026-07-28: the usage map is a body figure, not a supplement). Body figures carry \caption;
