...
The canonical arc is `1_method/docs/manuscript-drafts/sections/04_results.md` (updated 2026-07-22), and the figure **set** (numbering,
...
1. **Correct the seed claim** in `1_method/docs/manuscript-drafts/sections/06_methods.md` M5, and decide what the paper says about it. The honest sentence is that the ensembles are large enough (1000 to 10000 recordings) that the reported statistics are stable, but the exact datasets cannot be regenerated bit for bit. Add the resolved seed to `meta.json` before any rerun.
