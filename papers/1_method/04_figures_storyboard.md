...
> **Arc superseded (retired doc).** The current figure set is owned by `decisions.md` "The figure set" (five body figures + supplements) and the argumentative arc by `docs/manuscript-drafts/sections/04_results.md`; the old `00_master_plan_v2.md` is itself retired. The panel-level detail below (especially Fig 1 and Fig 2) is kept only as reference. Do NOT treat the 4-figure arc or the Δ/τ_min × N_ch axes here as current.
