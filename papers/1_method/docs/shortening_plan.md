...
- (l) 'ribbons showing where each moves as the interval is swept...' fragment (1427-1428): one deletion executed once - counted under the results item, the identical captions item counted at zero; the Figure 7 caption copy (1381-1382) is the survivor.
  - **VOID 2026-08-26.** Both copies are gone and neither should come back: the canonical `map` chunk of figure_7.Rmd draws NO ribbon (geom_ribbon survives only in the archived `render`, `regions` and `frontiers` chunks), so the caption sentence this item preserved described something the panel does not show. The interval sweep is still computed and printed by the knit, and the map is a single slice at Delta*k_off = 0.1, which the caption and the first line of the subsection now say in words.
