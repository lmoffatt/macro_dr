...
\maketitle
\begin{abstract}
Biological measurements are never instantaneous. From electrophysiology to single-molecule spectroscopy, observations typically integrate stochastic dynamics over finite acquisition windows, yet many inference methods treat such data as point samples. This mismatch distorts the statistical information carried by the data and biases kinetic inference. Here we provide an information-theoretic analysis of inference from interval-averaged observations of continuous-time Markov models, using MacroIR, a recursive likelihood framework for macroscopic interval data. By conditioning each acquisition window on its initial and final states, MacroIR restores a Markov structure at the interval level, propagates boundary-conditioned moments in the original $k$-dimensional state space, without explicitely doint the $k^2$ expansion of boundary-conditioned models through the help of linear-operator identities. We compare MacroIR with alternative macroscopic likelihood approximations using the information distortion matrix, which quantifies how each method deforms the Fisher geometry of the exact interval-averaged likelihood. Across regimes where kinetic rates exceed acquisition speed, MacroIR preserves score consistency and Fisher information substantially better than competing approaches, while non-recursive and point-sampling methods systematically lose or misallocate information. A score-based diagnostic, testing mean score $\approx 0$ and score variance $\approx$ Fisher information, provides an internal check of likelihood self-consistency. These results establish MacroIR not merely as a practical correction for time averaging, but as an information-preserving inference principle for stochastic biological dynamics observed through finite acquisition windows.
\end{abstract}
