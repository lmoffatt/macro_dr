...
$\mathrm{av}=0$ the observation is treated as an instantaneous sample: the occupancy is
propagated to the sampling point first, the conductance is $\bm{\gamma}$, and
\begin{equation}
  \widetilde{\bm{\gamma}^\top\bm{\Sigma}\bm{\gamma}} \;\to\; \bm{\gamma}^\top\bm{\Sigma}\bm{\gamma},
  \qquad
  \mathbf{g} \;\to\; \mathbf{P}(\Delta)^\top\bm{\Sigma}\,\bm{\gamma}.
  \label{eq:av0}
\end{equation}
At $\mathrm{av}=1$ the same two expressions hold with $\bar{\bm{\gamma}}_0$ in place of
$\bm{\gamma}$: the conductance is conditioned on the state at the window's start, but the
contraction is still an ordinary quadratic form on the $K$ states. At $\mathrm{av}=2$ the
