...
That license has a boundary, and it bounds what the comparison verifies. The
likelihood and the simulator are two independent implementations, so everything
downstream of the point where they separate is tested: the propagation, the update,
the emission arithmetic, the accumulation. Upstream they share one specification, the
rate matrix built from the parameters, the conductance vector, the protocol and the
sampling times, and a diagnostic built on their agreement cannot see an error there.
For a two-state scheme that specification is small enough to be read and checked by
eye, which is why this section writes it out symbol by symbol below. The general
remedy is different in kind, verifying each object against the theorems it must
satisfy, the semigroup and Chapman-Kolmogorov identities for the propagator, the
stationarity condition, the normalisation of the occupancy and the moment identities
of the emission, and that is a piece of work with its own design, not undertaken
here.
% TRANSPLANT T2, 2026-08-11, the whole "what the diagnostic cannot see" paragraph, moved out of the
% Discussion (157 words) because it is a statement about this section's own specification and belongs
% beside the sentence that licenses the comparison. The Discussion keeps a one-clause pointer in its
% closing sentence; if this paragraph is ever removed, that clause has to be expanded back into a
% paragraph there.
Every
...
is the anchor the Diagnostics section writes $\mathbf{H}$, and the expectation is not
...
% summed over intervals", with no expectation, while 03_diagnostics.tex defines the same object with
...
rather than two. It is not interchangeable with the analytic anchor per recording: the
numerical Fisher is widely indefinite replicate by replicate, and only pooling across
replicates recovers a definite matrix, which is one reason the body is anchored on the
analytic construction. That battery (data directory \texttt{433ed13}) is retained in the
deposit, and no number reported in this paper is taken from it. Where the two constructions can be
compared on the runs the body does use, they agree for the members that condition on the past and
they do not agree for the one that does not: the analytic form reproduces the differenced one to
within about two per cent in size for \texttt{R}, \texttt{MR} and \texttt{IR} over the measured
range, while for \texttt{NR} it over-states the information as channels are added at low
instrumental noise, by factors of $1.26$ and $1.60$ at $N_{\mathrm{ch}}=10^{3}$ and $10^{4}$, and
misses by an amount that varies across directions by $1.9$ and $3.1$ there. Neither least-squares
arm carries the comparison, since the sweep that formed the differenced Fisher was not run for
them. % src: projects/eLife_2025/figures/figure_4_source_data/figure_4_source_data_gaussian_fisher.csv, pool anchor (medians over the seven intervals; the algorithms present are macro_NR, macro_R, macro_MR, macro_IR and the superseded macro_NMR, at noise 0.1, 1 and 10 only; nonlinearsqr, nonlinearsqr_g and macro_INR are absent from the file entirely, the _G sweep being Gaussian-only so the differenced Fisher is never formed there). Medians in size: R 1.011, MR 1.020, IR 1.000, NR 1.010 overall but 1.007/1.010/1.264/1.602 by channel count at noise 0.1. The single worst cell of the file, NR at N_ch 1e4 and interval 1, reads 11.5 and is the ill-conditioned corner Figure 4 greys, excluded from the sentence above for the same reason.
...
Coverage is deliberately not a rectangle and it differs by member: the boundary-conditioned
member spans eleven channel counts from $5$ to $10^4$ across all twelve noise levels, and every
