...
% their macroscopic partners to 1e-16 rather than being displaced by half an interval.
% SUBJECT REPAIRED 2026-08-25 (naive round, CONFIRMED): "Each arm therefore" had been swallowed
% into the comment line above, so the PDF printed "...(ILSE, data key nonlinearsqr). shares its...".
Each arm therefore shares its mean model exactly with the macroscopic member carrying the same averaging flag,
...
% which figures_build_plan.md:220 explicitly retires, since IR's TOTAL predicted variance is MR's.
% SUBJECT REPAIRED 2026-08-25 (naive round, CONFIRMED): "The least-squares arms and the macroscopic
% members are" had been swallowed into the comment line above, so the rendered caption printed
% "...carrying IR's variance. written by different producers...".
The least-squares arms and the macroscopic members are
...
MacroIR, the member called \texttt{IR} here, was compared; the deposited run metadata
of that study confirm the identification, its two arms differing in the recursion flag
alone, and the deposited endpoint flag coincides with this study's INR exactly, because
without a recursive update conditioning on one end and conditioning on both describe
the same likelihood, reproduced to the byte on the reference binary. It is reported
here only in the supplement.}
% src: papers/_program/decisions.md sec.2 (INR = MacroINR, the published control; supplement member 2026-07-29)
% src: docs/bibliography/MacroIR_prior_art_map.md A.11 (deposit metadata: 414 _likelihood_model.csv,
% recursive 0<->SNR / 1<->DR one to one, every other flag constant; SNR ran averaging=2)
% src: tmp/verify_av2_no_update.macroir on the reference binary macrodr-reference-0ffbda7, 2026-08-25:
% non-recursive averaging=2 vs averaging=1, same simulation, outputs byte-identical
% (tmp/verify_av2_diag_av{1,2}.csv, cmp -s). This closes A.11's one open [VERIFY] link.
...
on the simplex. Damping the step is what replaced correcting the state after it. The
earlier form of this code took the undamped step and truncated the occupancy entrywise at
zero before renormalising, which is not differentiable where it acts and so carries, one
level up, the defect this paragraph rejects for the coefficient itself; with the step
damped instead no operation produces a negative entry, and the renormalisation that
follows validates rather than repairs, failing on a real one.
The covariance is treated separately and takes its full step, since the
...
moments and the first derivatives the score has already produced; that is what makes it
positive semidefinite by construction and free of a second pass over the record. The
distortion quantities follow from the two. That layer reached the macroscopic likelihood
after the earlier analysis had been run, whose fits were obtained by Markov chain Monte
Carlo and needed no gradient. The dependence is not
