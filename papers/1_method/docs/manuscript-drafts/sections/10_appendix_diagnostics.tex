...
$\boldsymbol{\theta}_0$ the recordings were generated from,
\begin{equation}
  S(\hat{\boldsymbol{\theta}}) \;\approx\; S(\boldsymbol{\theta}_0)
    + \nabla_{\boldsymbol{\theta}} S(\boldsymbol{\theta}_0)\,(\hat{\boldsymbol{\theta}}-\boldsymbol{\theta}_0),
  \label{eq:score-expansion}
\end{equation}
and using $S(\hat{\boldsymbol{\theta}})=0$ at a maximum-likelihood estimate, with the empirical Hessian
replaced by its expectation $-\mathbf{H}$,
\begin{equation}
  \hat{\boldsymbol{\theta}}-\boldsymbol{\theta}_0 \;\approx\; \mathbf{H}^{-1}S(\boldsymbol{\theta}_0).
  \label{eq:displacement}
\end{equation}
Taking the covariance of Eq.~\ref{eq:displacement} over independent recordings gives the sandwich,
\begin{equation}
  \Sigma \;=\; \mathbf{H}^{-1}\mathbf{J}\,\mathbf{H}^{-1}
...
vanish while $\mathrm{Cov}(S)$ departs from $\mathbf{H}$, and at $\boldsymbol{\theta}_{\mathrm{pool}}$ it
...
Both corrections are first order, and the sandwich is verified directly against the empirical
covariance of the estimates over the simulated recordings and against the coverage of the nominal
region (Figure~\ref{fig:clouds}).
% COMPRESSED 2026-08-26, item "COMPRESS ~30 words, Both are first order" of
% docs/shortening_plan.md. Cut: "and the manuscript does not lean on that" and "which is what an
% expansion cannot certify on its own". SURVIVING COPY: the body's diagnostics section, at "are
% first-order approximations (Appendix 3). We verify it directly, without leaning ...". The residue
% keeps both facts the item requires, the first-order status and the direct verification, and keeps
% the Figure 2 pointer.
...
The sample distortion $\mathbf{C}_s$, built from the within-interval score covariance
$\mathbf{J}_s$, and the correlation distortion $\mathbf{R}$ compose exactly. The exact identity is
...
\begin{equation}
  D_{\mathrm{AI}} \;=\; \Bigl(\textstyle\sum_i \log^2\lambda_i\Bigr)^{1/2}
   \;=\; \sqrt{d\,(\bar e^{\,2}+s^2)},
   % s^2 is the population variance (divisor d, matching the emitter, likelihood.cpp
   % compute_distortion_scalars: var_log = sum_sqr_dev/d); with the sample divisor the identity
   % fails by sqrt(d/(d-1)) on s. Convention stated 2026-08-25 (naive round, CONFIRMED).
  \label{eq:dai}
\end{equation}
