...
observable and the closures below are the only approximation under test. Carrying a general
...
one endpoint (the start) & \texttt{MNR}, \texttt{MR}, \texttt{VR} & interval-mean conductance given the start state \\
...
$\mathrm{av}=0$), \texttt{MNR} (non-recursive, $\mathrm{av}=1$, total start-conditioned
...
\texttt{MR} and \texttt{VR} split the step from \texttt{R} to \texttt{IR}, and \texttt{MNR}
is not separable from \texttt{NR} over the range measured. MacroIR is
\texttt{IR}.} % src: papers/_program/nomenclature.md (the lattice as implemented; the ladder for the Methods presentation); MNR row and supplement reason from sections/06_methods.tex Table 2 caption
\label{tab:ladder}
\end{table}
