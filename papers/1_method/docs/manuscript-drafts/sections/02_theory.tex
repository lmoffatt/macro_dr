...
$\mathrm{av}=0$), \texttt{R} (recursive, $\mathrm{av}=0$), \texttt{MR} (recursive,
...
where $\mathrm{av}$ counts the conditioned endpoints. The body walks
\texttt{LSE}, \texttt{NR}, \texttt{R}, \texttt{IR}; \texttt{MR} and \texttt{VR} split the
step from \texttt{R} to \texttt{IR} and are reported in the Supplement. MacroIR is
\texttt{IR}.} % src: papers/_program/nomenclature.md (the lattice as implemented; the ladder for the Methods presentation)
\label{tab:ladder}
\end{table}
