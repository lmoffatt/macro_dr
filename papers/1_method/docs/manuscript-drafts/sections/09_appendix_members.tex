...
$\bar{v}_{i\to j}$; the start-conditioned mean conductance $\bar{\boldsymbol{\gamma}}_0$ of
Eq.~\ref{eq:gammabar}; the start-conditioned interval variance
$\mathbf{w}$; and, for the members that ignore the acquisition
averaging, the instantaneous per-state conductance vector $\boldsymbol{\gamma}$. Two matrices
...
Eq.~\ref{eq:macror} already has, $\boldsymbol{\gamma}^\top\boldsymbol{\Sigma}\boldsymbol{\gamma}$ and
$\boldsymbol{\Sigma}\boldsymbol{\gamma}$, evaluated there with the boundary objects substituted. Both are written out in Appendix~\ref{app:derivation}, the
bilinear one as the scalar of Eq.~\ref{eq:tilde} and the vector one as the gain
$\mathbf{g}_{0\to\Delta}=\widetilde{\boldsymbol{\gamma}^\top\boldsymbol{\Sigma}}$ of Eq.~\ref{eq:vector-tilde}, and both
collapse to $K\times K$ arithmetic once the boundary covariance Eq.~\ref{eq:sig-bnd} is
substituted.
% COMPRESSED 2026-08-26, item "COMPRESS ~105 words, The tilde operator" of docs/shortening_plan.md,
% executed under its binding adjustment. Cut: "so the K^2 x K^2 object is never built" (SURVIVING
% COPIES: Appendix 1, at the sentence where only K-vectors and K x K matrices survive the
% contraction, and the body's family section) and the closing sentence naming G and H of Eq. GH as
% what tells the two tildes apart, one power of gammabar against two (SURVIVING COPY: Appendix 1,
% where G and H are defined inside Eqs. tilde and vector-tilde and the same contrast is drawn).
% KEPT DELIBERATELY, and the adjustment is explicit about it: the scalar/natural-units parenthetical
% above. The Methods copy of that disambiguation was cut on 2026-08-26, so this is now its SOLE
% survivor in the manuscript. Do not cut it in a later pass without restoring one elsewhere first.
% Eq. GH keeps its label and its definition; G and H are still used by name in switch 1 below.
...
Every one of the six runs the same four steps on each window: propagate the occupancy
...
is $\boldsymbol{\gamma}$, and, with $\boldsymbol{\Sigma}$ read in that mid-window frame,
\begin{equation}
  \widetilde{\boldsymbol{\gamma}^\top\boldsymbol{\Sigma}\boldsymbol{\gamma}} \;\to\; \boldsymbol{\gamma}^\top\boldsymbol{\Sigma}\boldsymbol{\gamma},
  \qquad
  \mathbf{g} \;\to\; \mathbf{P}(\Delta/2)^\top\boldsymbol{\Sigma}\,\boldsymbol{\gamma}.
  \label{eq:av0}
\end{equation}
At the start-conditioned setting the same two expressions hold with $\bar{\boldsymbol{\gamma}}_0$ in
place of $\boldsymbol{\gamma}$ and the full $\mathbf{P}(\Delta)$ in the gain, since the conductance is
...
boundary therefore does two things at once: it replaces $\boldsymbol{\Sigma}_0$ by
$\boldsymbol{\Sigma}_0-\mathrm{diag}(\boldsymbol{\mu}_0)$ and adds the boundary term ($\boldsymbol{\mu}_0^\top
\mathbf{H}\mathbf{1}$ in the variance, $\mathbf{G}^\top\boldsymbol{\mu}_0$ in the gain). The
...
$(\boldsymbol{\mu}^{\mathrm{post}}, \boldsymbol{\Sigma}^{\mathrm{post}})$ into the next window. A
...
Table~\ref{tab:appendix-members} sets the six members against the three switches, and it is
...
\bottomrule
\end{tabularx}
\captionof{table}{The six members as settings of the three switches. Reading down the
...
dispatched are in the supplemental material.}
...
\begin{equation}
  \boldsymbol{\mu}_0^\top\mathbf{w}\big|_{\text{total}}
  \;=\;
  \boldsymbol{\mu}_0^\top\mathbf{w}\big|_{\text{residual}}
  \;+\; \boldsymbol{\mu}_0^\top\mathbf{H}\mathbf{1}
  \;-\; \bar{\boldsymbol{\gamma}}_0^\top\mathrm{diag}(\boldsymbol{\mu}_0)\,\bar{\boldsymbol{\gamma}}_0,
...
ends. The
decompositions differ, the sum does not; what that leaves as the whole of the difference, and why
the equality is between two maps from a prior and not between two recordings, is
Appendix~\ref{app:derivation}.
% COMPRESSED 2026-08-26, item "COMPRESS ~165 words, the A37 paragraph and the Results-steps
% paragraph" of docs/shortening_plan.md. Cut here: the on-a-recording caveat, from "It does not
% follow that the two report the same variance on a recording" to "tested only where the two
% members still hold the same prior". SURVIVING COPY: Appendix 1, at "The equality is therefore
% between two maps from a prior to a predicted variance, and not between two recordings. Once the
% gains differ the priors differ...", which the plan marks as the single statement of the caveat and
% forbids any other pass from cutting. What guards this paragraph against the reading the 2026-08-06
% note below was written about is the surviving "Evaluated at the SAME prior" and "at a common
% prior" in the text above: the claim is never made unqualified. Kept, as the item requires: Eq.
% mr-ir-identity, the sentence identifying its two extra pieces with what switch 1 moves out of the
% third term of Eq. pred-var, "The decompositions differ, the sum does not", and the gain sentence.
...
flips, for the reason given in Appendix~\ref{app:derivation}. The two together are the
\texttt{R}-to-\texttt{IR} gap.
% COMPRESSED 2026-08-26, same item as the paragraph above. Cut: the explanation that the predictive
% variance divides the gain and that the two steps are separable in the algebra without being
% separable in what a recording does with them. SURVIVING COPY: Appendix 1, at "VR removes that
% spread from the third term without restoring it in the second, predicts less variance than either
% from the same state, and its posterior departs from MR's at once", which is the same point made
% on VR specifically. THE CLAUSE "neither step is confined to what it flips" IS KEPT ON PURPOSE,
% with a pointer in place of the explanation: without it the sentence reads as the retired slogan
% the 2026-08-06 note below records, which four repository documents forbid. Nine words is a cheap
% guard against reinstating an error this file has already made once.
...
mean occupancy alone, $\boldsymbol{\mu}_\Delta = \mathbf{P}(\Delta)^\top\boldsymbol{\mu}_0$, predicts
$\bar{y}^{\mathrm{pred}}_{0\to\Delta}$ from Eq.~\ref{eq:ypred}, and assigns to every residual one
scale common to the whole record, marginalised rather than fitted (Methods). There is no gain, so
no occupancy object is ever conditioned on the data.
% COMPRESSED 2026-08-26, item "COMPRESS ~45 words, Least squares, which is off the lattice" of
% docs/shortening_plan.md. Cut: "with no covariance and no update", the gloss that the constant
% variance is fitted as the residual sum of squares over the number of samples, and "Neither term
% of Eq. pred-var that depends on the gating survives". SURVIVING COPIES: the body's family
% section, which establishes that the two least-squares arms sit off the lattice and discard the
% variance, and the Methods passage on the marginalized scale, which is where the fitted variance
% is specified. Kept, as the item requires: the no-setting-of-the-switches statement, the mean-only
% propagation, the Eq. ypred pointer, the constant fitted variance, no gain and nothing conditioned
% on the data, and the anchor-not-a-member verdict.
