...
\textbf{An arrow means the object spans the acquisition window.} $P_{i\to j}(\Delta)$ is a
transition across it, $\bar\gamma_{i\to j}$ an average over it, $\bar y_{0\to\Delta}$ a recorded
value formed from it. \textbf{No arrow means the object lives at one instant}: $\bm{\mu}_0$ and
$\bm{\Sigma}_0$ are the occupancy mean and covariance where the window opens, $\bm{\mu}_\Delta$ and
$\bm{\Sigma}_\Delta$ where it closes. \textbf{A dot in a slot means that end has been collapsed}, as
in $\bar\gamma_{i\to\cdot}$. For a quantity conditioned on the pair the dot denotes the
$P$-weighted collapse, $a_{i\to\cdot}:=\sum_j P_{i\to j}(\Delta)\,a_{i\to j}
=\mathbb{E}[a\mid X_0=i]$, so a collapsed object is defined the moment its pair-resolved version is.
For a probability the weight is already inside and the collapse is the plain margin.

Letters name states and the arrow names the time relation. Collapsing one end does not remove the
arrow, because the object still spans: $\bar\gamma_{i\to\cdot}$ is an average over the window
conditioned on one of its ends, not a quantity at an instant.

\smallskip
\textbf{The arrow says what the object is, not how it was obtained.} $\bm{\Sigma}_\Delta$ carries
no arrow even though a propagator was used to reach it, because it is the covariance of the
occupancy at one time with itself.

\smallskip
A quantity indexed by pairs is written once and read according to the operator it meets, as a
$K\times K$ matrix under an entrywise product and as a vector of length $K^2$ under a contraction
against $\bm{\Sigma}_{0\to\Delta}$, the two being the same array. Throughout, $\circ$ is the
entrywise product and $\mathbf{1}$ the vector of ones. Where a whole recording is in view rather
than one window, a subscript $t$ indexes the window, as in $\bar y_t$; inside a window the two ends
are $0$ and $\Delta$.
\end{tcolorbox}
...
\subsection*{Every symbol against the name it carries in the implementation}

The equations above are written with the objects the implementation forms, and the correspondence
...
\subsection*{What is enough to reproduce this}
...
\paragraph{What was verified, and to what tolerance.} The identities of this section were checked
