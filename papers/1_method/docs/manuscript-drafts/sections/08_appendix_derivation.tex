...
The interval average is not a function of the occupancy at any one time, so no conductance vector
on the $K$ states delivers its mean. On the $K^2$ pairs of states a channel occupies at the two
ends of the window one does, and the construction of every interval member is that substitution
carried out in three steps. Run the update on the pairs, with the pair-conditioned mean interval
conductance $\bar\gamma_{i\to j}$ in place of $\bm{\gamma}$. Charge what the conductance still
varies by once the pair is fixed, its variance $\bar v_{i\to j}$, to the noise term of the update,
where it enters as a variance carried per state. That is legitimate rather than convenient: given
the counts the extra fluctuation has mean zero, so
$\mathrm{Cov}(\mathbf{N},\bar y)=\mathrm{Cov}(\mathbf{N},\mathbb{E}[\bar y\mid\mathbf{N}])$ and it
touches $\sigma^2$ alone, entering neither the predicted mean nor the gain. Then marginalize the
start index, which the next window has no use for. Of the three steps the first two are exact, and
the Gaussian replacement is where the approximation enters.
