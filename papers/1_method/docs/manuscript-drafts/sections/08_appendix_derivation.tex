...
The two moments this asks for are cheap at any $K$. Both are derivatives at $\alpha=0$ of the
tilted semigroup $e^{(\mathbf{Q}+\alpha\bm{\Gamma})\Delta}$, the generator biased by the
conductance, with $\bm{\Gamma}=\mathrm{diag}(\bm{\gamma})$, so each costs one matrix exponential
and no object larger than $K\times K$ is formed in building them.
