...
recorded value correlates them, so from the first update onward $\bm{\Sigma}$ is
...
\[
  \sum_{k,l}(\bm{\Sigma}_{0\to\Delta})_{(i,j),(k,l)}\,\bar{\gamma}_{k\to l},
\]
...
conditioning on a single channel's $X_0$ returns the same expressions only while $\bm{\Sigma}_0$ is
...
Eq.~\ref{eq:cdiff} does not depend on $\bm{\Sigma}_0$ at all. The same deviation is invisible in
...
\begin{equation}
  ...
  \qquad
  \bm{\Sigma}^{\mathrm{post}}=\bm{\Sigma}^{\mathrm{prior}}
...
being $\bm{\Sigma}$ throughout this paper. It is Eq.~\ref{eq:macror} up to the per-channel convention, the source of the factors of
...
\begin{equation}
  \bar y = \bar{\bm{\gamma}}_0^\top\mathbf{N}_0 + v(\mathbf{N}_0),
  \qquad
