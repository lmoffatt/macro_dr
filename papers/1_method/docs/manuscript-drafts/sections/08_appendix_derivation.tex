...
appendix carries the derivation behind those statements, in the order it is built: the law of the
interval average and why it has no closed form, the boundary-conditioned moments and how they are
evaluated, the three steps that turn the instantaneous update into the interval one, the
start-conditioned members stated from their own assumptions, the two algebraic differences that
separate them from the boundary-conditioned one, and both held against the exact law. Three
subsections close it and depend on none of the others: the correspondence with the
linear-filtering literature, what the uniform acquisition window misses against a real amplifier,
and what was checked numerically and to what tolerance. It is written to be checked rather
...
$\boldsymbol{\mu}_0,\boldsymbol{\Sigma}_0$ are the occupancy mean and covariance where the window opens,
$\boldsymbol{\mu}_\Delta,\boldsymbol{\Sigma}_\Delta$ where it closes. \textbf{A dot marks a collapsed end}:
...
Every moment in the construction is carried on a single channel. Writing $\mathbf{X}$ for the
indicator vector of the state one channel occupies, which is one on that state and zero
elsewhere, $\boldsymbol{\mu}=\mathbb{E}[\mathbf{X}]$ is its probability vector and
$\boldsymbol{\Sigma}=\mathrm{Cov}(\mathbf{X})$ its covariance, which is
$\mathrm{diag}(\boldsymbol{\mu})-\boldsymbol{\mu}\boldsymbol{\mu}^\top$ for a channel nothing has been observed about
and vanishes when its state is known. Per channel is a normalization the whole construction is
carried in, and not a claim that the channels stay independent under it: conditioning on a shared
recorded value correlates them, so from the first update onward $\boldsymbol{\Sigma}$ is
$\mathrm{Cov}(\mathbf{N})/N_{\mathrm{ch}}$ and not one channel's own covariance. Before any
observation the channels are independent of each other, so the counts
$\mathbf{N}=\sum_c \mathbf{X}^{(c)}$ over the ensemble have
$\mathbb{E}[\mathbf{N}]=N_{\mathrm{ch}}\boldsymbol{\mu}$ and
$\mathrm{Cov}(\mathbf{N})=N_{\mathrm{ch}}\boldsymbol{\Sigma}$, which is where every factor of
...
% RECIBIDO 2026-08-27 desde 02_framework.tex (T9) Y RETIRADO EL MISMO DIA, cuando la opcion B
% devolvio al cuerpo una ecuacion de frontera (Ec. macroir) y con ella la definicion. Lo que se
% habia aterrizado aqui quedaba dicho TRES veces: la convencion de la flecha ya esta en el recuadro
% "Three marks, and each one says one thing" al abrir este apendice; el "no explicit K^2 object" ya
% esta en el parrafo de abajo ("with no object larger than K x K formed anywhere"); y la definicion
% mas el encadenamiento pasaron al cuerpo con Ec. macroir. El diagnostico de T9 seguia siendo
% correcto (la notacion estaba varada en el cuerpo porque ninguna ecuacion la usaba); lo que cambio
% es cual de las dos reparaciones opuestas se tomo.
...
semigroup $e^{(\mathbf{Q}+\alpha\boldsymbol{\Gamma})\Delta}$, the generator biased by the conductance.
Writing
\begin{equation}
  \mathbf{F}_1(\Delta)=\left.\frac{\partial}{\partial\alpha}
      e^{(\mathbf{Q}+\alpha\boldsymbol{\Gamma})\Delta}\right|_{\alpha=0},
  \qquad
  2\,\mathbf{F}_2(\Delta)=\left.\frac{\partial^{2}}{\partial\alpha^{2}}
      e^{(\mathbf{Q}+\alpha\boldsymbol{\Gamma})\Delta}\right|_{\alpha=0},
  \label{eq:tilted-derivatives}
\end{equation}
with $\boldsymbol{\Gamma}=\mathrm{diag}(\boldsymbol{\gamma})$, the $(i\to j)$ entry of the first is
...
was run on. Writing $\mathbf{Q}=\mathbf{V}\boldsymbol{\Lambda}\mathbf{W}$, with
$\boldsymbol{\Lambda}=\mathrm{diag}(\lambda_1,\dots,\lambda_K)$ the eigenvalues, $\mathbf{V}$ the
matrix of right eigenvectors and $\mathbf{W}=\mathbf{V}^{-1}$, the transition matrix is
$\mathbf{P}(\Delta)=\mathbf{V}e^{\boldsymbol{\Lambda}\Delta}\mathbf{W}$ and the two derivatives are
the same sandwich with the conductance carried in the eigenbasis. With
$\widehat{\boldsymbol{\Gamma}}=\mathbf{W}\boldsymbol{\Gamma}\mathbf{V}$ and $\circ$ the entrywise product,
\begin{align}
  \frac{\mathbf{F}_1(\Delta)}{\Delta}
    &= \mathbf{V}\bigl(\widehat{\boldsymbol{\Gamma}}\circ\mathbf{E}_2\bigr)\mathbf{W},
...
\begin{equation}
  \mathcal{Q}_2=\begin{pmatrix}\mathbf{Q} & \boldsymbol{\Gamma}\\ \boldsymbol{0} & \mathbf{Q}\end{pmatrix},
  \qquad
  \mathcal{Q}_3=\begin{pmatrix}\mathbf{Q} & \boldsymbol{\Gamma} & \boldsymbol{0}\\
                               \boldsymbol{0} & \mathbf{Q} & \boldsymbol{\Gamma}\\
                               \boldsymbol{0} & \boldsymbol{0} & \mathbf{Q}\end{pmatrix},
  \label{eq:augmented}
\end{equation}
...
$e^{\mathcal{Q}_3\Delta}$ on its own returns the transition matrix and both moments with no
spectral machinery, and where no reliable eigensolver is at hand it is the construction to
use. The route taken here is the spectral one because this paper propagates the derivative
...
parameters in closed form, by a first-order perturbation of the same decomposition. Every
number reported in this paper comes from the spectral route.
% COMPRESSED 2026-08-26, item "COMPRESS ~85 words, Van Loan alternative" of
% docs/shortening_plan.md. Kept, as the item's residue requires: Eq. augmented with its
% vanloan1978computing citation, the one sentence that it returns the transition matrix and both
% moments with no spectral machinery and is the construction to use where no reliable eigensolver
% is at hand, and the one sentence that the spectral route was chosen for the closed-form parameter
% derivatives, with every reported number coming from it. Discussion's "the derivative of the
% spectral reconstruction inside it (Appendix 1)" still lands on that sentence.
% Cut: the 3K-block derivative-payload tour ("an object of side 3K whose nine blocks hold three
% distinct matrices") and the "the reason is the derivatives rather than the exponential" preamble.
% NOTE for whoever reads the caveat comment below: with the payload clause gone the text now makes
% NO claim about the cost of the augmented route at all, which is a stronger form of what that
% caveat asks for, so the caveat stands unviolated and is kept for the record.
...
\begin{equation}
  \mathbb{E}\bigl[\bar y \bigm| \mathbf{N}\bigr]=\boldsymbol{\gamma}^\top\mathbf{N},
  \qquad
...
\begin{equation}
  \sigma^2 \;=\; \epsilon^2_{0\to\Delta}
    \;+\; N_{\mathrm{ch}}\,\boldsymbol{\gamma}^\top\boldsymbol{\Sigma}\boldsymbol{\gamma}
    \;+\; N_{\mathrm{ch}}\,\boldsymbol{\mu}^\top\mathbf{w},
  \label{eq:macror-w}
\end{equation}
with every other line of Eq.~\ref{eq:macror} unchanged. Setting $\mathbf{w}=\boldsymbol{0}$ returns the
...
\[
  \underbracket{\boldsymbol{\mu}^\top\boldsymbol{\gamma}}_{\text{the mean}}
  \qquad
  \underbracket{\boldsymbol{\mu}^\top\mathbf{w}}_{\text{the state-dependent noise}}
  \qquad
  \underbracket{\boldsymbol{\gamma}^\top\boldsymbol{\Sigma}\boldsymbol{\gamma}}_{\text{what the state contributes}}
  \qquad
  \underbracket{\boldsymbol{\Sigma}\boldsymbol{\gamma}}_{\text{state against current},\ \mathbf{g}}
\]
two against $\boldsymbol{\mu}$ and two against $\boldsymbol{\Sigma}$, and everything else in Eq.~\ref{eq:macror} is
a scalar. Extending the filter to a recorded value that spans the window therefore cannot require a
new algorithm. It requires choosing a state variable and supplying $(\boldsymbol{\gamma},\mathbf{w})$ on
...
% RECORTADO 2026-08-27, segunda pasada. La apertura repetia casi palabra por palabra las dos
% oraciones que el cuerpo adquirio ESE MISMO DIA alrededor de Ec. macroir, y despues apuntaba a esa
% ecuacion igual. Verificado sobre texto vivo: el cuerpo dice "no conductance vector on the K states
% delivers its mean" y "A channel that starts in i and ends in j contributes gammabar_{i->j} on
% average, so the interval average is linear in the boundary counts", y abre con la misma expresion
% "On the K^2 pairs ... one does". Queda la premisa una vez, el puntero, y los tres pasos intactos,
% que es lo que la nota del 2026-08-26 exige que esta subseccion conserve.
No conductance vector on the $K$ states delivers the mean of an interval average, and on the
$K^2$ boundary pairs one does. That substitution is Eq.~\ref{eq:macroir} of the body, and three
steps carry it out here. The two counts are one: the first three substitutions of
Eq.~\ref{eq:macroir} are step one, its fourth is step two, and the start index the body sums away
is step three.
% PUNTERO ANADIDO 2026-08-27: el cuerpo ahora MUESTRA la sustitucion (Ec. macroir), asi que esta
% subseccion pasa a ser su desarrollo y no su unica sede. Sin el puntero las dos se leen como dos
% enunciados independientes de lo mismo.
Run Eq.~\ref{eq:macror} on the pairs, with $\bar{\gamma}_{i\to j}$ in place of
$\boldsymbol{\gamma}$ and the boundary covariance in place of $\boldsymbol{\Sigma}$. Charge what the
...
boundaries and marginalizes the trajectory between them analytically. It is not a transition state
in the mechanistic sense, and the word \emph{transition} is not used for it anywhere in this paper.
% COMPRESSED 2026-08-26, item "COMPRESS ~40 words, static condensation" of docs/shortening_plan.md,
% which executes duplication (f). Cut: the clause "and chaining intervals leaves a single state
% variable per junction, since the end state of one interval is the start state of the next".
% SURVIVING COPY: the body's family section, where the reader first meets the chaining point.
% Kept deliberately, as the item requires: the static-condensation and spatial-Markov analogy, which
% appears nowhere else in the paper, and the "not a transition state in the mechanistic sense"
% disclaimer.
...
$(\boldsymbol{\mu}_0,\boldsymbol{\Sigma}_0)$ be the channel's state at the window's start. It leaves $i$ and
lands in $j$ with probability $P_{i\to j}(\Delta)$, so
\begin{align}
  \mu_{i\to j} &= \mu_{0,i}\,P_{i\to j}(\Delta), \label{eq:mu-bnd}\\
  (\boldsymbol{\Sigma}_{0\to\Delta})_{(i\to j),(k\to l)} &=
    P_{i\to j}(\Delta)\,\bigl(\boldsymbol{\Sigma}_0-\mathrm{diag}(\boldsymbol{\mu}_0)\bigr)_{i k}\,
    P_{k\to l}(\Delta)
    \;+\; \bigl[\mathrm{diag}(\boldsymbol{\mu}_{0\to\Delta})\bigr]_{(i,j),(k,l)}.
  \label{eq:sig-bnd}
\end{align}
...
because a channel takes exactly one of them. Subtracting $\mathrm{diag}(\boldsymbol{\mu}_0)$ in the
...
\begin{align}
  \boldsymbol{\mu}_\Delta &= \mathbf{P}(\Delta)^\top \boldsymbol{\mu}_0, \label{eq:mu-prop}\\
  \boldsymbol{\Sigma}_\Delta &= \mathbf{P}(\Delta)^\top\bigl(\boldsymbol{\Sigma}_0-\mathrm{diag}(\boldsymbol{\mu}_0)\bigr)\mathbf{P}(\Delta) + \mathrm{diag}(\boldsymbol{\mu}_\Delta), \label{eq:sig-prop}
\end{align}
...
\begin{equation}
  \bar{y}^{\mathrm{pred}}_{0\to\Delta} \;=\; N_{\mathrm{ch}}\,\boldsymbol{\mu}_0^\top \bar{\boldsymbol{\gamma}}_0,
  \label{eq:ypred}
\end{equation}
since $\boldsymbol{\mu}_{0\to\Delta}$ contracted against $\bar{\gamma}$ is Eq.~\ref{eq:gammabar}
summed against the prior, and its variance has three terms,
\begin{equation}
  \sigma^2_{0\to\Delta} \;=\; \epsilon^2_{0\to\Delta}
  \;+\; N_{\mathrm{ch}}\,\widetilde{\boldsymbol{\gamma}^\top\boldsymbol{\Sigma}\boldsymbol{\gamma}}
  \;+\; N_{\mathrm{ch}}\,\boldsymbol{\mu}_0^\top\mathbf{w}.
  \label{eq:pred-var}
\end{equation}
...
$\boldsymbol{\gamma}^\top\boldsymbol{\Sigma}\boldsymbol{\gamma}$ of Eq.~\ref{eq:macror} evaluated on the pairs, and the
tilde is what marks that: under it the two $\boldsymbol{\gamma}$ stand for
$\bar{\gamma}_{i\to j}$ and $\boldsymbol{\Sigma}$ for the $K^2\times K^2$ boundary covariance,
not for the $K$-vector and the $K\times K$ matrix they name everywhere else. Written out with $\boldsymbol{\Sigma}_{0\to\Delta}$
of Eq.~\ref{eq:sig-bnd} and then collapsed by substituting it,
\begin{align}
  \widetilde{\boldsymbol{\gamma}^\top\boldsymbol{\Sigma}\boldsymbol{\gamma}}
  &\;=\; \sum_{i,j}\ \sum_{k,l}
    \bar{\gamma}_{i\to j}\,
    \bigl(\boldsymbol{\Sigma}_{0\to\Delta}\bigr)_{(i,j),(k,l)}\,
    \bar{\gamma}_{k\to l} \nonumber\\[2pt]
  &\;=\; \bar{\boldsymbol{\gamma}}_0^\top\bigl(\boldsymbol{\Sigma}_0-\mathrm{diag}(\boldsymbol{\mu}_0)\bigr)\bar{\boldsymbol{\gamma}}_0
    \;+\; \boldsymbol{\mu}_0^\top\bigl(\mathbf{H}\mathbf{1}\bigr),
  \label{eq:tilde}
\end{align}
...
$\bar{\boldsymbol{\gamma}}_0^\top\boldsymbol{\Sigma}_0\bar{\boldsymbol{\gamma}}_0$ keeps the first summand and drops
...
\[
  \sum_{k,l}(\boldsymbol{\Sigma}_{0\to\Delta})_{(i,j),(k,l)}\,\bar{\gamma}_{k\to l},
\]
% DISPLAYED 2026-08-30: inline, the sum cannot break and ran past the right margin on page 38 of the
% 02:02 build (Luciano). Same formula, nothing else changed.
and step three sums its start index away, leaving the
$\mathbf{g}_{0\to\Delta}$ the interval update uses,
\begin{equation}
  \mathbf{g}_{0\to\Delta} \;=\; \widetilde{\boldsymbol{\gamma}^\top\boldsymbol{\Sigma}}
  \;=\; \mathbf{P}(\Delta)^\top\bigl(\boldsymbol{\Sigma}_0-\mathrm{diag}(\boldsymbol{\mu}_0)\bigr)\bar{\boldsymbol{\gamma}}_0
...
\begin{align}
  \boldsymbol{\mu}^{\mathrm{post}} &= \boldsymbol{\mu}_\Delta + \frac{\mathbf{g}_{0\to\Delta}}{\sigma^2_{0\to\Delta}}\,\delta, \label{eq:mu-post}\\
  \boldsymbol{\Sigma}^{\mathrm{post}} &= \boldsymbol{\Sigma}_\Delta - \frac{N_{\mathrm{ch}}\,\mathbf{g}_{0\to\Delta}\mathbf{g}_{0\to\Delta}^\top}{\sigma^2_{0\to\Delta}}. \label{eq:sig-post}
\end{align}
...
% VERIFIED before writing, papers/1_method/decisions/recompute/macroir_from_macror_boundary.py: MacroR run on the K^2 pairs and
...
from the same place. All three of $\boldsymbol{\mu}$, $\boldsymbol{\Sigma}$ and $\mathbf{g}_{0\to\Delta}$ belong to one
...
$\mathrm{Cov}(\mathbf{N}_\Delta)=N_{\mathrm{ch}}\boldsymbol{\Sigma}_\Delta$. The rank-one
...
% COMPRESSED 2026-08-26, item "COMPRESS ~80 words, The start-conditioned members, stated forward"
% of docs/shortening_plan.md. Cut here: the "usually presented as abbreviations / they are not /
% the difference matters" framing, which restated the subsection's purpose. Cut at the end of the
% MR paragraph below: "Neither needs a state larger than K" (said one sentence earlier as "complete
% without any object larger than the K states") and the "This is worth stating plainly" preamble.
% Kept, as the item's residue requires: Eq. mr-assumption with everything MR does following from it
% and needing no object larger than the K states; Eq. mr-objects identified with Eq. gammabar and
% Eq. macror-w; and the row-sums sentence ending "reached without ever forming a pair". The falsity
% paragraph below is on the untouched list and is not edited.
The construction above reaches \texttt{IR} by choosing the boundary pair as the state. The members
that stop at the interval's start are stated here from their own assumptions rather than as that
construction with the collapse brought forward.
...
Eq.~\ref{eq:macror-w} then asks for. Both come from the same
...
\texttt{MR} requires are the row sums, and the row sums are integrals of $K$-vectors, so
\texttt{MR} is reached without ever forming a pair.
...
$\bar{\boldsymbol{\gamma}}_0$ in place of $\boldsymbol{\gamma}$, two
...
states,
\begin{equation}
  \underbracket{\sum_{j} P_{i\to j}(\Delta)\bigl[\bar{v}_{i\to j}
    + \bigl(\bar{\gamma}_{i\to j}-\bar{\gamma}_{i\to\cdot}\bigr)^2\bigr]}_{\text{total, }\texttt{MR}}
  \;=\; \underbracket{\sum_{j} P_{i\to j}(\Delta)\,\bar{v}_{i\to j}}_{\text{residual, }\texttt{VR},\,\texttt{IR}}
...
$\boldsymbol{\mu}_0^\top\mathrm{Var}_{j}[\bar{\gamma}_{i\to j}\mid i]$, which is what
...
of what separates them is the gain, an identity written out at
Eq.~\ref{eq:mr-ir-identity}. In the other direction, the predicted
...
$(\boldsymbol{\mu}_0,\boldsymbol{\Sigma}_0)$ that law has exact first two moments which either member can be held
...
\begin{equation}
  ...
  + \underbracket{\mathbb{E}\bigl[\mathrm{Var}(\bar y \mid \mathbf{N}_0)\bigr]}_{N_{\mathrm{ch}}\,\boldsymbol{\mu}_0^\top\mathbf{w}}
  + \underbracket{\mathrm{Var}\bigl[\mathbb{E}(\bar y \mid \mathbf{N}_0)\bigr]}_{N_{\mathrm{ch}}\,\bar{\boldsymbol{\gamma}}_{0}^\top\boldsymbol{\Sigma}_0\bar{\boldsymbol{\gamma}}_{0}} ,
  \label{eq:ltv}
\end{equation}
which is Eq.~\ref{eq:pred-var} read with the start-conditioned objects: the middle term is
$\bar{\boldsymbol{\gamma}}_0^\top\boldsymbol{\Sigma}_0\bar{\boldsymbol{\gamma}}_0$, the tilde dropped, and $\mathbf{w}$ is
...
variance of the recorded value} given $(\boldsymbol{\mu}_0,\boldsymbol{\Sigma}_0)$, so they agree with each other
for the reason that each agrees with the truth, and nothing is measured by that agreement.
% COMPRESSED 2026-08-26, item "COMPRESS ~115 words, The variance: both members are exact" of
% docs/shortening_plan.md. Cut here: "This is Eq. mr-vr read from the outside: the same second
% moment sits in the state term of one member and in the noise term of the other, which is why the
% totals coincide", the third statement of the MR/IR variance identity. SURVIVING ANCHORS: Eq.
% mr-vr itself, with its two underbraces, and Eq. mr-ir-identity in Appendix 2 as the switch-level
% algebra; the Methods table caption cites Eq. mr-vr for the state-dependent noise and is unaffected.
% Cut in the paragraph below: the intermediate algebra reading Eq. emission into the second
% underbrace and supplying the factor of N_ch, leaving the one-sentence caveat the item requires.
The variable conditioned on in Eq.~\ref{eq:ltv} is the count vector and not one channel's state:
conditioning on a single channel's $X_0$ returns the same expressions only while $\boldsymbol{\Sigma}_0$ is
still that channel's own covariance, which it stops being at the first update.
...
\begin{equation}
  ...
  = \underbracket{\frac{1}{N_{\mathrm{ch}}}\mathrm{Cov}\bigl[\mathbb{E}(\mathbf{N}_\Delta\mid \mathbf{N}_0),\ \bar y\bigr]}_{\textstyle \mathbf{g}^{\mathtt{MR}}_{0\to\Delta}}
...
$\mathbf{g}^{\mathtt{MR}}_{0\to\Delta}=\mathbf{P}(\Delta)^\top\boldsymbol{\Sigma}_0\bar{\boldsymbol{\gamma}}_0$ exactly. The
...
\begin{equation}
  ...
  \label{eq:cdiff}
\end{equation}
which is Eq.~\ref{eq:vector-tilde} minus $\mathbf{P}(\Delta)^\top\boldsymbol{\Sigma}_0\bar{\boldsymbol{\gamma}}_0$
...
Eq.~\ref{eq:cdiff} does not depend on $\boldsymbol{\Sigma}_0$ at all. The same deviation is invisible in
one block and decisive in the other because it enters the variance squared and the covariance
once: a second moment can sit in either of the two variance terms without the total noticing,
which is Eq.~\ref{eq:mr-vr}, while a collapsed first moment has nowhere to go, a noise term having
no first moment. That asymmetry between $D^2$ and $D$ is the whole of the difference between the
two members, and it is a consequence of Eq.~\ref{eq:mr-assumption} rather than a defect of the
arithmetic that implements it.
% COMPRESSED 2026-08-26, item "COMPRESS ~85 words, Why the same quantity is invisible in one block
% and decisive in the other" of docs/shortening_plan.md. The subsection header is gone and the two
% sentences the item asks for are folded onto the end of the covariance paragraph above, which is
% where the deviation D is defined. Nothing else in the paper states the D-squared-versus-D
% asymmetry, so it is compressed rather than dropped; "column profile", which occurred only in the
% cut text, is gone with it.
...
The correspondence with that literature is nevertheless exact; the relation is conceded in the
Discussion and placed against the prior art in Appendix~\ref{app:priorart}, and the equations that
% PUNTERO ANADIDO 2026-08-27: el unico destino que daba era "the Discussion", y la oracion viva de
% la Discusion se redujo ese mismo dia a un reenvio al Apendice 5, asi que el lector hacia dos
% saltos para llegar a donde esta la posicion contra la literatura.
make it checkable are here.
% COMPRESSED 2026-08-26, item "COMPRESS ~45 words, opening of the linear-filtering-frame
% subsection" of docs/shortening_plan.md. Cut: the "forty years of work on the same measurement
% problem" and "rather than take it on faith" ornament, which restated the Discussion's concession.
% SURVIVING COPY: the Discussion, where the concession is made and the specificity case is argued;
% the sentence kept here holds the pointer to it.
...
\begin{equation}
  ...
  \qquad
  \boldsymbol{\Sigma}^{\mathrm{post}}=\boldsymbol{\Sigma}^{\mathrm{prior}}
    -\frac{\mathrm{Cov}(\mathbf{x},y)\,\mathrm{Cov}(\mathbf{x},y)^\top}{\mathrm{Var}(y)} ,
  \label{eq:kf}
\end{equation}
written in the symbols used here, the state covariance that literature writes $\mathbf{P}$ for
being $\boldsymbol{\Sigma}$ throughout this paper. It is Eq.~\ref{eq:macror} up to the per-channel convention, the source of the factors of
...
The standard difficulty is that $\bar y$ is not a function of the state at any single time, so it
is not a fixed linear functional of $\mathbf{x}_\Delta$ plus noise. The device is
state augmentation: carry the running integral
$z_t=\int_0^{t}\boldsymbol{\gamma}^\top\mathbf{N}(s)\,\mathrm{d}s$ alongside the counts, so that
...
three blocks are
\begin{align}
  \mathbb{E}\bigl[\bar y\bigr] &= N_{\mathrm{ch}}\,\boldsymbol{\mu}_0^\top\bar{\boldsymbol{\gamma}}_0,
  \label{eq:kf-mean}\\
  ...
  \frac{\mathrm{Cov}\bigl[\mathbf{N}_\Delta,\bar y\bigr]}{N_{\mathrm{ch}}}
    ...
    = \mathbf{P}(\Delta)^\top\boldsymbol{\Sigma}_0\bar{\boldsymbol{\gamma}}_0
      + \Bigl(\textstyle\sum_{i}\mu_{i\to j}D_{i\to j}\Bigr)_{j} .
  \label{eq:kf-cross}
\end{align}
...
so they perform the same update and return the same posterior over the occupancy.

% COMPRESSED 2026-08-26, item "COMPRESS ~90 words, the 'Three things follow' paragraph" of
% docs/shortening_plan.md. Cut: the "Three things about that follow" announcement and the preamble
% clause of each of the three, one sentence now carrying each. All three facts are retained as the
% item requires, the reset caveat verbatim in substance, and the "device not information" point
% keeps its cross-reference to Eqs. mu-prop and sig-prop.
The augmented state has $K+1$ entries against the boundary state's $K^2$, so \emph{the pair is a
device of the derivation and not information the filter must carry}, which is the same conclusion
Eqs.~\ref{eq:mu-prop} and~\ref{eq:sig-prop} reached from the other side. Given
$(\mathbf{N}_\Delta, z_\Delta)$ the recorded value is deterministic, so what
Eq.~\ref{eq:pred-var} splits between its second and third terms sits entirely inside
$\mathrm{Var}(z_\Delta)$, and that is why the cross block needs only $\mathbf{F}_1$ while the
variance block needs $\mathbf{F}_2$ as well. The equivalence holds per window, with a reset: the
augmented filter must discard the posterior on $z$ and open the next window with $z=0$, because
the next window's integral is a fresh one and is independent of the old given the occupancy.

\texttt{MR} is not the augmented filter under any choice of blocks. It is the filter for a
misspecified measurement model,
\begin{equation}
  \bar y = \bar{\boldsymbol{\gamma}}_0^\top\mathbf{N}_0 + v(\mathbf{N}_0),
  \qquad
...
linear, and replaces the law by a Gaussian only at the two places the box above names, so the two
routes arrive at the same equations because the conditioning of a joint Gaussian consumes nothing
but the first two moments. The pair survives as the presentation even though the augmented state
reaches the same blocks with fewer entries: on the pair the recorded average is linear in
the state with no conditional expectation taken, the collapse hands over the next window's prior at
no cost, and the per-state variance slot is what makes \texttt{R}, \texttt{MR} and \texttt{IR} one
construction with three readings.
% COMPRESSED 2026-08-26, item "COMPRESS ~180 words, closing paragraph of the linear-filtering-frame
% subsection" of docs/shortening_plan.md. Cut: the two "consequences" sentences. The
% imported-measurement-equation point is already carried by Eq. mr-kf and by the DE-KALMANED note
% above; the Sigma meaning-versus-convention point survives in Methods, at the sentence on the one
% moment at which Sigma genuinely is a single channel's covariance. Also cut: "The augmentation
% gives no comparably natural form to the other two members." Kept, as the item requires: the
% linear-Gaussian presumption and why occupancy counts are not one, the exact-two-moments plus
% two-closures statement with the reason the routes agree, and the sentence on why the pair remains
% the presentation. Residue is ~170 words rather than the ~95 the item estimates, because those
% three retained contents do not compress further without dropping one of them.
...
% COMPRESSED 2026-08-26, item "COMPRESS ~60 words, acquisition-kernel subsection" of
% docs/shortening_plan.md. Two pieces of rhetorical scaffolding are gone, the opening "Four
% properties of that table matter more than the numbers in it" and the closing "The two concessions
% are one concession"; all four numeric facts the item protects are untouched, and the surviving
% statement that the residue lands in the instrumental-noise partition where the Discussion already
% argues an unmeasured departure of the noise model lives now carries the join by itself. Actual
% saving ~21 words, not the ~60 the item estimates: the item names only these two sentences as
% removable and everything else in the paragraph is one of the protected facts.
From $f_c\Delta=1$ upward the
...
which is where the Discussion already argues an unmeasured departure of the noise model lives.
...
\subsection*{What was verified, and to what tolerance}
 ...
 The identities of the boundary-state construction were checked
...
Eq.~\ref{eq:cdiff} holds to $1\times10^{-17}$ in $36$ cases, independently of $\boldsymbol{\Sigma}_0$, and
...
% src: papers/1_method/decisions/recompute/mr_vs_ir_boxes.py and papers/1_method/decisions/recompute/macroir_from_macror_boundary.py.
