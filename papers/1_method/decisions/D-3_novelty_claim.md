...
- **The Comm Biol control is this paper's real-data demonstration** (ADDED 2026-07-29). Comm Biol 2025 already ranked **nine** kinetic schemes by Bayesian evidence with MacroIR and repeated the ranking with MacroINR as a control: *"Model ranking was sensitive to likelihood approximation: the control method (MacroINR) systematically underestimated evidence for schemes with conformational intermediates"*, and *"leading to systematically different evidence values (Supplementary Table S1)"*. So **"an invalid likelihood corrupts model comparison" is not a promise in this paper, it is a published, real-data observation**, and what eLife adds is why, by how much, and where. Two consequences to handle deliberately: the winning margin over the top non-conformational alternative is only **6.4** (5.5-6.7), which is the scale a systematic likelihood distortion can move; and the state of the code behind those evidences, which this line called an open question when it was written. **That question is now closed, 2026-07-31, and the answer is the opposite of what was feared** (corrected 2026-08-10): the regression that dropped the `N·ms` interval-variance term entered at `a3e0a89` (2025-12-02), *after* submission, and the submitted source `macro_dr_submission b4a0e28` adds the term in the same place, gated on the `variance` flag alone (`../../_program/nomenclature.md:68-76`, verified against the submission repository). The defective build is `NMR` and it names only what ran in this repository between `a3e0a89` and `1f7138b`; it is not what produced the published evidences. Do not re-raise this without new evidence about the submitted source. Separately and still open, on a different object: the `gvar_i` against `Σ_j P_ij gvar_ij` reading of the Table S4 validation flags, which the audit of 2026-06-26 found to be genuine misspecification and not a code error. See `../decisions.md`.
...
- ~~**MR sign**~~ **CLOSED (D-4 §2.1): over-confident, ~1.6 at the headline cell, recomputed on the freeze. The prose copies that say "overestimates variance" have it backwards for the parameter covariance, which is what "over-confident" names. The observable-variance direction is a different object and is now settled (2026-08-06): equal to IR's at the same prior, and 69% to 81% above IR once the differing gains have driven the priors apart. Do not read one as the other. See D-4 §2.1 and `../figures_build_plan.md:195-245`.**
