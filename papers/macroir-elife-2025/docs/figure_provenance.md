...
**What it computes.** A single recording, no grid, no ensemble, no MLE, no bootstrap. Twenty channels, six measurement intervals at 50 kHz with 100 raw samples averaged into each, giving 2 ms per interval and a 12 ms recording; agonist is 0 during interval 0 and 10 for intervals 1 to 5. One trajectory is simulated with 250 sub-steps per interval. That one recording is then passed through all five likelihood algorithms via `calc_likelihood_diagnostic`, writing one long-format CSV per algorithm (`figure_1_likelihood_diagnostic_{NR,R,MNR,MR,IR}.csv`) plus the sub-resolved trajectory (`figure_1_simulation.csv`). Two further IR-only files are written and no notebook reads them.

**One inconsistency to be aware of.** This script builds NR and R with `variance_approximation = 0` while MNR, MR and IR get `variance_approximation = 1` (`figure_1.macroir:41, 47, 53, 59, 66`). Runs 2, 3 and 4 use `variance_approximation = true` for all five. Figure 1 is illustrative rather than quantitative, so this does not propagate into any reported number, but it should not be described as "the same five builds as the rest of the paper".

**Notebook.** `figures/paper/figure_1.Rmd` reads those six CSVs (with no `skip`, see §2), and assembles a five-column by four-row panel: prior open probability, predicted current with its spread and the innovation, posterior (NR and MNR are marked "no update (open loop)"), and cumulative log-likelihood. It calls `build_figure` twice, producing `Figure_1.pdf` from samples 3 and 4 (the 6 to 10 ms zoom) and `Figure_S1.pdf` from samples 0 to 4. Both are saved at 7.0 by 7.5 inches.
...
The canonical arc is `results_plan.md` (2026-07-14), which supersedes the figure list in `00_master_plan_v2.md` §5.
...
| 5 | none selected | none | intended `1c2ae6f` | **not built.** Candidates in `figures/in_progress/`, sorted by `figure_5_PLAN.md`. `results_plan.md:95` calls it the critical-path figure |
...
1. **Correct the seed claim** in `methods_plan.md` M5, and decide what the paper says about it. The honest sentence is that the ensembles are large enough (1000 to 10000 recordings) that the reported statistics are stable, but the exact datasets cannot be regenerated bit for bit. Add the resolved seed to `meta.json` before any rerun.
...
4. **Decide the anchor split** (`methods_plan.md` M10). Today it is forced, not chosen: MacroNMR does not exist in the Gaussian family at all. Either run the missing cells or state and justify the split.
